...
public class CacheConfig {
    ...
    private CaffeineCacheManager buildCaffeineManager(
        ...
        manager.registerCustomCache(CacheNames.BOOKING_BY_ID, Caffeine.newBuilder()
                .expireAfterWrite(readTtlSeconds, TimeUnit.SECONDS)
                .maximumSize(readMaxSize)
                .recordStats()
                .build());
        manager.registerCustomCache(CacheNames.BOOKINGS_BY_CUSTOMER, Caffeine.newBuilder()
                .expireAfterWrite(readTtlSeconds, TimeUnit.SECONDS)
                .maximumSize(readMaxSize)
                .recordStats()
                .build());
        manager.registerCustomCache(CacheNames.BOOKING_SLOTS, Caffeine.newBuilder()
                .expireAfterWrite(readTtlSeconds, TimeUnit.SECONDS)
                .maximumSize(readMaxSize)
                .recordStats()
                .build());
    ...
    private RedisCacheManager buildRedisManager(
        ...
        Map<String, RedisCacheConfiguration> configs = Map.of(
                CacheNames.BOOKING_BY_ID, readConfig,
                CacheNames.BOOKINGS_BY_CUSTOMER, readConfig,
                CacheNames.BOOKING_SLOTS, readConfig,
