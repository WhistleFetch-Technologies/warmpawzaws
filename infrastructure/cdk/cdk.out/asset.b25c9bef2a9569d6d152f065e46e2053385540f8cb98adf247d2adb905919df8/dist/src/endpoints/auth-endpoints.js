...
const authService = __importStar(require("../../../supabase/functions/make-server-3dd53475/auth-service"));
const otp_1 = require("../../../supabase/functions/make-server-3dd53475/lib/repositories/otp");
const db_1 = require("../../../supabase/functions/make-server-3dd53475/lib/db");
const client_sns_1 = require("@aws-sdk/client-sns");
const response_utils_1 = require("../../../supabase/functions/make-server-3dd53475/response-utils");
const cognitoHelper = __importStar(require("../../../supabase/functions/make-server-3dd53475/cognito-helper"));
function registerAuthEndpoints(app) {
        ...
        try {
            ...
            try {
                ...
                await (0, otp_1.getOtpRepository)().create({
                    ...
                    max_attempts: 3,
                    metadata: {
                        cognito_session: cognitoResponse.session,
                        challenge_name: cognitoResponse.challengeName,
                    },
                });
                ...
                // Fetch AWS Settings from platform_settings table
    ...
    app.get("/make-server-3dd53475/auth/diagnostic/all-vendors", async (c) => {
        try {
            ...
            const customersRepo = await Promise.resolve().then(() => __importStar(require('../../lib/repositories/customers'))).then(m => m.getCustomersRepository());
            // Get all vendors
    ...
    app.post("/make-server-3dd53475/auth/verify-session", async (c) => {
        try {
            ...
            return (0, response_utils_1.sendError)(c, error, 500, { valid: false });
        }
    });
    /**
     * POST /auth/logout
    ...
    app.get("/make-server-3dd53475/auth/debug/vendor/:phone", async (c) => {
        try {
            ...
            const { getDbClient, selectQuery } = await Promise.resolve().then(() => __importStar(require('../../lib/db')));
            ...
            // ✅ SQL: 3. Check users table (if exists)
            try {
            ...
            // ✅ SQL: 5. Check vendor applications (if table exists)
            try {
                const { data: applications } = await dbClient
            ...
            return (0, response_utils_1.sendError)(c, error, 500, { stack: error.stack });
        }
    });
    /**
