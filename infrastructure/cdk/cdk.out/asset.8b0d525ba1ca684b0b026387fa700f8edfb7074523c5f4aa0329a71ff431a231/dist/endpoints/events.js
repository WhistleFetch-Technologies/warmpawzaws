...
exports.registerEventEndpoints = registerEventEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerEventEndpoints(app) {
    /**
    ...
    app.get("/events/vendor/:vendorId", async (c) => {
      ...
      `;
            ...
            const events = await (0, rds_connection_1.query)(eventsQuery, params).catch(() => ({ rows: [] }));
            // Calculate stats
            const allEvents = events.rows;
            const stats = {
                total: allEvents.length,
                upcoming: allEvents.filter((e) => new Date(e.event_date) >= new Date() && e.status !== 'completed').length,
                ongoing: allEvents.filter((e) => e.status === 'ongoing').length,
                completed: allEvents.filter((e) => e.status === 'completed').length,
                totalAttendees: allEvents.reduce((sum, e) => sum + (e.current_attendees || 0), 0),
            };
            return c.json({
                success: true,
                events: allEvents,
    ...
    app.post("/events", async (c) => {
        try {
            ...
            const { vendorId, name, description, category, eventDate, startTime, endTime, venue, registrationRequired, maxAttendees, fees, imageUrl, tags, } = eventData;
            if (!vendorId || !name || !eventDate || !startTime) {
                return c.json({ error: 'vendorId, name, eventDate, and startTime are required' }, 400);
            }
            const event = await (0, rds_connection_1.insert)('events', {
                vendor_id: vendorId,
                name,
    ...
    app.post("/events/:eventId/register", async (c) => {
        try {
            ...
            const { customerId, attendeeName, attendeeEmail, attendeePhone, numberOfPeople, pets, specialRequirements, } = await c.req.json();
            if (!customerId || !attendeeName || !attendeePhone) {
                return c.json({ error: 'customerId, attendeeName, and attendeePhone are required' }, 400);
            }
            // Get event
            const events = await (0, rds_connection_1.select)('events', { id: eventId, status: 'published' });
            if (events.length === 0) {
            ...
            // Create registration
            const registration = await (0, rds_connection_1.insert)('event_registrations', {
                event_id: eventId,
            ...
            await (0, rds_connection_1.update)('events', { id: eventId }, { current_attendees: (event.current_attendees || 0) + 1 });
            return c.json({
                success: true,
    ...
    app.get("/events/:eventId/registrations", async (c) => {
        try {
            const { eventId } = c.req.param();
            const registrations = await (0, rds_connection_1.query)(`SELECT r.*, c.name as customer_name, c.phone as customer_phone
         FROM event_registrations r
         LEFT JOIN customers c ON r.customer_id = c.id
         WHERE r.event_id = $1
         ORDER BY r.created_at DESC`, [eventId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
