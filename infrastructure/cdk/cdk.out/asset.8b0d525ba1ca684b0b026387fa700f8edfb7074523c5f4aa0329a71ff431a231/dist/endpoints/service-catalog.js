...
function registerServiceCatalogEndpoints(app) {
    ...
    app.post("/admin/service-catalog", async (c) => {
        try {
            ...
            const { service_id, service_name, display_name, description, category_id, category_name, sub_category_id, sub_category_name, applicable_roles, service_style, base_price, duration_minutes, metadata, display_order, } = body;
            // Validation
            ...
            const existing = await (0, rds_connection_1.query)('SELECT * FROM service_catalog WHERE service_id = $1', [service_id]);
            if (existing.rows.length > 0) {
                return c.json({ error: 'Service with this ID already exists' }, 409);
            }
            const newService = await (0, rds_connection_1.insert)('service_catalog', {
                service_id,
                service_name,
    ...
    app.put("/admin/service-catalog/:serviceId", async (c) => {
        try {
            ...
            const existing = await (0, rds_connection_1.query)('SELECT * FROM service_catalog WHERE service_id = $1 OR id = $1', [serviceId]);
            if (existing.rows.length === 0) {
                return c.json({ error: 'Service not found' }, 404);
            }
            ...
            if (body.sub_category_id !== undefined)
                updateData.sub_category_id = body.sub_category_id;
            if (body.sub_category_name !== undefined)
                updateData.sub_category_name = body.sub_category_name;
            if (body.applicable_roles !== undefined)
                updateData.applicable_roles = body.applicable_roles;
            if (body.service_style !== undefined)
                updateData.service_style = body.service_style;
            ...
            await (0, rds_connection_1.update)('service_catalog', { id: service.id }, updateData);
            // Fetch updated service
            const updated = await (0, rds_connection_1.query)('SELECT * FROM service_catalog WHERE id = $1', [service.id]);
            return c.json({
                success: true,
    ...
    app.delete("/admin/service-catalog/:serviceId", async (c) => {
        try {
            ...
            await (0, rds_connection_1.update)('service_catalog', { id: service.id }, {
                status: 'archived',
