...
function registerNotificationEndpoints(app) {
    /**
     * GET /notifications
    ...
    app.get("/notifications", async (c) => {
      ...
      `;
            ...
            const notifications = await (0, rds_connection_1.query)(notificationQuery, params);
            // Count unread
            const unreadCount = await (0, rds_connection_1.query)('SELECT COUNT(*) as count FROM notifications WHERE recipient_id = $1 AND recipient_type = $2 AND is_read = false', [userId, userType]);
            return c.json({
                success: true,
    ...
    app.post("/notifications", async (c) => {
        try {
            ...
            const { userId, userType, notificationType, title, message, data, sendSms, sendPush, } = notificationData;
            ...
            if (sendSms) {
                try {
                    ...
                    if (userType === 'customer') {
                        const customers = await (0, rds_connection_1.select)('customers', { id: userId });
                        phone = customers[0]?.phone || null;
                    }
                    else if (userType === 'vendor') {
                        const vendors = await (0, rds_connection_1.select)('vendors', { id: userId });
    ...
    app.put("/notifications/:notificationId/read", async (c) => {
        try {
            ...
            const updated = await (0, rds_connection_1.update)('notifications', { id: notificationId }, { is_read: true, read_at: new Date() });
            if (updated.length === 0) {
    ...
    app.put("/notifications/read-all", async (c) => {
        try {
            ...
            await (0, rds_connection_1.query)('UPDATE notifications SET is_read = true, read_at = NOW() WHERE user_id = $1 AND user_type = $2 AND is_read = false', [userId, userType]);
            return c.json({
                success: true,
