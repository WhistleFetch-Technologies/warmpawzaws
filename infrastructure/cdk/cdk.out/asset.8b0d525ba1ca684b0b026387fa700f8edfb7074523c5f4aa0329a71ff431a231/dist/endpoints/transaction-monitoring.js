...
exports.registerTransactionMonitoringEndpoints = registerTransactionMonitoringEndpoints;
...
function registerTransactionMonitoringEndpoints(app) {
    /**
    ...
    app.get("/admin/transactions/stats", async (c) => {
        try {
            ...
            const payments = await (0, rds_connection_1.query)(`SELECT * FROM payments 
         WHERE created_at >= $1 AND created_at <= $2`, [startDate, endDate]).catch(() => ({ rows: [] }));
            // Get refunds in range
            const refunds = await (0, rds_connection_1.query)(`SELECT * FROM refunds 
         WHERE created_at >= $1 AND created_at <= $2`, [startDate, endDate]).catch(() => ({ rows: [] }));
            const totalTransactions = payments.rows.length;
            const totalVolume = payments.rows.reduce((sum, p) => sum + parseFloat(p.amount || '0'), 0);
            const completedPayments = payments.rows.filter((p) => p.payment_status === 'paid');
            const pendingPayments = payments.rows.filter((p) => p.payment_status === 'pending');
            const failedPayments = payments.rows.filter((p) => p.payment_status === 'failed');
    ...
    app.get("/admin/transactions", async (c) => {
        try {
            ...
            const countResult = await (0, rds_connection_1.query)(countQuery, countParams).catch(() => ({ rows: [{ count: '0' }] }));
            const totalCount = parseInt(countResult.rows[0]?.count || '0', 10);
            // Enrich with customer and vendor info
            const enrichedTransactions = await Promise.all(result.rows.map(async (payment) => {
                let customer = 'Unknown';
                let vendor = 'Unknown';
                if (payment.customer_id) {
                    const customers = await (0, rds_connection_1.select)('customers', { id: payment.customer_id }).catch(() => []);
                    if (customers.length > 0) {
                        customer = customers[0].full_name || customers[0].phone || 'Unknown';
                    }
                }
                if (payment.vendor_id) {
                    const vendors = await (0, rds_connection_1.select)('vendors', { id: payment.vendor_id }).catch(() => []);
                    if (vendors.length > 0) {
    ...
    app.get("/admin/transactions/export", async (c) => {
        try {
            ...
            const payments = await (0, rds_connection_1.query)(`SELECT * FROM payments 
         WHERE created_at >= $1 AND created_at <= $2 
         ORDER BY created_at DESC`, [startDate, endDate]).catch(() => ({ rows: [] }));
            // Generate CSV
