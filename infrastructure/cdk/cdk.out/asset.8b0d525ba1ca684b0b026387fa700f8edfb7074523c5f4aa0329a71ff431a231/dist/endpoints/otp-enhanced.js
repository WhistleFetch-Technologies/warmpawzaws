...
exports.registerEnhancedOtpEndpoints = registerEnhancedOtpEndpoints;
...
function registerEnhancedOtpEndpoints(app) {
    /**
    ...
    app.post("/bookings/:bookingId/generate-otp", async (c) => {
        try {
            ...
            const customers = await (0, rds_connection_1.select)('customers', { id: booking.customer_id });
    ...
    app.post("/bookings/:bookingId/verify-otp", async (c) => {
        try {
            ...
            // Find OTP token
            const otpTokens = await (0, rds_connection_1.query)(`SELECT * FROM otp_tokens
         ...
         LIMIT 1`, [bookingId, action, sessionNumber.toString()]).catch(() => ({ rows: [] }));
            ...
            if (otp !== otpToken.otp_code) {
                await (0, rds_connection_1.update)('otp_tokens', { id: otpToken.id }, { attempts: (otpToken.attempts || 0) + 1 });
                return c.json({
                    success: false,
                    verified: false,
                    message: 'Invalid OTP',
            ...
            // Mark OTP as used
            await (0, rds_connection_1.update)('otp_tokens', { id: otpToken.id }, {
                is_used: true,
            ...
            if (Object.keys(updateData).length > 0) {
                await (0, rds_connection_1.update)('bookings', { id: bookingId }, updateData);
            }
            return c.json({
                success: true,
                verified: true,
    ...
    app.post("/bookings/create-with-otp", async (c) => {
        try {
            ...
            const { customerId, vendorId, serviceType, serviceId, staffId, scheduledDate, scheduledTime, petId, price, notes, } = body;
            ...
            // Create booking
            const booking = await (0, rds_connection_1.insert)('bookings', {
...
//# sourceMappingURL=otp-enhanced.js.map