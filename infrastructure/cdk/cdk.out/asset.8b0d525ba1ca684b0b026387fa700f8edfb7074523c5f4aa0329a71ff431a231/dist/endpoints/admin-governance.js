...
exports.registerAdminGovernanceEndpoints = registerAdminGovernanceEndpoints;
...
// HANDLERS
// ============================================================================
class PropagateChangesHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        ...
        try {
            ...
            await (0, rds_connection_1.insert)('admin_audit_log', {
                action: 'propagate',
                resource_type: type,
                resource_id: data.id || data.vendor_id || data.role_id,
                details: JSON.stringify(data),
                performed_by: context.event.requestContext?.authorizer?.claims?.sub || context.event.requestContext?.identity?.user || 'system',
    ...
    async propagateVendorStatusChange(data) {
        const { vendor_id, new_status } = data;
        // Send notification to vendor
        await (0, aws_clients_1.publishToSNS)('vendor-notifications', {
            type: 'status_change',
            vendor_id,
            new_status,
        ...
        await (0, aws_clients_1.sendToSQS)('notification-queue', {
            type: 'sms',
    ...
    async propagateRoleCapabilitiesChange(data) {
        ...
        const vendors = await (0, rds_connection_1.select)('vendors', { role_id, status: 'active' });
    ...
    async propagateTierChange(data) {
        ...
        const vendors = await (0, rds_connection_1.select)('vendors', { tier: tier_id });
    ...
    async propagatePlatformSettingsChange() {
        ...
        await (0, aws_clients_1.publishToSNS)('platform-notifications', {
            type: 'settings_updated',
            message: 'Platform settings have been updated.',
            timestamp: new Date().toISOString(),
        });
    }
    async propagateServiceCatalogChange(data) {
        const { service_id, action } = data;
        // Invalidate search index
        await (0, aws_clients_1.sendToSQS)('search-index-queue', {
    ...
    async propagatePromotionChange(data) {
        ...
        if (action === 'create') {
            await (0, aws_clients_1.publishToSNS)('customer-notifications', {
                type: 'new_promotion',
                promotion_id,
    ...
    async propagateBannerChange(data) {
    ...
    async propagateTaxRuleChange(data) {
    ...
    async invalidateCache(key) {
        ...
        await (0, rds_connection_1.insert)('cache_invalidations', {
            cache_key: key,
            invalidated_at: new Date(),
        });
    }
}
class InvalidateCacheHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        const body = this.parseBody(context.event);
...
class GovernanceStatusHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        // Get recent propagation events
        const recentEvents = await (0, rds_connection_1.query)(`
    ...
    `);
        ...
        const pendingNotifications = await (0, rds_connection_1.query)(`
      SELECT COUNT(*) as count FROM notification_queue 
      WHERE status = 'pending'
    `);
        // Get cache invalidation stats
        const cacheStats = await (0, rds_connection_1.query)(`
...
// HONO ROUTER SETUP
// ============================================================================
function registerAdminGovernanceEndpoints(app) {
...
//# sourceMappingURL=admin-governance.js.map