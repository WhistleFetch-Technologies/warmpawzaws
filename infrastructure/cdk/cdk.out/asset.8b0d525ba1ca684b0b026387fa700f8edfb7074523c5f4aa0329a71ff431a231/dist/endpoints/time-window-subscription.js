...
exports.registerTimeWindowSubscriptionEndpoints = registerTimeWindowSubscriptionEndpoints;
...
function registerTimeWindowSubscriptionEndpoints(app) {
    /**
    ...
    app.post("/booking/subscription/time-window", async (c) => {
        try {
            const { bookingId, customerId, providerId, serviceType, timeWindow, recurringSchedule, startDate, endDate, totalSessions, } = await c.req.json();
            ...
            const timeWindowPlan = await (0, rds_connection_1.insert)('subscription_plans', {
                vendor_id: vendorId,
    ...
    app.get("/subscriptions/time-window/:subscriptionId", async (c) => {
        try {
            ...
            const bookingsResult = await (0, rds_connection_1.query)(`SELECT * FROM bookings 
         WHERE package_details->>'subscriptionId' = $1 
         LIMIT 1`, [subscriptionId]).catch(() => ({ rows: [] }));
            let timeWindow = null;
            let recurringSchedule = null;
            if (bookingsResult.rows.length > 0 && bookingsResult.rows[0].package_details) {
                const packageDetails = bookingsResult.rows[0].package_details;
    ...
    app.get("/subscriptions/time-window/customer/:customerId", async (c) => {
        try {
            ...
            const subscriptions = await (0, rds_connection_1.query)(`SELECT cs.* FROM customer_subscriptions cs
         INNER JOIN bookings b ON b.package_details->>'subscriptionId' = cs.id::text
         WHERE cs.customer_id = $1
         ORDER BY cs.created_at DESC`, [customerId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
...
//# sourceMappingURL=time-window-subscription.js.map