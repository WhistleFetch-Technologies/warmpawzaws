...
exports.registerCustomerBookingHistoryEndpoints = registerCustomerBookingHistoryEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerCustomerBookingHistoryEndpoints(app) {
    /**
    ...
    app.get("/customer/:customerId/bookings", async (c) => {
      ...
      `;
            ...
            const bookings = await (0, rds_connection_1.query)(bookingQuery, params);
            // Get statistics
            const statsQuery = await (0, rds_connection_1.query)(`SELECT 
           COUNT(*) as total,
    ...
    app.get("/customer/:customerId/bookings/:bookingId", async (c) => {
        try {
            ...
            const bookingQuery = await (0, rds_connection_1.query)(`SELECT b.*,
                v.business_name as vendor_name,
         ...
         WHERE b.id = $1 AND b.customer_id = $2`, [bookingId, customerId]);
            if (bookingQuery.rows.length === 0) {
                return c.json({ error: 'Booking not found' }, 404);
            }
            const booking = bookingQuery.rows[0];
            // Get prescription if exists
            const prescriptions = await (0, rds_connection_1.query)('SELECT * FROM prescriptions WHERE booking_id = $1', [bookingId]);
            // Get review if exists
            const reviews = await (0, rds_connection_1.query)('SELECT * FROM reviews WHERE booking_id = $1 AND customer_id = $2', [bookingId, customerId]);
            return c.json({
                success: true,
                booking: {
                    id: booking.id,
    ...
    app.get("/customer/:customerId/bookings/follow-up-eligible", async (c) => {
        try {
            ...
            const eligibleBookings = await (0, rds_connection_1.query)(`SELECT b.*,
                v.business_name as vendor_name,
                v.phone as vendor_phone,
                s.name as service_name
         FROM bookings b
         ...
         ORDER BY b.completed_at DESC`, [customerId]);
            // Enrich with prescription and review status
            const enrichedBookings = await Promise.all(eligibleBookings.rows.map(async (booking) => {
                const prescription = await (0, rds_connection_1.query)('SELECT id FROM prescriptions WHERE booking_id = $1', [booking.id]);
                const review = await (0, rds_connection_1.query)('SELECT id FROM reviews WHERE booking_id = $1', [booking.id]);
                return {
                    id: booking.id,
...
//# sourceMappingURL=customer-booking-history.js.map