...
exports.registerPackageEndpoints = registerPackageEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerPackageEndpoints(app) {
    /**
    ...
    app.get("/packages/discover", async (c) => {
      ...
      `;
            ...
            const packages = await (0, rds_connection_1.query)(packageQuery, params);
            // Filter by pet type if provided (assuming pet_types is JSONB)
            let filteredPackages = packages.rows;
            if (petType) {
                filteredPackages = packages.rows.filter((pkg) => {
    ...
    app.get("/packages/:packageId", async (c) => {
        try {
            ...
            const packages = await (0, rds_connection_1.query)(`SELECT p.*, v.business_name as vendor_name, v.address, v.city, v.phone, v.rating
         FROM service_packages p
         INNER JOIN vendors v ON p.vendor_id = v.id
    ...
    app.post("/packages/:packageId/enroll", async (c) => {
        try {
            ...
            // Get package
            const packages = await (0, rds_connection_1.select)('service_packages', { id: packageId });
    ...
    app.get("/packages/enrollments/:customerId", async (c) => {
        try {
            ...
            const enrollments = await (0, rds_connection_1.query)(`SELECT b.*, p.name as package_name, p.total_sessions, v.business_name as vendor_name
         FROM bookings b
    ...
    app.get("/packages/:packageId/sessions/:bookingId", async (c) => {
        try {
         ...
         WHERE booking_id = $1
         ORDER BY session_number ASC`, [bookingId]);
            return c.json({
                success: true,
