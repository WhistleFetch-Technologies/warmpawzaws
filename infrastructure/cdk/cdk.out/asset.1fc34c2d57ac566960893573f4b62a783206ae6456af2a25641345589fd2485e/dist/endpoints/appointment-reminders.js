...
exports.registerAppointmentReminderEndpoints = registerAppointmentReminderEndpoints;
const rds_connection_1 = require("../database/rds-connection");
const sns_client_1 = require("../utils/sns-client");
const client_sns_1 = require("@aws-sdk/client-sns");
function registerAppointmentReminderEndpoints(app) {
    /**
    ...
    app.post("/customer/:customerId/reminder-preferences", async (c) => {
        try {
            ...
            await (0, rds_connection_1.update)('customers', { id: customerId }, {
                preferences: {
                    ...existingPreferences,
    ...
    app.get("/customer/:customerId/reminder-preferences", async (c) => {
        try {
            ...
            const preferences = customers[0].preferences?.reminderPreferences || {
                enabled: true,
                channels: ['sms', 'push'],
                timings: [24, 1],
        ...
        try {
            ...
            for (const timingHours of preferences.timings) {
                ...
                for (const channel of preferences.channels) {
                    const reminder = await (0, rds_connection_1.insert)('appointment_reminders', {
    ...
    app.post("/bookings/:bookingId/send-reminder", async (c) => {
        try {
            ...
            // Get service
            const services = await (0, rds_connection_1.select)('services', { id: booking.service_id });
            ...
            if (channel === 'sms' && customer.phone) {
                ...
                // Log reminder
                await (0, rds_connection_1.insert)('appointment_reminders', {
                    booking_id: bookingId,
                    customer_id: booking.customer_id,
                    channel: 'sms',
    ...
    app.get("/customer/:customerId/reminders", async (c) => {
        try {
            const { customerId } = c.req.param();
            const reminders = await (0, rds_connection_1.query)(`SELECT * FROM appointment_reminders 
         WHERE customer_id = $1 
         ORDER BY scheduled_for DESC 
         LIMIT 50`, [customerId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
