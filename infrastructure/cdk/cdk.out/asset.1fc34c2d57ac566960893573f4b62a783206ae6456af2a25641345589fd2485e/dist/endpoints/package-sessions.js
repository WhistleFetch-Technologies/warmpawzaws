...
exports.registerPackageSessionEndpoints = registerPackageSessionEndpoints;
...
// ============================================================================
class StartSessionHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        ...
        const existingSessions = await (0, rds_connection_1.select)('package_sessions', {
        ...
        if (existingSessions.length > 0) {
            await (0, rds_connection_1.update)('package_sessions', { id: existingSessions[0].id }, {
                status: 'in_progress',
                started_at: new Date(),
            });
        }
        else {
            await (0, rds_connection_1.insert)('package_sessions', {
                booking_id: bookingId,
                session_number: sessionNumber,
                status: 'in_progress',
                started_at: new Date(),
...
class CompleteSessionHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        const body = this.parseBody(context.event);
        const { bookingId, sessionNumber, notes, outcome } = body;
        this.validateRequired(body, ['bookingId', 'sessionNumber']);
        // ✅ SQL: Get session
        const sessions = await (0, rds_connection_1.select)('package_sessions', {
            booking_id: bookingId,
            session_number: sessionNumber,
        });
        ...
        await (0, rds_connection_1.update)('package_sessions', { id: sessions[0].id }, {
            status: 'completed',
        ...
        const allSessions = await (0, rds_connection_1.select)('package_sessions', {
            booking_id: bookingId,
        });
        ...
        if (completedSessions.length === totalSessions && totalSessions > 0) {
            await (0, rds_connection_1.update)('bookings', { id: bookingId }, { status: 'completed' });
        }
        return this.success({
...
class GetSessionProgressHandler extends base_handler_1.BaseHandler {
    async handle(context) {
...
// HONO ROUTER SETUP
// ============================================================================
function registerPackageSessionEndpoints(app) {
...
//# sourceMappingURL=package-sessions.js.map