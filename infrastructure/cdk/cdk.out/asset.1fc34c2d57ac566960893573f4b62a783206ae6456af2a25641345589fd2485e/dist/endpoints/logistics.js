...
async function getShiprocketToken() {
    ...
    try {
        const settings = await (0, rds_connection_1.select)('platform_settings', { setting_key: 'platform:integrations:shiprocket' });
        const config = settings.length > 0 ? settings[0].setting_value : null;
...
function registerLogisticsEndpoints(app) {
    ...
    app.post("/logistics/shiprocket/create-order", async (c) => {
        try {
            ...
            await (0, rds_connection_1.insert)('shipments', {
                order_id: orderData.orderId,
    ...
    app.post("/logistics/shiprocket/generate-awb", async (c) => {
        try {
            ...
            await (0, rds_connection_1.update)('shipments', { shipment_id: shipmentId.toString() }, {
