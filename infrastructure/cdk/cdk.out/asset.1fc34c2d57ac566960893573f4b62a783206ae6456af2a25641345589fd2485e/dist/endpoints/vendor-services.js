...
exports.registerVendorServicesEndpoints = registerVendorServicesEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerVendorServicesEndpoints(app) {
    /**
    ...
    app.get("/vendor/:vendorId/services", async (c) => {
        try {
            ...
            for (const style of serviceStyles) {
                const services = await (0, rds_connection_1.query)(`SELECT vs.*, s.name as base_service_name, s.description as base_description
           ...
           ORDER BY vs.created_at DESC`, [vendorId, style]);
    ...
    app.get("/vendor/:vendorId/services/:serviceStyle", async (c) => {
        try {
            ...
            const services = await (0, rds_connection_1.query)(`SELECT vs.*, s.name as base_service_name
         FROM vendor_services vs
         LEFT JOIN services s ON vs.service_id = s.id
         WHERE vs.vendor_id = $1
         AND vs.service_style = $2
         AND vs.is_enabled = true
         ORDER BY vs.created_at DESC`, [vendorId, serviceStyle]);
            return c.json({
                success: true,
    ...
    app.post("/vendor/:vendorId/services", async (c) => {
        try {
            ...
            const { serviceId, serviceStyle, customPrice, customDuration, isEnabled, publishStatus, isCustomService, } = serviceData;
            if (!serviceId || !serviceStyle) {
                return c.json({ error: 'serviceId and serviceStyle are required' }, 400);
            }
            // Check if service already exists
            const existing = await (0, rds_connection_1.query)(`SELECT id FROM vendor_services
         WHERE vendor_id = $1 AND service_id = $2 AND service_style = $3`, [vendorId, serviceId, serviceStyle]);
            if (existing.rows.length > 0) {
                return c.json({ error: 'Service already exists for this style' }, 409);
            }
            // Get base service info
            const baseServices = await (0, rds_connection_1.select)('services', { id: serviceId });
            if (baseServices.length === 0) {
                return c.json({ error: 'Base service not found' }, 404);
            }
            const baseService = baseServices[0];
            const vendorService = await (0, rds_connection_1.insert)('vendor_services', {
                vendor_id: vendorId,
    ...
    app.put("/vendor/:vendorId/services/:serviceId", async (c) => {
        try {
            ...
            const serviceData = await c.req.json();
            const updated = await (0, rds_connection_1.update)('vendor_services', { id: serviceId, vendor_id: vendorId }, {
    ...
    app.delete("/vendor/:vendorId/services/:serviceId", async (c) => {
        try {
            const { vendorId, serviceId } = c.req.param();
            await (0, rds_connection_1.query)('DELETE FROM vendor_services WHERE id = $1 AND vendor_id = $2', [serviceId, vendorId]);
            return c.json({
                success: true,
    ...
    app.post("/vendor/:vendorId/services/custom", async (c) => {
        try {
            const { vendorId } = c.req.param();
            const serviceData = await c.req.json();
            const { serviceName, description, category, subCategory, serviceStyle, price, duration, } = serviceData;
            ...
            const baseService = await (0, rds_connection_1.insert)('services', {
                name: serviceName,
