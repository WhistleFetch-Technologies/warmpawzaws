...
function registerHealthEndpoints(app) {
    ...
    app.get("/health", async (c) => {
        try {
            const dbHealthy = await (0, rds_connection_1.checkDbHealth)();
            return c.json({
    ...
    app.get("/health/database", async (c) => {
        try {
            ...
            const stats = await (0, rds_connection_1.query)('SELECT COUNT(*) as count FROM vendors').catch(() => ({ rows: [{ count: '0' }] }));
            return c.json({
                status: dbHealthy ? 'healthy' : 'unhealthy',
                responseTime,
                timestamp: new Date().toISOString(),
