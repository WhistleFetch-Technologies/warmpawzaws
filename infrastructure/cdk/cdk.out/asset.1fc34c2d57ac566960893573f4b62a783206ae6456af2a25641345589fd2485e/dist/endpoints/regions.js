...
exports.registerRegionEndpoints = registerRegionEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerRegionEndpoints(app) {
    /**
     * GET /regions
     * Get all regions
     */
    app.get("/regions", async (c) => {
        try {
            const regions = await (0, rds_connection_1.select)('regions', { is_active: true }, { orderBy: 'name', orderDirection: 'ASC' });
            return c.json({
                success: true,
    ...
    app.get("/regions/:regionId", async (c) => {
        try {
            ...
            const regions = await (0, rds_connection_1.select)('regions', { id: regionId });
            if (regions.length === 0) {
    ...
    app.post("/admin/regions", async (c) => {
        try {
            ...
            const region = await (0, rds_connection_1.insert)('regions', {
                name,
                code,
    ...
    app.put("/admin/regions/:regionId", async (c) => {
        try {
            const { regionId } = c.req.param();
            const regionData = await c.req.json();
            const updated = await (0, rds_connection_1.update)('regions', { id: regionId }, {
