...
exports.registerReviewEndpoints = registerReviewEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerReviewEndpoints(app) {
    /**
     * GET /reviews
    ...
    app.get("/reviews", async (c) => {
      ...
      `;
            ...
            const reviews = await (0, rds_connection_1.query)(reviewQuery, params);
            // Calculate average rating
            let avgRating = 0;
            if (reviews.rows.length > 0) {
                avgRating = reviews.rows.reduce((sum, r) => sum + (r.rating || 0), 0) / reviews.rows.length;
            }
            return c.json({
                success: true,
    ...
    app.post("/reviews", async (c) => {
        try {
            ...
            if (bookingId) {
                const existing = await (0, rds_connection_1.query)('SELECT id FROM reviews WHERE booking_id = $1', [bookingId]);
                if (existing.rows.length > 0) {
                    return c.json({ error: 'Review already exists for this booking' }, 409);
                }
            }
            const review = await (0, rds_connection_1.insert)('reviews', {
    ...
    app.put("/reviews/:reviewId", async (c) => {
        try {
            ...
            const updated = await (0, rds_connection_1.update)('reviews', { id: reviewId }, {
                rating: rating,
                comment: comment,
                images: images,
    ...
    app.post("/admin/reviews/:reviewId/approve", async (c) => {
        try {
            const { reviewId } = c.req.param();
            const updated = await (0, rds_connection_1.update)('reviews', { id: reviewId }, { is_approved: true, approved_at: new Date() });
            if (updated.length === 0) {
                return c.json({ error: 'Review not found' }, 404);
            }
            return c.json({
    ...
    app.post("/admin/reviews/:reviewId/reject", async (c) => {
        try {
            ...
            const updated = await (0, rds_connection_1.update)('reviews', { id: reviewId }, { is_approved: false, rejection_reason: reason || null });
            if (updated.length === 0) {
                return c.json({ error: 'Review not found' }, 404);
            }
            return c.json({
