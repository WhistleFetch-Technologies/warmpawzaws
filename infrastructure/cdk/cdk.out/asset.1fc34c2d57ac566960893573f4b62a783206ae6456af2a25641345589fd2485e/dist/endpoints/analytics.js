...
function registerAnalyticsEndpoints(app) {
    /**
    ...
    app.get("/analytics/vendor/:vendorId/dashboard", async (c) => {
        try {
            ...
            const bookingStats = await (0, rds_connection_1.query)(`SELECT 
         ...
         FROM bookings
         WHERE vendor_id = $1 ${dateFilter}`, params);
            const stats = bookingStats.rows[0];
            // Get reviews
            const reviewStats = await (0, rds_connection_1.query)(`SELECT 
           COUNT(*) as total_reviews,
           COALESCE(AVG(rating), 0) as avg_rating
         FROM reviews
         WHERE vendor_id = $1 AND is_approved = true`, [vendorId]);
    ...
    app.get("/analytics/customer/:customerId/dashboard", async (c) => {
        try {
         ...
         WHERE customer_id = $1`, [customerId]);
            const stats = bookingStats.rows[0];
            // Get reviews count
            const reviews = await (0, rds_connection_1.query)('SELECT COUNT(*) as total_reviews FROM reviews WHERE customer_id = $1', [customerId]);
            // Get orders count
            const orders = await (0, rds_connection_1.query)('SELECT COUNT(*) as total_orders FROM orders WHERE customer_id = $1', [customerId]);
            // Get wallet balance
            const wallets = await (0, rds_connection_1.select)('customer_wallets', { customer_id: customerId });
    ...
    app.get("/analytics/platform/overview", async (c) => {
        try {
            ...
            const vendorStats = await (0, rds_connection_1.query)(`SELECT 
         ...
         FROM vendors`);
            ...
            const customerStats = await (0, rds_connection_1.query)(`SELECT COUNT(*) as total_customers FROM customers`);
            // Get booking statistics
            const bookingStats = await (0, rds_connection_1.query)(`SELECT 
           COUNT(*) as total_bookings,
           COUNT(*) FILTER (WHERE status = 'completed') as completed_bookings,
         ...
         FROM bookings`);
            ...
            const orderStats = await (0, rds_connection_1.query)(`SELECT 
    ...
    app.get("/analytics/vendor/:vendorId/revenue", async (c) => {
      ...
      `;
            ...
            const revenue = await (0, rds_connection_1.query)(revenueQuery, params);
            return c.json({
                success: true,
