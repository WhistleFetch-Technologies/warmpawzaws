...
async function checkIdempotencyKey(key) {
    ...
    try {
        const result = await (0, rds_connection_1.query)(`SELECT entity_id, response, http_status 
 ...
 */
async function storeIdempotencyKey(key, entityType, entityId, response, httpStatus = 200, expiryHours = 24) {
    if (!key)
        return;
    // Check if table exists
    const hasTable = await checkTableExists();
    ...
    try {
        ...
        await (0, rds_connection_1.query)(`INSERT INTO idempotency_keys (key, entity_type, entity_id, response, http_status, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (key) DO NOTHING`, [key, entityType, entityId, JSON.stringify(response), httpStatus, expiresAt]);
    }
    catch (error) {
 ...
 */
async function withIdempotency(idempotencyKey, entityType, handler) {
    ...
    if (result.data && result.data.id) {
        await storeIdempotencyKey(idempotencyKey, entityType, result.data.id, result.data, result.status);
    }
    return { ...result, cached: false };
}
/**
