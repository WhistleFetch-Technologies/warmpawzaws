...
exports.rewardsLoyaltySystemSQL = rewardsLoyaltySystemSQL;
const hono_1 = require("hono");
const db_1 = require("../lib/db");
const wallets_1 = require("../lib/repositories/wallets");
// Helper repository functions (inline SQL replacement - TODO: Create proper repositories)
const getLoyaltyRepository = () => ({
    getActiveRules: async () => {
        return (0, db_1.selectQuery)('loyalty_rules', { is_active: true });
    },
    findOrCreate: async (userId) => {
        const existing = await (0, db_1.selectQuery)('loyalty_profiles', { user_id: userId }, { limit: 1 });
        if (existing[0])
            return existing[0];
        const now = new Date().toISOString();
        const [newProfile] = await (0, db_1.insertQuery)('loyalty_profiles', {
            user_id: userId,
            total_points: 0,
    ...
    getTransactions: async (userId, options) => {
        return (0, db_1.selectQuery)('loyalty_transactions', { user_id: userId }, { limit: options?.limit || 50, orderBy: 'created_at', orderDirection: 'desc' });
    },
    addPoints: async (userId, points, reason, referenceId, description) => {
        const profile = await getLoyaltyRepository().findOrCreate(userId);
        const pool = await (0, db_1.getDbClient)();
        await pool.query('UPDATE loyalty_profiles SET total_points = total_points + $1, lifetime_points_earned = lifetime_points_earned + $1, updated_at = $2 WHERE user_id = $3', [points, new Date().toISOString(), userId]);
        await (0, db_1.insertQuery)('loyalty_transactions', {
            user_id: userId,
            points: points,
        ...
        const [updated] = await (0, db_1.selectQuery)('loyalty_profiles', { user_id: userId }, { limit: 1 });
        return updated || profile;
    },
    redeemPoints: async (userId, points, reason) => {
        const profile = await getLoyaltyRepository().findOrCreate(userId);
        if ((profile?.total_points || 0) < points)
            throw new Error('Insufficient points');
        const pool = await (0, db_1.getDbClient)();
        await pool.query('UPDATE loyalty_profiles SET total_points = total_points - $1, lifetime_points_redeemed = lifetime_points_redeemed + $1, updated_at = $2 WHERE user_id = $3', [points, new Date().toISOString(), userId]);
        await (0, db_1.insertQuery)('loyalty_transactions', {
            user_id: userId,
            points: -points,
...
const getReferralsRepository = () => ({
    findByReferrer: async (referrerId) => {
        const [result] = await (0, db_1.selectQuery)('referrals', { referrer_id: referrerId }, { limit: 1 });
        return result || null;
    },
    findByCode: async (code) => {
        const [result] = await (0, db_1.selectQuery)('referrals', { referral_code: code }, { limit: 1 });
        return result || null;
    },
    findByReferred: async (referredId) => {
        const [result] = await (0, db_1.selectQuery)('referrals', { referred_id: referredId }, { limit: 1 });
        return result || null;
    },
    applyReferral: async (data) => {
        const pool = await (0, db_1.getDbClient)();
        // Find referral by code
        const [referral] = await (0, db_1.selectQuery)('referrals', { referral_code: data.referral_code }, { limit: 1 });
        if (!referral)
            throw new Error('Referral code not found');
        // Update referral with referred_id
        await (0, db_1.updateQuery)('referrals', { referral_code: data.referral_code }, {
    ...
    generateUniqueCode: async (prefix) => {
        const pool = await (0, db_1.getDbClient)();
        let code = '';
        let exists = true;
        while (exists) {
            code = `${prefix}${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
            const [existing] = await (0, db_1.selectQuery)('referrals', { referral_code: code }, { limit: 1 });
            exists = !!existing;
        }
        return code;
    },
    create: async (data) => {
        const [result] = await (0, db_1.insertQuery)('referrals', {
...
// HELPER FUNCTIONS
// ============================================================================
function calculateLoyaltyTier(pointsBalance, userType) {
    const customerTiers = [
    ...
    try {
        ...
        if (rules.length === 0) {
            for (const rule of DEFAULT_RULES) {
                await (0, db_1.upsertQuery)('loyalty_rules', {
            ...
            return (0, response_utils_1.sendSuccess)(c, { rules: seededRules });
        }
        return (0, response_utils_1.sendSuccess)(c, { rules });
    }
    catch (e) {
...
app.post('/make-server-3dd53475/admin/loyalty/rules', async (c) => {
    try {
        ...
        const loyaltyRepo = getLoyaltyRepository();
        const rules = await loyaltyRepo.getActiveRules();
        return (0, response_utils_1.sendSuccess)(c, { rules }, 'Rules updated');
    }
    catch (e) {
...
app.get('/make-server-3dd53475/loyalty/profile/:userId', async (c) => {
    try {
        ...
        const userType = c.req.query('type') || 'customer';
        const loyaltyRepo = getLoyaltyRepository();
        ...
        // Calculate tier
        const tier = calculateLoyaltyTier(profile?.total_points || 0, userType);
        return (0, response_utils_1.sendSuccess)(c, {
            profile: {
                userId,
                userType,
                pointsBalance: profile?.total_points || 0,
                totalPointsEarned: profile?.lifetime_points_earned || 0,
                totalPointsRedeemed: profile?.lifetime_points_redeemed || 0,
                referralCode,
                history: transactions.map((t) => ({
                    id: t.id,
                    userId: t.customer_id || t.user_id,
...
app.post('/make-server-3dd53475/loyalty/process-action', async (c) => {
    try {
        ...
        if (!rule) {
            ...
            return (0, response_utils_1.sendSuccess)(c, { pointsAwarded: 0, message: 'No rule matched' });
        }
        // Get profile
        const profile = await loyaltyRepo.findOrCreate(userId);
        ...
        if (rule.frequency === 'one-time') {
            ...
            if (hasDone) {
                ...
                return (0, response_utils_1.sendSuccess)(c, { pointsAwarded: 0, message: 'Already claimed' });
            }
        }
        // Calculate points
        let pointsToAward = 0;
        ...
        if (pointsToAward <= 0) {
            return (0, response_utils_1.sendSuccess)(c, { pointsAwarded: 0, message: 'No points earned (amount too low)' });
        }
        // Award points
        const updatedProfile = await loyaltyRepo.addPoints(userId, pointsToAward, actionKey, metadata?.referenceId, rule.name);
        console.log(`✅ [LOYALTY] Awarded ${pointsToAward} points to ${userId}`);
        return (0, response_utils_1.sendSuccess)(c, {
            pointsAwarded: pointsToAward,
            newBalance: updatedProfile?.total_points || 0
        });
    }
    catch (e) {
...
app.post('/make-server-3dd53475/loyalty/redeem', async (c) => {
    try {
        ...
        if (!pointsToRedeem || pointsToRedeem <= 0) {
            return (0, response_utils_1.sendError)(c, 'Invalid points amount', 400);
        }
        const loyaltyRepo = getLoyaltyRepository();
        const walletsRepo = (0, wallets_1.getWalletsRepository)();
        // Get profile
        const profile = await loyaltyRepo.findOrCreate(userId);
        if ((profile?.total_points || 0) < pointsToRedeem) {
            return (0, response_utils_1.sendError)(c, 'Insufficient points balance', 400);
        }
        // Redeem points
        ...
        return (0, response_utils_1.sendSuccess)(c, {
            ...
            newPointsBalance: updatedProfile?.total_points || 0,
...
app.post('/make-server-3dd53475/loyalty/referral/apply', async (c) => {
    try {
        ...
        if (!referral) {
            return (0, response_utils_1.sendError)(c, 'Invalid referral code', 400);
        }
        if (referral.referrer_id === newUserId) {
            return (0, response_utils_1.sendError)(c, 'Cannot refer yourself', 400);
        }
        // Check if already referred
        const existingReferral = await referralsRepo.findByReferred(newUserId);
        if (existingReferral) {
            return (0, response_utils_1.sendError)(c, 'User already referred', 400);
        }
        // Apply referral
        ...
        return (0, response_utils_1.sendSuccess)(c, { message: 'Referral code applied' });
    }
    catch (e) {
...
function rewardsLoyaltySystemSQL(mainApp) {
    // Mount all routes from the internal app to the main app
    mainApp.route('/', app);
}
exports.default = rewardsLoyaltySystemSQL;
//# sourceMappingURL=rewards-loyalty-system-sql.js.map