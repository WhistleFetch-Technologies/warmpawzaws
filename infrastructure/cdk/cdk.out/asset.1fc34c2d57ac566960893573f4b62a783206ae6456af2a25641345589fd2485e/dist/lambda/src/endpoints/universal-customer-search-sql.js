...
exports.registerUniversalCustomerSearch = registerUniversalCustomerSearch;
...
// TODO: Create these utility functions or import from appropriate location
const getPrimarySpecialization = (specializations) => {
    return specializations && specializations.length > 0 ? specializations[0] : '';
};
 ...
 */
function generateSlotsFromTimeWindow(startTime, endTime, durationMinutes = 30) {
    const slots = [];
 ...
 */
async function getDynamicRoleMapping() {
    try {
        // ✅ SQL: Get all active roles from roles table (master data)
        // TODO: Create roles repository or use direct SQL query
        const allRoles = await (0, db_1.selectQuery)('roles', { is_active: true });
 ...
 */
function getStaticFallbackMapping(serviceCategory) {
    const categoryMap = {
        'veterinary_services': ['veterinarian', 'pet_clinic', 'vet_clinic'],
        'grooming_services': ['pet_groomer'],
        'training_services': ['pet_trainer'],
 ...
 */
function registerUniversalCustomerSearch(app) {
    /**
    ...
    app.get('/make-server-3dd53475/universal/search', async (c) => {
        try {
            ...
            let vendors = allVendors.filter((v) => v.status === 'approved' &&
                v.is_active === true &&
            ...
            if (roleId) {
                vendors = vendors.filter((v) => v.role_id === roleId);
     ...
     */
    async function returnCenters(c, vendors, serviceCategory, serviceStyle, searchQuery, feeMin, feeMax, availableToday, sortBy, limit, offset, customerLat, customerLon) {
        const centers = [];
        const staffRepo = (0, staff_1.getStaffRepository)();
        const reviewsRepo = (0, reviews_1.getReviewsRepository)();
        // TODO: Create scheduling repository or use direct SQL queries
        for (const vendor of vendors) {
            // ✅ SQL: Get at_center services for this vendor
            const vendorServicesData = await (0, db_1.selectQuery)('vendor_services', {
                vendor_id: vendor.id,
            ...
            const activeStaff = await staffRepo.findByVendor(vendor.id);
            // Calculate average fee from all active staff
            const avgFee = activeStaff.length > 0
                ? activeStaff.reduce((sum, s) => sum + (s.consultation_fee || s.consultationFee || 0), 0) / activeStaff.length
            ...
            for (const staff of activeStaff) {
                ...
                const reviewsData = await (0, db_1.selectQuery)('reviews', { staff_id: staff.id }, { orderBy: 'created_at', orderDirection: 'desc' });
                const reviewsArray = reviewsData || [];
                allReviews.push(...reviewsArray);
                totalReviews += reviewsArray.length;
                if (reviewsArray.length > 0) {
                    const staffRating = reviewsArray.reduce((sum, r) => sum + (r.rating || 0), 0) / reviewsArray.length;
                ...
                try {
                    ...
                    for (let i = 0; i < 7; i++) {
                        ...
                        for (const staff of activeStaff) {
                            ...
                            const availabilityData = await (0, db_1.selectQuery)('staff_availability', {
                                staff_id: staff.id,
                                date: date,
                                is_available: true
                            }, { limit: 1 });
                            if (availabilityData && availabilityData.length > 0) {
                                ...
                                const scheduleData = await (0, db_1.selectQuery)('vendor_schedule_slots', {
                                ...
                                if (scheduleData && scheduleData.length > 0) {
                                    ...
                                    if (schedule.time_windows && Array.isArray(schedule.time_windows)) {
                                        const enabledWindow = schedule.time_windows.find((w) => w.is_enabled === true);
                                        if (enabledWindow) {
                                            nextAvailable = {
                                                date,
            ...
            if (searchQuery && searchQuery.length > 0) {
                ...
                const servicesMatch = publishedServices.some((s) => s.service_name?.toLowerCase().includes(searchLower));
            ...
            const centerObj = {
                ...
                staff: activeStaff.map(s => ({
                    ...
                    photo: s.photo || s.profile_photo_url || null,
                    specialization: getPrimarySpecialization(s.specialization ? [s.specialization] : []),
                    specializations: getAllSpecializations(s.specialization ? [s.specialization] : []),
                    degree: s.degree || s.qualifications || '',
                    experience: s.experience_years || s.experience || s.yearsOfExperience || 0,
                    yearsOfExperience: s.experience_years || s.experience || s.yearsOfExperience || 0,
                    consultationFee: s.consultation_fee || s.consultationFee || 0,
                    rating: s.rating || 0,
                    reviewCount: s.review_count || s.reviewCount || 0,
                    bio: s.bio || s.about || '',
                    gender: s.gender || null,
                    languages: s.languages || s.spoken_languages || ['English', 'Hindi']
                })),
                // Pricing
                averageFee: Math.round(avgFee),
                feeRange: activeStaff.length > 1 ? {
                    min: Math.min(...activeStaff.map(s => s.consultation_fee || s.consultationFee || 0)),
                    max: Math.max(...activeStaff.map(s => s.consultation_fee || s.consultationFee || 0))
                } : null,
                // Ratings
                rating: isNaN(centerRating) ? 0 : parseFloat(centerRating.toFixed(1)),
                reviewCount: totalReviews,
                topReviews: allReviews.slice(0, 3).map((r) => ({
                    id: r.id,
                    rating: r.rating,
                    comment: r.comment,
     ...
     */
    async function returnStaff(c, vendors, serviceCategory, serviceStyle, searchQuery, feeMin, feeMax, availableToday, sortBy, limit, offset, customerLat, customerLon) {
        let allStaff = [];
        ...
        for (const vendor of vendors) {
            ...
            for (const staff of vendorStaff) {
                if (staff.is_active) {
                    ...
                    const vendorData = await vendorsRepo.findById(staff.vendor_id);
                    staff.vendorInfo = vendorData ? {
                        id: vendorData.id,
                        businessName: vendorData.business_name || vendorData.owner_name,
        ...
        if (serviceStyle) {
            ...
            const staffWithMatchingServices = [];
            for (const staff of allStaff) {
                // ✅ SQL: Get staff services with matching style
                const staffServicesData = await (0, db_1.selectQuery)('staff_services', { staff_id: staff.id, service_style: serviceStyle, is_active: true });
            ...
            for (const staff of allStaff) {
                ...
                const staffServicesData = await (0, db_1.selectQuery)('staff_services', { staff_id: staff.id, is_active: true });
        ...
        allStaff = allStaff.filter((s) => s.matchingServicesCount > 0);
        ...
        allStaff = allStaff.filter((staff) => {
            const fee = staff.consultation_fee || staff.consultationFee || 0;
        ...
        if (availableToday) {
            ...
            for (const staff of allStaff) {
                ...
                const availabilityData = await (0, db_1.selectQuery)('staff_availability', { staff_id: staff.id, date: today, is_available: true }, { limit: 1 });
                if (availabilityData && availabilityData.length > 0) {
                    ...
                    const dayOfWeek = dateObj.getDay();
                    // Use executeRaw for complex OR query
                    const scheduleData = await (0, db_1.executeRaw)(`SELECT * FROM vendor_schedule_slots 
           WHERE vendor_id = $1 
           AND (staff_id IS NULL OR staff_id = $2)
           AND day_of_week = $3 
           AND is_available = true 
           LIMIT 1`, [staff.vendor_id, staff.id, dayOfWeek]);
                    if (scheduleData && scheduleData.length > 0) {
                        const schedule = scheduleData[0];
                        // Check if any time window is enabled
                        if (schedule.time_windows && Array.isArray(schedule.time_windows)) {
                            const hasEnabledWindow = schedule.time_windows.some((w) => w.is_enabled === true);
                            if (hasEnabledWindow) {
                                availableStaff.push(staff);
                            }
                        }
        ...
        const enrichedStaff = await Promise.all(allStaff.map(async (staff) => {
            // ✅ SQL: Get reviews
            const reviewsData = await (0, db_1.selectQuery)('reviews', { staff_id: staff.id }, { orderBy: 'created_at', orderDirection: 'desc' });
            const reviewsArray = reviewsData || [];
            ...
            try {
                ...
                for (let i = 0; i < 7; i++) {
                    const date = new Date(Date.now() + i * 86400000).toISOString().split('T')[0];
                    const availabilityData = await (0, db_1.selectQuery)('staff_availability', { staff_id: staff.id, date: date, is_available: true }, { limit: 1 });
                    if (availabilityData && availabilityData.length > 0) {
                        const availability = availabilityData[0];
                        nextAvailable = {
                            date,
            ...
            return {
                ...
                consultationFee: staff.consultation_fee || staff.consultationFee || 0,
                gender: staff.gender || '',
    ...
    app.get('/make-server-3dd53475/customer/staff/:staffId', async (c) => {
        try {
            ...
            for (const style of ['at_center', 'at_home', 'tele']) {
                ...
                // Get staff services to filter
                const staffServices = await (0, db_1.selectQuery)('staff_services', { staff_id: staff.id, is_active: true });
                const staffServiceIds = staffServices.map((ss) => ss.service_id);
                // Filter services assigned to this staff
                const assignedServices = vendorServices.filter((s) => staffServiceIds.includes(s.service_id)).map((s) => ({ ...s, serviceStyle: style }));
            ...
            for (let i = 0; i < 7; i++) {
                const date = new Date(Date.now() + i * 86400000).toISOString().split('T')[0];
                const dayAvailData = await (0, db_1.selectQuery)('staff_availability', { staff_id: staff.id, date: date }, { limit: 1 });
            ...
            // ✅ SQL: Get reviews
            const reviewsData = await (0, db_1.selectQuery)('reviews', { staff_id: staff.id }, { orderBy: 'created_at', orderDirection: 'desc', limit: 10 });
            const reviewsArray = reviewsData || [];
            const rating = reviewsArray.length > 0
                ? reviewsArray.reduce((sum, r) => sum + (r.rating || 0), 0) / reviewsArray.length
                : 0;
            const staffDetails = {
                ...
                photo: staff.photo || staff.profile_photo_url || null,
                specialization: staff.specialization || '',
                degree: staff.degree || staff.qualifications || '',
                experience: staff.experience_years || staff.experience || 0,
                consultationFee: staff.consultation_fee || staff.consultationFee || 0,
                gender: staff.gender || null,
                languages: staff.languages || staff.spoken_languages || ['English', 'Hindi'],
                ...
                // Availability
                availability,
                nextAvailable: availability[0]?.slots?.find((s) => s.status === 'available') || null,
                // Role info
                roleId: staff.role || staff.role_id || null,
                roleName: staff.role || staff.role_name || null,
                serviceCategory: staff.service_category || null,
                // Metadata
                isActive: staff.is_active,
                canAcceptBookings: staff.canAcceptBookings !== false
            };
        ...
        try {
            ...
            for (const staffMember of vendorStaff) {
                if (staffMember.is_active) {
                    ...
                    const reviewsData = await (0, db_1.selectQuery)('reviews', { staff_id: staffMember.id }, { orderBy: 'created_at', orderDirection: 'desc' });
                    const reviewsArray = reviewsData || [];
                    const staffRating = reviewsArray.length > 0
                        ? reviewsArray.reduce((sum, r) => sum + (r.rating || 0), 0) / reviewsArray.length
                        : 0;
                    const staffMemberAny = staffMember;
                    staff.push({
                        id: staffMember.id,
                        name: staffMember.full_name || staffMemberAny.name,
                        photo: staffMemberAny.photo || null,
                        specialization: staffMember.specialization,
                        degree: staffMemberAny.degree || null,
                        experience: staffMemberAny.experience || null,
                        consultationFee: staffMemberAny.consultationFee || null,
            ...
            const centerDetails = {
                ...
                businessName: vendor.business_name || vendor.owner_name,
                fullName: vendor.owner_name,
                photo: vendorAny.business_logo || null,
                // Location
                address: vendor.address || '',
                city: vendor.city || '',
                state: vendor.state || '',
                ...
                reviewCount: totalReviews,
                reviews: allReviews.slice(0, 10).map((r) => ({
                    id: r.id,
                    rating: r.rating,
                    comment: r.comment,
                    customerId: r.customer_id,
...
//# sourceMappingURL=universal-customer-search-sql.js.map