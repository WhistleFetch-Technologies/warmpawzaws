...
exports.insuranceEndpoints = insuranceEndpoints;
const response_utils_1 = require("./response-utils");
const db_1 = require("../lib/db");
/**
 * Calculate premium based on pet age, breed, and coverage
 */
function calculatePremium(basePremium, petAge, petBreed, coverageAmount) {
...
function insuranceEndpoints(app) {
    const BASE_PATH = "/make-server-3dd53475";
    // Helper repository functions (inline SQL replacement - TODO: Create proper repository)
    const insuranceRepo = {
        getAllPlans: async (filters) => {
            ...
            return (0, db_1.selectQuery)('insurance_plans', where);
        },
        getPlanById: async (planId) => {
            const [result] = await (0, db_1.selectQuery)('insurance_plans', { id: planId }, { limit: 1 });
            return result || null;
        },
        createPolicy: async (data) => {
            const policyId = `policy_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            const now = new Date().toISOString();
            const [result] = await (0, db_1.insertQuery)('insurance_policies', {
                id: policyId,
                plan_id: data.planId,
        ...
        getPolicyById: async (policyId) => {
            const [result] = await (0, db_1.selectQuery)('insurance_policies', { id: policyId }, { limit: 1 });
            return result || null;
        },
        updatePolicy: async (policyId, data) => {
            const [result] = await (0, db_1.updateQuery)('insurance_policies', { id: policyId }, {
                ...data,
    ...
    app.get(`${BASE_PATH}/insurance/plans`, async (c) => {
        try {
            ...
            let plans = allPlans;
            if (minCoverage > 0) {
                plans = plans.filter((p) => ((p.coverage.accidentCover || 0) + (p.coverage.illnessCover || 0)) >= minCoverage);
            }
            if (maxPremium < 999999) {
                plans = plans.filter((p) => p.monthlyPremium <= maxPremium);
            }
            // Sort by premium
    ...
    app.post(`${BASE_PATH}/insurance/calculate-premium`, async (c) => {
        try {
            ...
            if (!plan) {
                return (0, response_utils_1.sendError)(c, 'Plan not found', 404);
            }
            const monthlyPremium = calculatePremium(plan.monthlyPremium, petAge, petBreed || 'mixed', coverageAmount || plan.coverage.accidentCover);
            return (0, response_utils_1.sendSuccess)(c, {
                planId,
    ...
    app.post(`${BASE_PATH}/insurance/policy/purchase`, async (c) => {
        try {
            const body = await c.req.json();
            const { customerId, petId, petName, petAge, petBreed, planId, paymentFrequency = 'monthly' } = body;
            ...
            if (petAge < plan.minCoverAge || (plan.maxCoverAge && petAge > plan.maxCoverAge)) {
                return (0, response_utils_1.sendError)(c, `Pet age must be between ${plan.minCoverAge} and ${plan.maxCoverAge || 'no limit'} years`, 400);
            }
            // Calculate premium
            const monthlyPremium = calculatePremium(plan.monthlyPremium, petAge, petBreed, plan.coverage.accidentCover);
    ...
    app.post(`${BASE_PATH}/insurance/policy/:policyId/documents/upload`, async (c) => {
        try {
            ...
            if (!policy) {
                return (0, response_utils_1.sendError)(c, 'Policy not found', 404);
            }
            ...
            const uploadedDocTypes = updatedDocuments.map((d) => d.documentType);
            ...
            if (!updated) {
                return (0, response_utils_1.sendError)(c, 'Failed to update policy', 500);
            }
    ...
    app.post(`${BASE_PATH}/insurance/claim/file`, async (c) => {
        try {
            const body = await c.req.json();
            const { policyId, claimType, incidentDate, claimAmount, description, veterinarianName, clinicName, documents = [] } = body;
            ...
            if (policy.status !== 'active') {
                return (0, response_utils_1.sendError)(c, 'Policy is not active', 400);
            }
            ...
            if (claimAmount > maxCover) {
                return (0, response_utils_1.sendError)(c, `Claim amount exceeds coverage limit of ₹${maxCover}`, 400);
            }
            // ✅ SQL: Create claim
    ...
    app.get(`${BASE_PATH}/insurance/claim/:claimId`, async (c) => {
        try {
            ...
            if (!claim) {
                return (0, response_utils_1.sendError)(c, 'Claim not found', 404);
            }
    ...
    app.post(`${BASE_PATH}/insurance/claim/:claimId/update-status`, async (c) => {
        try {
            ...
            if (!updated) {
                return (0, response_utils_1.sendError)(c, 'Failed to update claim', 500);
            }
    ...
    app.post(`${BASE_PATH}/insurance/plan/create`, async (c) => {
        try {
            const body = await c.req.json();
            const { planName, provider, type, coverage, monthlyPremium, deductible, maxCoverAge, minCoverAge = 0, waitingPeriod = 30, features = [], exclusions = [] } = body;
            ...
            return (0, response_utils_1.sendSuccess)(c, { plan }, 'Plan created successfully');
        }
        catch (error) {
...
//# sourceMappingURL=insurance-endpoints-sql.js.map