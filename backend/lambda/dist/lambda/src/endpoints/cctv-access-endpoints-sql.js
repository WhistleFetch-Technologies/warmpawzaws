...
exports.cctvAccessEndpointsSQL = cctvAccessEndpointsSQL;
...
async function getCCTVSettings() {
    ...
    const settingsResult = await pool.query('SELECT * FROM platform_settings WHERE setting_key = $1', ['cctv_config']);
...
async function updateCCTVSettings(settings) {
...
app.get('/make-server-3dd53475/vendor/cctv/:vendorId', async (c) => {
    try {
        ...
        const cameras = (settings.cameras || []).filter((cam) => cam.vendorId === vendorId);
        ...
        return c.json({
            ...
            onlineCount: cameras.filter((cam) => cam.status === 'online').length
        });
    }
    catch (error) {
...
app.get('/make-server-3dd53475/vendor/cctv/:vendorId/:cameraId', async (c) => {
    try {
        ...
        const camera = cameras.find((cam) => cam.id === cameraId && cam.vendorId === vendorId);
        if (!camera) {
            return c.json({
                success: false,
...
app.put('/make-server-3dd53475/vendor/cctv/:vendorId/:cameraId', async (c) => {
    try {
        ...
        const settings = await getCCTVSettings();
        const cameras = settings.cameras || [];
        const index = cameras.findIndex((cam) => cam.id === cameraId && cam.vendorId === vendorId);
        if (index === -1) {
            return c.json({
                success: false,
                error: 'Camera not found'
...
app.delete('/make-server-3dd53475/vendor/cctv/:vendorId/:cameraId', async (c) => {
    try {
        const { vendorId, cameraId } = c.req.param();
        const settings = await getCCTVSettings();
        settings.cameras = (settings.cameras || []).filter((cam) => !(cam.id === cameraId && cam.vendorId === vendorId));
        await updateCCTVSettings(settings);
        return c.json({
            success: true,
...
app.get('/make-server-3dd53475/vendor/cctv/:vendorId/access-logs', async (c) => {
    try {
        const vendorId = c.req.param('vendorId');
        const settings = await getCCTVSettings();
        const logs = (settings.access_logs || []).filter((log) => log.vendorId === vendorId);
...
function cctvAccessEndpointsSQL(mainApp) {
    mainApp.route('/', app);
}
exports.default = cctvAccessEndpointsSQL;
//# sourceMappingURL=cctv-access-endpoints-sql.js.map