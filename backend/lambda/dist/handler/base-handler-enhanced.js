...
exports.BaseHandlerEnhanced = void 0;
const jwt_verification_1 = require("../utils/jwt-verification");
...
class BaseHandlerEnhanced {
    ...
    getRequestId(event, context) {
        ...
        return context.requestId || context.awsRequestId || `req-${Date.now()}`;
    }
    /**
     * Parse request body
     */
     ...
     */
    async extractAndVerifyAuth(event) {
        const headers = this.getHeaders(event);
        // Use the utility function to extract and verify token
        const result = await (0, jwt_verification_1.extractAndVerifyAuthToken)(headers);
     ...
     */
    error(message, statusCode = 400, code = ERROR_CODES.INTERNAL_ERROR, details, requestId) {
     ...
     * Log request start
     */
    logRequestStart(event, requestId) {
     ...
     * Log request completion
     */
    logRequestComplete(event, response, duration, requestId) {
        const method = event.requestContext?.http?.method || 'UNKNOWN';
        const path = event.rawPath || event.requestContext?.http?.path || '/';
        console.log(JSON.stringify({
            level: 'INFO',
     ...
     * Handle errors
     */
    handleError(error, event, context, requestId) {
...
exports.BaseHandlerEnhanced = BaseHandlerEnhanced;
//# sourceMappingURL=base-handler-enhanced.js.map