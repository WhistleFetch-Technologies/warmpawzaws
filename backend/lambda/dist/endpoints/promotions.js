...
exports.registerPromotionEndpoints = registerPromotionEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerPromotionEndpoints(app) {
    /**
    ...
    app.get("/promotions/active", async (c) => {
      ...
      `;
            ...
            const promotions = await (0, rds_connection_1.query)(promotionsQuery, params);
            return c.json({
                success: true,
    ...
    app.post("/promotions/apply", async (c) => {
        try {
            ...
            // Get promotion
            const promotions = await (0, rds_connection_1.select)('promotions', { id: promotionId, is_active: true });
    ...
    app.get("/coupons/validate/:couponCode", async (c) => {
        try {
            ...
            const coupons = await (0, rds_connection_1.select)('coupons', { code: couponCode.toUpperCase(), is_active: true });
            if (coupons.length === 0) {
            ...
            // Check usage limit
            if (coupon.max_uses) {
                const usageCount = await (0, rds_connection_1.query)('SELECT COUNT(*) as count FROM coupon_usages WHERE coupon_id = $1', [coupon.id]).catch(() => ({ rows: [{ count: '0' }] }));
    ...
    app.post("/coupons/apply", async (c) => {
        try {
            ...
            const validation = await fetch(`/coupons/validate/${couponCode}?amount=${amount}`).then(r => r.json());
            if (!validation.success || !validation.valid) {
                return c.json({ error: validation.error || 'Invalid coupon' }, 400);
            }
            // Record usage
            if (customerId) {
                await (0, rds_connection_1.insert)('coupon_usages', {
    ...
    app.post("/admin/promotions", async (c) => {
        try {
            ...
            const { code, name, description, discount_type, discount_value, min_order_value, max_discount, valid_from, valid_until, usage_limit, usage_limit_per_user, applicable_to, is_active = true, } = body;
            ...
            const promotion = await (0, rds_connection_1.insert)('promotions', {
    ...
    app.put("/admin/promotions/:id", async (c) => {
        try {
            ...
            if (body.min_order_value !== undefined)
                updateData.min_order_amount = body.min_order_value;
            if (body.max_discount !== undefined)
                updateData.max_discount_amount = body.max_discount;
            if (body.valid_from !== undefined)
                updateData.start_date = new Date(body.valid_from);
            if (body.valid_until !== undefined)
                updateData.end_date = body.valid_until ? new Date(body.valid_until) : null;
            if (body.usage_limit !== undefined)
                updateData.max_uses = body.usage_limit;
            if (body.usage_limit_per_user !== undefined)
                updateData.max_uses_per_user = body.usage_limit_per_user;
            if (body.applicable_to !== undefined)
                updateData.applicable_to = body.applicable_to;
            if (body.is_active !== undefined)
                updateData.is_active = body.is_active;
            await (0, rds_connection_1.update)('promotions', { id }, updateData);
            const updated = await (0, rds_connection_1.select)('promotions', { id });
            return c.json({
                success: true,
                promotion: updated[0],
    ...
    app.delete("/admin/promotions/:id", async (c) => {
        try {
            const { id } = c.req.param();
            await (0, rds_connection_1.deleteRows)('promotions', { id });
            return c.json({
                success: true,
    ...
    app.post("/admin/coupons", async (c) => {
        try {
            const body = await c.req.json();
            const { code, discount_type, discount_value, min_order_value, max_discount, valid_from, valid_until, usage_limit, is_active = true, } = body;
            if (!code || !discount_type || discount_value === undefined) {
                return c.json({ error: 'code, discount_type, and discount_value are required' }, 400);
            }
            const coupon = await (0, rds_connection_1.insert)('coupons', {
                code: code.toUpperCase(),
                discount_type,
                discount_value,
                min_order_amount: min_order_value,
    ...
    app.put("/admin/coupons/:id", async (c) => {
        try {
            ...
            if (body.is_active !== undefined)
                updateData.is_active = body.is_active;
            await (0, rds_connection_1.update)('coupons', { id }, updateData);
            const updated = await (0, rds_connection_1.select)('coupons', { id });
            return c.json({
                success: true,
                coupon: updated[0],
    ...
    app.delete("/admin/coupons/:id", async (c) => {
        try {
            const { id } = c.req.param();
            await (0, rds_connection_1.deleteRows)('coupons', { id });
            return c.json({
                success: true,
