...
class GetEnhancedBookingDetailsHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        ...
        try {
            ...
            const [prescriptions, medicalRecords, chatMessages, pet, vendor, staff, service, customer,] = await Promise.all([
                // Prescriptions for this booking
                (0, rds_connection_1.query)(`SELECT * FROM prescriptions 
           WHERE booking_id = $1 AND is_active = true 
           ORDER BY created_at DESC`, [bookingId]).catch(() => ({ rows: [] })),
                // Medical records for this booking
                (0, rds_connection_1.query)(`SELECT * FROM medical_records 
           WHERE booking_id = $1 AND is_active = true 
           ORDER BY created_at DESC`, [bookingId]).catch(() => ({ rows: [] })),
                // Chat messages for this booking
                (0, rds_connection_1.query)(`SELECT * FROM chat_messages 
           WHERE booking_id = $1 
           ORDER BY created_at ASC`, [bookingId]).catch(() => ({ rows: [] })),
                // Pet information
                booking.pet_id
                    ? (0, rds_connection_1.select)('pets', { id: booking.pet_id }).catch(() => [])
                    : Promise.resolve([]),
                // Vendor information
                booking.vendor_id
                    ? (0, rds_connection_1.select)('vendors', { id: booking.vendor_id }).catch(() => [])
                    : Promise.resolve([]),
                // Staff information
                booking.staff_id || booking.assigned_staff_id
                    ? (0, rds_connection_1.select)('staff', { id: booking.staff_id || booking.assigned_staff_id }).catch(() => [])
                    : Promise.resolve([]),
                // Service information
                booking.service_id
            ...
            const response = {
                ...
                chat: {
                    ...
                    hasUnreadMessages: chatMessages.rows?.some((msg) => !msg.is_read) || false,
                },
                summary: {
 ...
 */
class GetBookingPrescriptionsHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        const bookingId = context.event.pathParameters?.bookingId;
        if (!bookingId) {
            return this.error('Booking ID is required', 400);
        ...
        try {
            const prescriptions = await (0, rds_connection_1.query)(`SELECT * FROM prescriptions 
         WHERE booking_id = $1 AND is_active = true 
         ORDER BY created_at DESC`, [bookingId]).catch(() => ({ rows: [] }));
 ...
 */
class GetBookingMedicalRecordsHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        const bookingId = context.event.pathParameters?.bookingId;
        if (!bookingId) {
            return this.error('Booking ID is required', 400);
        ...
        try {
            const medicalRecords = await (0, rds_connection_1.query)(`SELECT * FROM medical_records 
         WHERE booking_id = $1 AND is_active = true 
         ORDER BY created_at DESC`, [bookingId]).catch(() => ({ rows: [] }));
 ...
 */
class GetBookingChatHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        const bookingId = context.event.pathParameters?.bookingId;
        if (!bookingId) {
            return this.error('Booking ID is required', 400);
