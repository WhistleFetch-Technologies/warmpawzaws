...
exports.registerInsuranceEndpoints = registerInsuranceEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerInsuranceEndpoints(app) {
    /**
     * GET /insurance/plans
    ...
    app.get("/insurance/plans", async (c) => {
      ...
      `;
            ...
            const plans = await (0, rds_connection_1.query)(plansQuery, params).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
                plans: plans.rows,
    ...
    app.post("/insurance/policies", async (c) => {
        try {
            ...
            // Get plan
            const plans = await (0, rds_connection_1.select)('insurance_plans', { id: planId, is_active: true });
            if (plans.length === 0) {
            ...
            // Create policy
            const policy = await (0, rds_connection_1.insert)('insurance_policies', {
    ...
    app.get("/insurance/policies/customer/:customerId", async (c) => {
        try {
            ...
            const policies = await (0, rds_connection_1.query)(`SELECT p.*, pl.name as plan_name, pl.provider, pl.type as plan_type
         FROM insurance_policies p
         INNER JOIN insurance_plans pl ON p.plan_id = pl.id
         WHERE p.customer_id = $1
         ORDER BY p.created_at DESC`, [customerId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
    ...
    app.post("/insurance/claims", async (c) => {
        try {
            const { policyId, claimType, incidentDate, claimAmount, description, veterinarianName, clinicName, documents, } = await c.req.json();
            ...
            const policies = await (0, rds_connection_1.select)('insurance_policies', { id: policyId, status: 'active' });
            if (policies.length === 0) {
                return c.json({ error: 'Active insurance policy not found' }, 404);
            }
            const policy = policies[0];
            // Create claim
            const claim = await (0, rds_connection_1.insert)('insurance_claims', {
                policy_id: policyId,
    ...
    app.get("/insurance/claims/policy/:policyId", async (c) => {
        try {
            ...
            const claims = await (0, rds_connection_1.query)(`SELECT * FROM insurance_claims
         WHERE policy_id = $1
         ORDER BY created_at DESC`, [policyId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
                claims: claims.rows,
