...
exports.registerPetResortEndpoints = registerPetResortEndpoints;
...
// ============================================================================
class GetResortRoomsHandler extends base_handler_1.BaseHandler {
...
// ============================================================================
class GetRoomAvailabilityHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId;
            ...
            // Get all rooms
            const allRooms = await (0, rds_connection_1.query)(`
        SELECT * FROM resort_rooms
        WHERE vendor_id = $1 AND is_active = true
        ORDER BY room_number ASC
      `, [vendorId]);
            // Get bookings for the date range
            const bookings = await (0, rds_connection_1.query)(`
        SELECT 
          b.id,
          b.room_id,
          b.check_in_date,
      ...
      `, [vendorId, checkInDate, checkOutDate]);
            ...
            const availability = allRooms.rows.map((room) => {
                const roomBookings = bookings.rows.filter((b) => b.room_id === room.id);
            ...
            return this.success({
                ...
                rooms: availability,
                availableRooms: availability.filter((r) => r.isAvailable),
...
// ============================================================================
class CreateRoomHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId ||
            ...
            const { room_number, room_type, capacity, nightly_price, accommodates_small, accommodates_medium, accommodates_large, amenities, description, images } = body;
            if (!vendorId) {
                return this.error('Vendor ID is required', 400);
            }
      ...
      `, [vendorId, room_number]);
            ...
            // Create room
            const newRoom = await (0, rds_connection_1.query)(`
...
// ============================================================================
class UpdateRoomHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId ||
            ...
            const body = this.parseBody(context.event);
            const { room_type, capacity, nightly_price, accommodates_small, accommodates_medium, accommodates_large, amenities, description, images, is_active } = body;
            if (!vendorId || !roomId) {
                return this.error('Vendor ID and Room ID are required', 400);
            }
            // Verify room belongs to vendor
            const room = await (0, rds_connection_1.query)(`
        SELECT id FROM resort_rooms
...
// REGISTER ENDPOINTS
// ============================================================================
function registerPetResortEndpoints(app) {
...
//# sourceMappingURL=pet-resort.js.map