    ...
    async handle(context) {
        ...
        if (vendor.razorpay_account_id && vendor.bank_verified) {
            try {
                ...
                const payments = await (0, rds_connection_1.select)('payments', { booking_id: bookingId, payment_status: 'completed' });
                if (payments.length > 0 && payments[0].razorpay_payment_id) {
                    // Create transfer via Razorpay Route API
                    const transfer = await (0, razorpay_client_1.razorpayRequest)('/transfers', 'POST', {
                    ...
                    await (0, rds_connection_1.update)('settlements', { id: settlement.id }, {
