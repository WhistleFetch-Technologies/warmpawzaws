...
exports.registerPetCafeEndpoints = registerPetCafeEndpoints;
...
// ============================================================================
class GetCafeTablesHandler extends base_handler_1.BaseHandler {
...
// ============================================================================
class GetTableAvailabilityHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId;
            ...
            // Get all tables
            const allTables = await (0, rds_connection_1.query)(`
      ...
      `, [vendorId, date]);
            ...
            const availability = allTables.rows.map((table) => {
            ...
            return this.success({
                ...
                numberOfPax,
                tables: availability,
                availableTables: availability.filter((t) => t.isAvailable),
...
// ============================================================================
class CreateTableHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId ||
      ...
      `, [vendorId, table_number]);
            ...
            // Create table
            const newTable = await (0, rds_connection_1.query)(`
...
// ============================================================================
class UpdateTableHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId ||
            ...
            const table = await (0, rds_connection_1.query)(`
        SELECT id FROM cafe_tables
...
// REGISTER ENDPOINTS
// ============================================================================
function registerPetCafeEndpoints(app) {
...
//# sourceMappingURL=pet-cafe.js.map