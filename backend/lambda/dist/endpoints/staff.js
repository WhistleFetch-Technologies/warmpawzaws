...
function registerStaffEndpoints(app) {
    // ============================================
    ...
    app.get("/customer/discover-staff", async (c) => {
      ...
      `;
            ...
            if (roleId) {
                ...
                const role = await (0, rds_connection_1.query)('SELECT name, id FROM roles WHERE name = $1 OR id = $1', [roleId]);
          ...
          WHERE ss.staff_id = s.id AND ss.service_id = $${paramIndex}
            ...
            const staffResults = await (0, rds_connection_1.query)(staffQuery, params);
            ...
            if (serviceStyle === 'at_home' && customerLat && customerLng) {
                ...
                eligibleStaff = eligibleStaff.filter((staff) => staff.service_styles && JSON.parse(staff.service_styles || '[]').includes('at_center'));
            }
            else if (serviceStyle === 'tele') {
                // Filter staff available for tele
                eligibleStaff = eligibleStaff.filter((staff) => staff.service_styles && JSON.parse(staff.service_styles || '[]').includes('tele'));
            }
            // Get availability for each staff
            const staffWithAvailability = await Promise.all(eligibleStaff.map(async (staff) => {
                const availability = await (0, rds_connection_1.query)(`SELECT * FROM staff_availability 
             WHERE staff_id = $1 
    ...
    app.get("/vendor/:vendorId/staff", async (c) => {
        try {
            ...
            const staff = await (0, rds_connection_1.select)('staff', { vendor_id: vendorId }, { orderBy: 'created_at', orderDirection: 'DESC' });
            // Enrich with services and availability
            const enrichedStaff = await Promise.all(staff.map(async (s) => {
                const services = await (0, rds_connection_1.query)('SELECT ss.*, sv.name as service_name FROM staff_services ss INNER JOIN services sv ON ss.service_id = sv.id WHERE ss.staff_id = $1', [s.id]);
                return {
                    ...s,
    ...
    app.post("/vendor/:vendorId/staff", async (c) => {
        try {
            ...
            if (roleName && roleName.length === 36 && roleName.includes('-')) {
                ...
                const role = await (0, rds_connection_1.query)('SELECT name FROM roles WHERE id = $1', [roleName]);
                if (role.rows.length > 0) {
                    roleName = role.rows[0].name;
                }
            }
            const staff = await (0, rds_connection_1.insert)('staff', {
                vendor_id: vendorId,
                name: staffData.name,
            ...
            if (staffData.services && Array.isArray(staffData.services)) {
                for (const serviceId of staffData.services) {
                    await (0, rds_connection_1.insert)('staff_services', {
                        staff_id: staff[0].id,
    ...
    app.put("/vendor/:vendorId/staff/:staffId", async (c) => {
        try {
            ...
            const staffData = await c.req.json();
            const updated = await (0, rds_connection_1.update)('staff', { id: staffId }, {
                name: staffData.name,
                phone: staffData.phone,
            ...
            if (staffData.services && Array.isArray(staffData.services)) {
                ...
                await (0, rds_connection_1.query)('DELETE FROM staff_services WHERE staff_id = $1', [staffId]);
                // Insert new services
                for (const serviceId of staffData.services) {
                    await (0, rds_connection_1.insert)('staff_services', {
                        staff_id: staffId,
    ...
    app.delete("/vendor/:vendorId/staff/:staffId", async (c) => {
        try {
            const { staffId } = c.req.param();
            await (0, rds_connection_1.update)('staff', { id: staffId }, { is_active: false });
    ...
    app.get("/vendor/:vendorId/staff/:staffId/availability", async (c) => {
      ...
      `;
            ...
            const availability = await (0, rds_connection_1.query)(availabilityQuery, params);
    ...
    app.post("/vendor/:vendorId/staff/:staffId/availability", async (c) => {
        try {
            ...
            for (const avail of availabilities) {
                const result = await (0, rds_connection_1.insert)('staff_availability', {
                    staff_id: staffId,
