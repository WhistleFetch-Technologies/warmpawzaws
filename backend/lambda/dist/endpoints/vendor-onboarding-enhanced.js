...
exports.registerVendorOnboardingEndpointsEnhanced = registerVendorOnboardingEndpointsEnhanced;
...
const vendors_1 = require("@warmpawz/api-contracts/vendors");
// ============================================================================
// PHASE 1: AUTH & ENTRY
// ============================================================================
class GetOnboardingStatusHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        ...
        try {
            ...
            let identity = await (0, rds_connection_1.select)('vendor_identity', { phone });
            if (identity.length === 0) {
                // Create new identity with INIT status
                const newIdentity = await (0, rds_connection_1.insert)('vendor_identity', {
                    phone,
            ...
            if (vendorIdentity.application_id) {
                const apps = await (0, rds_connection_1.select)('vendor_onboarding_applications', {
            ...
            return this.error(error.message || 'Failed to get onboarding status', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
    getNextStep(status) {
        const stepMap = {
...
// ============================================================================
class GetAvailableRolesHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        const requestId = context.requestId;
        try {
            ...
            return this.error(error.message || 'Failed to get roles', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
class SelectRoleHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        const body = this.parseBody(context.event);
        const requestId = context.requestId;
        // Validate request with Zod schema
        const validationResult = vendors_1.SelectVendorRoleRequestSchema.safeParse(body);
        ...
        try {
            // Get vendor identity
            const identities = await (0, rds_connection_1.select)('vendor_identity', { phone });
            if (identities.length === 0) {
                return this.error('Vendor identity not found', 404, 'NOT_FOUND', undefined, requestId);
            }
            const identity = identities[0];
            // Validate role exists and is active
            const roles = await (0, rds_connection_1.select)('roles', { id: role_id, is_active: true });
            if (roles.length === 0) {
                return this.error('Role not found or inactive', 404, 'NOT_FOUND', undefined, requestId);
            }
            // Update identity with selected role
            await (0, rds_connection_1.update)('vendor_identity', { id: identity.id }, {
                selected_role_id: role_id,
                updated_at: new Date().toISOString(),
            });
            // Transition to ROLE_PENDING if currently INIT
            if (identity.onboarding_status === 'INIT') {
                await (0, rds_connection_1.query)(`SELECT transition_onboarding_status($1, $2, NULL, 'system', 'role_selected', '{}'::jsonb)`, [identity.id, 'ROLE_PENDING']);
            }
            return this.success({
            ...
            return this.error(error.message || 'Failed to select role', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
// ============================================================================
// PHASE 3: VENDOR TYPE
// ============================================================================
class SelectVendorTypeHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        const body = this.parseBody(context.event);
        const requestId = context.requestId;
        // Validate request with Zod schema
        const validationResult = vendors_1.SelectVendorTypeRequestSchema.safeParse(body);
        if (!validationResult.success) {
            return this.error('Validation failed', 400, 'VALIDATION_ERROR', { errors: validationResult.error.errors }, requestId);
        }
        ...
        try {
            ...
            const roles = await (0, rds_connection_1.select)('roles', { id: identity.selected_role_id });
            if (roles.length === 0) {
            ...
            if (!supportedTypes.includes(vendor_type)) {
                return this.error(`Vendor type '${vendor_type}' is not supported for this role. Supported: ${supportedTypes.join(', ')}`, 400, 'VALIDATION_ERROR', undefined, requestId);
            }
            // Update identity
            await (0, rds_connection_1.update)('vendor_identity', { id: identity.id }, {
                vendor_type,
            ...
            if (identity.onboarding_status === 'ROLE_PENDING') {
                await (0, rds_connection_1.query)(`SELECT transition_onboarding_status($1, $2, NULL, 'system', 'vendor_type_selected', '{}'::jsonb)`, [identity.id, 'FORM_PENDING']);
            }
            return this.success({
            ...
            return this.error(error.message || 'Failed to select vendor type', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
// ============================================================================
...
// ============================================================================
class GetOnboardingFormSchemaHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        const phone = context.event.queryStringParameters?.phone;
        const requestId = context.requestId;
        if (!phone) {
        ...
        try {
            ...
            if (!identity.selected_role_id || !identity.vendor_type) {
                return this.error('Role and vendor type must be selected first', 400, 'VALIDATION_ERROR', undefined, requestId);
            }
            // Get form schema from role config
            const schemaResult = await (0, rds_connection_1.query)(`SELECT get_onboarding_form_schema($1, $2) as schema`, [identity.selected_role_id, identity.vendor_type]);
            const schema = schemaResult.rows[0]?.schema;
            if (!schema) {
                return this.error('Form schema not found for this role and vendor type', 404, 'NOT_FOUND', undefined, requestId);
            }
            ...
            return this.error(error.message || 'Failed to get form schema', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
class SubmitApplicationHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        const body = this.parseBody(context.event);
        const requestId = context.requestId;
        // Validate request with Zod schema
        const validationResult = vendors_1.SubmitVendorApplicationRequestSchema.safeParse(body);
        if (!validationResult.success) {
            return this.error('Validation failed', 400, 'VALIDATION_ERROR', { errors: validationResult.error.errors }, requestId);
        }
        ...
        try {
            ...
            if (applicationId) {
                ...
                if (apps.length > 0) {
                    ...
                    if (app.status !== 'DRAFT' && app.status !== 'CLARIFICATION_REQUIRED') {
                        return this.error('Application is locked and cannot be edited', 403, 'FORBIDDEN', undefined, requestId);
                    }
                    // Update existing application
                    await (0, rds_connection_1.update)('vendor_onboarding_applications', { id: applicationId }, {
                        application_payload,
                ...
                const newApp = await (0, rds_connection_1.insert)('vendor_onboarding_applications', {
                ...
                await (0, rds_connection_1.update)('vendor_identity', { id: identity.id }, { application_id: applicationId });
            }
            // Transition to UNDER_REVIEW
            if (identity.onboarding_status === 'FORM_PENDING' || identity.onboarding_status === 'CLARIFICATION_REQUIRED') {
                await (0, rds_connection_1.query)(`SELECT transition_onboarding_status($1, $2, NULL, 'vendor', 'application_submitted', '{}'::jsonb)`, [identity.id, 'UNDER_REVIEW']);
            }
            return this.success({
            ...
            return this.error(error.message || 'Failed to submit application', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
// ============================================================================
...
// ============================================================================
class AdminReviewApplicationHandlerEnhanced extends base_handler_enhanced_1.BaseHandlerEnhanced {
    async handle(context) {
        ...
        // Validate request with Zod schema
        const validationResult = vendors_1.AdminReviewApplicationRequestSchema.safeParse(body);
        if (!validationResult.success) {
            return this.error('Validation failed', 400, 'VALIDATION_ERROR', { errors: validationResult.error.errors }, requestId);
        }
        ...
        try {
            ...
            if (application.status !== 'UNDER_REVIEW') {
                return this.error('Application is not in UNDER_REVIEW status', 400, 'VALIDATION_ERROR', undefined, requestId);
            }
            // Get vendor identity
            const identities = await (0, rds_connection_1.select)('vendor_identity', {
            ...
            if (action === 'APPROVE') {
                ...
                if (!comments) {
                    return this.error('Comments are required for clarification request', 400, 'VALIDATION_ERROR', undefined, requestId);
                }
                ...
                if (!rejection_reason) {
                    return this.error('Rejection reason is required', 400, 'VALIDATION_ERROR', undefined, requestId);
                }
                newStatus = 'REJECTED';
            ...
            await (0, rds_connection_1.query)(`SELECT transition_onboarding_status($1, $2, $3, 'admin', $4, $5::jsonb)`, [
                identity.id,
                newOnboardingStatus,
                admin_id,
                action.toLowerCase(),
            ...
            return this.error(error.message || 'Failed to review application', 500, 'INTERNAL_ERROR', undefined, requestId);
        }
    }
}
// ============================================================================
// HONO ROUTER SETUP
// ============================================================================
function registerVendorOnboardingEndpointsEnhanced(app) {
...
//# sourceMappingURL=vendor-onboarding-enhanced.js.map