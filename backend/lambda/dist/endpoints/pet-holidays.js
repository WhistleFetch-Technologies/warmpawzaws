...
exports.registerPetHolidaysEndpoints = registerPetHolidaysEndpoints;
...
// ============================================================================
class GetHolidayPackagesHandler extends base_handler_1.BaseHandler {
    async handle(context) {
      ...
      `;
            ...
            const packages = await (0, rds_connection_1.query)(packagesQuery, params);
            return this.success({
...
// ============================================================================
class GetHolidayPackageDetailsHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            ...
            // Get package details
            const packageData = await (0, rds_connection_1.query)(`
        SELECT 
          hp.*,
          v.business_name as vendor_name,
          v.city as vendor_city,
...
// ============================================================================
class GetVendorHolidayPackagesHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            ...
            const packages = await (0, rds_connection_1.query)(`
        SELECT 
          hp.*,
          COUNT(b.id) as booking_count,
...
// ============================================================================
class CreateHolidayPackageHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const vendorId = context.event.pathParameters?.id ||
                context.event.pathParameters?.vendorId ||
            ...
            const { title, destination, duration_days, price, group_size, tour_type, inclusions, exclusions, itinerary, applicable_dates, images, description } = body;
            if (!vendorId) {
                return this.error('Vendor ID is required', 400);
            }
            ...
            // Create package
            const newPackage = await (0, rds_connection_1.query)(`
...
// ============================================================================
class BookHolidayHandler extends base_handler_1.BaseHandler {
    async handle(context) {
        try {
            const customerId = context.userId;
            const body = this.parseBody(context.event);
            const { packageId, travelDate, numberOfPets, petIds, specialRequests } = body;
            if (!customerId) {
      ...
      `, [packageId]);
            ...
            if (packageInfo.applicable_dates) {
                ...
                const isApplicable = applicableDates.some((dateRange) => {
            ...
            // Create booking
            const booking = await (0, rds_connection_1.query)(`
...
// REGISTER ENDPOINTS
// ============================================================================
function registerPetHolidaysEndpoints(app) {
...
//# sourceMappingURL=pet-holidays.js.map