...
exports.registerTrainingProgressEndpoints = registerTrainingProgressEndpoints;
const rds_connection_1 = require("../database/rds-connection");
function registerTrainingProgressEndpoints(app) {
    /**
    ...
    app.post("/training/session/:sessionId/progress", async (c) => {
        try {
            ...
            const { skillsPracticed, behaviorObserved, issuesAddressed, improvementAreas, trainerNotes, rating, media, } = await c.req.json();
            // Get session (from package_sessions table)
            const sessions = await (0, rds_connection_1.select)('package_sessions', { id: sessionId });
            if (sessions.length === 0) {
                return c.json({ error: 'Training session not found' }, 404);
            }
            const session = sessions[0];
            // Update session with progress
            const updated = await (0, rds_connection_1.update)('package_sessions', { id: sessionId }, {
                status: 'completed',
    ...
    app.get("/training/progress/:packageId", async (c) => {
        try {
            ...
            const sessions = await (0, rds_connection_1.query)(`SELECT * FROM package_sessions
         WHERE package_id = $1
         ORDER BY session_number ASC`, [packageId]);
            ...
            const milestones = await (0, rds_connection_1.query)(`SELECT * FROM training_milestones
         WHERE package_id = $1
         ORDER BY target_session ASC`, [packageId]).catch(() => ({ rows: [] }));
            return c.json({
                success: true,
                progress: {
                    totalSessions,
    ...
    app.post("/training/milestones", async (c) => {
        try {
            const { packageId, petId, milestoneName, description, targetSession, criteria, } = await c.req.json();
            ...
            const milestone = await (0, rds_connection_1.insert)('training_milestones', {
    ...
    app.put("/training/milestones/:milestoneId/achieve", async (c) => {
        try {
            ...
            const updated = await (0, rds_connection_1.update)('training_milestones', { id: milestoneId }, {
                status: 'achieved',
...
//# sourceMappingURL=training-progress.js.map