...
exports.registerTierSystemEndpoints = registerTierSystemEndpoints;
...
function registerTierSystemEndpoints(app) {
    /**
    ...
    app.get("/vendor/:vendorId/tier", async (c) => {
        try {
            ...
            // Get vendor stats
            const bookings = await (0, rds_connection_1.query)(`SELECT COUNT(*) as count, SUM(total_amount) as revenue 
         FROM bookings 
         WHERE vendor_id = $1 AND status = 'completed'`, [vendorId]).catch(() => ({ rows: [{ count: '0', revenue: '0' }] }));
            ...
            return c.json({
                ...
                tier: {
                    current: currentTier,
                    commission: TIER_CONFIG[currentTier].commission,
                    stats: {
                        totalBookings,
                        totalRevenue,
                    },
                    nextTier: nextTier ? {
                        ...
                        requirements: TIER_CONFIG[nextTier],
    ...
    app.post("/vendor/:vendorId/tier/upgrade", async (c) => {
        try {
            ...
            // Update tier
            const updated = await (0, rds_connection_1.update)('vendors', { id: vendorId }, {
                tier: newTier,
                commission_percentage: TIER_CONFIG[newTier].commission,
                metadata: {
                    ...(vendor.metadata || {}),
                    tierHistory: [
                        ...(vendor.metadata?.tierHistory || []),
                        {
                            from: oldTier,
                            to: newTier,
    ...
    app.post("/admin/tiers/calculate-commissions", async (c) => {
        try {
            const vendors = await (0, rds_connection_1.select)('vendors', { is_active: true });
...
//# sourceMappingURL=tier-system.js.map