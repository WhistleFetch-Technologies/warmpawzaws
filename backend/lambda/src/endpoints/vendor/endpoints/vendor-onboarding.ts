...
class ActivateVendorHandler extends BaseHandler {
  async handle(context: HandlerContext): Promise<HandlerResponse> {
    ...
    try {
      ...
      // ✅ CRITICAL FIX: Exclude is_deleted from payload to prevent it from being set to true
      const { is_deleted, ...safePayload } = payload;
      ...
      if (safePayload.metadata && typeof safePayload.metadata === 'object') {
        const cleanMetadata = { ...safePayload.metadata };
      ...
      vendorData.tier = tierResolved.tier;
      vendorData.commission_percentage = tierResolved.commission_percentage;
