...
class GetRadarDistanceHandler extends BaseHandler {
...
class UpdateRadarDistanceHandler extends BaseHandler {
  async handle(context: HandlerContext): Promise<HandlerResponse> {
    try {
      const vendorId = context.event.pathParameters?.id || 
                      context.event.pathParameters?.vendorId ||
...
export function registerVendorRadarEndpoints(app: Hono) {
