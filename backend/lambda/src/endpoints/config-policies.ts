...
export function registerConfigPoliciesEndpoints(app: Hono) {
  ...
  /**
   * GET /config/fees
   * Get fee configuration for a service type
   */
  app.get("/config/fees", async (c) => {
    try {
      const serviceType = c.req.query('service_type') || 'default';

      // Try to get fees from database
      let fees = {
      ...
      try {
        const settings = await query(
          `SELECT * FROM admin_settings 
           WHERE setting_key LIKE '%fee%' OR setting_key LIKE '%delivery%'
        ...
        for (const row of settings.rows) {
          ...
          if (!isNaN(value)) {
            switch (row.setting_key) {
              case 'platform_fee_percentage': fees.platformFeePercentage = value; break;
              case 'convenience_fee': fees.convenienceFee = value; break;
              case 'platform_fee_flat': fees.platformFeeFlat = value; break;
              case 'max_platform_fee': fees.maxPlatformFee = value; break;
              case 'delivery_base_fee': fees.deliveryBaseFee = value; break;
              case 'delivery_per_km_rate': fees.deliveryPerKmRate = value; break;
              case 'free_delivery_threshold': fees.freeDeliveryThreshold = value; break;
            }
          }
        }
      } catch (dbError) {
        console.warn('[CONFIG] Could not fetch fees from DB, using defaults');
      }
