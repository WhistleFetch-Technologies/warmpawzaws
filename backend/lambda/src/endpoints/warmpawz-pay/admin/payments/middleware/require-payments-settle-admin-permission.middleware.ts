...
import {
  ...
  type WpayPaymentsSettlePermissionId,
...
function forbiddenResponse(c: Context): Response {
  const body: CatalogueErrorResponse = {
    ...
    error: {
      ...
      message: 'Insufficient permissions to settle Warmpawz Pay payouts',
    },
  };
  return c.json(body, 403);
}

export function requirePaymentsSettleAdminPermission(
  permission: WpayPaymentsSettlePermissionId,
): MiddlewareHandler {
  return async (c: Context, next: Next) => {
    ...
    if (!hasWpayPaymentsSettlePermission(permissions, permission)) {
