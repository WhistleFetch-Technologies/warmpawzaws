...
export async function dbCustomerBookingsBookingidGet0(bookingId: string) {
  return await query(
    `SELECT b.*,
...
export async function dbCustomerBookingsBookingidGet1(bookingId: string) {
  return await query(
...
export async function dbCustomerBookingsBookingidGet2(bookingId: string) {
  return await query(`SELECT completion_otp FROM bookings WHERE id = $1`, [bookingId]);
}

export async function dbCustomerBookingsBookingidGet3(bookingId: string) {
  return await query(
    `SELECT otp_code FROM otp_tokens
     WHERE metadata->>'bookingId' = $1
       AND metadata->>'action' = 'end'
...
export async function dbCustomerBookingsBookingidGet4(bookingId: string) {
  return await query('SELECT * FROM prescriptions WHERE booking_id = $1', [bookingId]);
}

export async function dbCustomerBookingsBookingidGet5(bookingId: string, booking: { customer_id: string }) {
  return await query(
