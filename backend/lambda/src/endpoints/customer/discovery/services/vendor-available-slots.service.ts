...
import * as vendor_available_slotsRepo from '../repos/vendor-available-slots.repo';
...
export async function executevendorAvailableSlots(c: Context) {
    ...
    try {
      ...
      if (!vendor) {
        ...
        // That endpoint uses getVendorIdsForAvailabilityLookup and queries with ANY(executevendorAvailableSlots::text[])
      ...
      try {
        const policies = await vendor_available_slotsRepo.dbVendorAvailableSlots11().catch(() => ({ rows: [] }));
      ...
      try {
        const bookResult = await vendor_available_slotsRepo
          .dbVendorAvailableSlots28(resolvedVendorId, date)
