...
import * as vendor_servicesRepo from '../repos/vendor-services.repo';
...
export async function executevendorServices(c: Context) {
    ...
    try {
            ...
            for (const pp of purchases.rows || []) {
              ...
              if (Array.isArray(inc) && inc.length > 0) {
                ...
                if (vsRow.rows?.length > 0) {
                  ...
                  const psRows = await vendor_servicesRepo.dbVendorServices2(pp)
      ...
      const result = await vendor_servicesRepo.dbVendorServices4(servicesQuery, queryParams)
