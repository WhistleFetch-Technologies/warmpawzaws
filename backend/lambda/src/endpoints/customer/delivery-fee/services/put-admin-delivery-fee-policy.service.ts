...
export async function executeputAdminDeliveryFeePolicy(c: Context) {
  try {
    ...
    if (existing.rows.length > 0) {
      await put_admin_delivery_fee_policyRepo.dbPutAdminDeliveryFeePolicy1(jsonStr, POLICY_KEY);
    } else {
      await put_admin_delivery_fee_policyRepo.dbPutAdminDeliveryFeePolicy2(
        POLICY_KEY,
        jsonStr,
        DESCRIPTION
      );
