...
import * as customer_announcements_getRepo from '../repos/customer_announcements_get.repo';
...
export async function executecustomerAnnouncementsGet(c: Context) {
    try {
      const limit = parseInt(c.req.query('limit') || '5', 10);

      // Fetch announcements from platform_settings
      const settingsResult = await customer_announcements_getRepo.dbCustomerAnnouncementsGet0().catch(() => ({ rows: [] }));
