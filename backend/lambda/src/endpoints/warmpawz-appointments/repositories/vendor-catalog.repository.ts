...
import { VENDOR_APPROVED_ACTIVE_SQL } from '../shared/merchant/merchant-eligibility-sql';
import { wapptCatalogueCustomerVisibleSql } from '../shared/catalogue-eligibility-sql';
...
const CUSTOMER_VISIBLE_PREDICATE = wapptCatalogueCustomerVisibleSql('c');
...
export class VendorCatalogRepository implements IVendorCatalogRepository {
  ...
  async updateAppointmentFee(params: UpdateAppointmentFeeParams): Promise<CatalogueRow | null> {
    const sql = `
      UPDATE ${CATALOGUE_TABLE}
      SET
        appointment_fee = $2,
