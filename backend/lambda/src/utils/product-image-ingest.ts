...
/** Matches vendor upload compression target (see image-types product budget). */
...
export async function ingestExternalProductImageUrl(
  ...
  try {
    ...
    if (!validation.ok) {
      console.warn('[product-image-ingest] Invalid image bytes, keeping original URL:', url, validation.message);
      return url;
    }
    const asset = await uploadDisplayImage({
      buffer: body,
      declaredContentType: validation.detectedMime,
      assetType: 'product',
      ownerId: vendorId,
      vendorId,
    });
