...
export async function publishToSNS(
  ...
  // Direct SMS path: used by referral/send payloads.
  // This avoids dependency on topic subscribers for simple OTP/referral texts.
  if (message?.type === 'sms' && message?.phone) {
    try {
      const smsCommand = new PublishCommand({
        PhoneNumber: String(message.phone),
      ...
      const smsResponse = await snsClient.send(smsCommand);
      console.log(`SNS direct SMS published: ${smsResponse.MessageId}`);
      return { messageId: smsResponse.MessageId || '' };
    } catch (error) {
      console.error('Error publishing direct SMS via SNS:', error);
      throw error;
    }
  }
