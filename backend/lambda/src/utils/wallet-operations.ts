...
const MEAL_ORDER_WALLET_DESC = 'Meal order wallet debit';

/**
 * Debit wallet for a one-time meal order checkout (no booking_id FK).
 */
export async function debitCustomerWalletForMealOrderInTransaction(
  client: PoolClient,
  params: {
    customerId: string;
    mealOrderId: string;
  ...
  const { customerId, mealOrderId, amount, idempotencyKey } = params;
  if (!customerId || !mealOrderId || !Number.isFinite(amount) || amount <= 0) {
    return { debited: 0, balanceAfter: 0 };
  }
  ...
  const desc = idempotencyKey
    ? `${MEAL_ORDER_WALLET_DESC} ${mealOrderId} ${idempotencyKey}`
    : `${MEAL_ORDER_WALLET_DESC} ${mealOrderId}`;
  ...
  await client.query('SAVEPOINT sp_sync_customer_wallet_balance_meal_order');
  try {
    await client.query(
      `UPDATE customers SET wallet_balance = GREATEST(0, COALESCE(wallet_balance, 0) - $1::numeric) WHERE id = $2::uuid`,
      [amount, customerId],
    );
    await client.query('RELEASE SAVEPOINT sp_sync_customer_wallet_balance_meal_order');
  } catch {
    await client.query('ROLLBACK TO SAVEPOINT sp_sync_customer_wallet_balance_meal_order');
  }
