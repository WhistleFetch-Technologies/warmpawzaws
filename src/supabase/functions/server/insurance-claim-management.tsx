...
// Initialize claims bucket
const initializeClaimsBucket = async () => {
  ...
  try {
    ...
    const bucketExists = buckets?.some(bucket => bucket.name === CLAIMS_BUCKET);
    
    if (!bucketExists) {
      console.log(`📦 Creating insurance claims bucket: ${CLAIMS_BUCKET}`);
      const { error } = await supabase.storage.createBucket(CLAIMS_BUCKET, {
        public: false, // Private bucket - requires signed URLs
        fileSizeLimit: 20971520 // 20MB limit
      });
      
      if (error && error.statusCode !== '409') {
        console.error('❌ Error creating claims bucket:', error);
      } else {
    ...
    console.error('❌ Error initializing claims bucket:', error);
  }
};

initializeClaimsBucket();
