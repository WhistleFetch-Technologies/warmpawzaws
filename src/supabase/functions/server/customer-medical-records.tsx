...
// Initialize medical documents bucket
const initializeMedicalDocsBucket = async () => {
  ...
  try {
    ...
    const bucketExists = buckets?.some(bucket => bucket.name === MEDICAL_DOCS_BUCKET);
    
    if (!bucketExists) {
      console.log(`📦 Creating medical documents bucket: ${MEDICAL_DOCS_BUCKET}`);
      const { error } = await supabase.storage.createBucket(MEDICAL_DOCS_BUCKET, {
        public: false, // Private bucket
        fileSizeLimit: 10485760 // 10MB limit
      });
      
      if (error && error.statusCode !== '409') {
        console.error('❌ Error creating medical docs bucket:', error);
      } else {
        console.log('✅ Medical docs bucket created successfully');
      }
    } else {
      console.log('✅ Medical docs bucket already exists');
    }
  } catch (error) {
    console.error('❌ Error initializing medical docs bucket:', error);
  }
};

initializeMedicalDocsBucket();
 ...
 */
app.post('/pets/:petId/medical-documents/upload', async (c) => {
  try {
 ...
 */
app.get('/pets/:petId/medical-documents', async (c) => {
  try {
    const petId = c.req.param('petId');
 ...
 * Upload pet photo
 */
app.post('/pets/:petId/photo', async (c) => {
  try {
    const petId = c.req.param('petId');
 ...
 */
app.delete('/medical-documents/:documentId', async (c) => {
  try {
 ...
 */
app.post('/medical-documents/share', async (c) => {
  try {
