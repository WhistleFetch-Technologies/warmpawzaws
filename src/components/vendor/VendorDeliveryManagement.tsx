  ...
  return (
    <div className="fixed inset-0 bg-[#FF8C42] black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-[#FF8C42] white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-gray-200 bg-[#FF8C42] gradient-to-r from-purple-50 to-blue-50">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div className="bg-[#FF8C42] purple-50 rounded-lg p-3 text-center border border-purple-200">
        ...
        <div className="p-4 border-b border-gray-200 bg-[#FF8C42] gray-50">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => setFilter('in_transit')} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                filter === 'in_transit' ? 'bg-purple-600 text-white' : 'bg-white text-gray-700 border border-gray-300'
              }`}
            >
              In Transit ({stats.inTransit})
            </Button>
            <Button onClick={() => setFilter('delivered')} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
            ...
            <div className="space-y-4">
              {filteredDeliveries.map((delivery) => {
                ...
                return (
                  <div
                    key={delivery.id}
                    className="bg-[#FF8C42] white border-2 border-gray-200 rounded-xl p-4 hover:border-purple-300 transition-colors cursor-pointer"
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex gap-2 overflow-x-auto">
                        {delivery.items?.slice(0, 3).map((item: any, idx: number) => (
                          <div key={idx} className="flex-shrink-0 bg-[#FF8C42] purple-50 rounded-lg px-3 py-1 text-xs">
                            {item.name} (x{item.quantity})
                          </div>
                        ))}
                        {delivery.items?.length > 3 && (
                          <div className="flex-shrink-0 bg-[#FF8C42] gray-100 rounded-lg px-3 py-1 text-xs text-gray-600">
                    ...
                    <div className="flex gap-2">
                      <Button onClick={(e) => {
                        ...
                        className="flex-1 py-2 px-4 bg-blue-50 hover:bg-[#FF8C42] blue-100 text-blue-700 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                      >
                      ...
                      <Button onClick={(e) => {
                        ...
                        className="flex-1 py-2 px-4 bg-purple-50 hover:bg-[#FF8C42] purple-100 text-purple-700 rounded-lg font-medium transition-colors"
                      >
                        View Details
                      </Button>
                    </div>
        ...
        {showDetailsModal && selectedDelivery && (
          <div className="absolute inset-0 bg-[#FF8C42] black/50 flex items-center justify-center p-4">
            <div className="bg-[#FF8C42] white rounded-xl p-6 max-w-md w-full max-h-[80vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  Delivery Details
                </h3>
                <Button onClick={() => setShowDetailsModal(false)}
                  className="p-1 hover:bg-[#FF8C42] gray-100 rounded-full"
                >
                  <X className="w-5 h-5" />
                </Button>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedDelivery.items?.map((item: any, idx: number) => (
                      <div key={idx} className="bg-[#FF8C42] gray-50 rounded-lg p-2 flex justify-between">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {selectedDelivery.status === 'pending' && (
                        <Button onClick={() => updateDeliveryStatus(selectedDelivery.id, 'picked_up')}
                          className="py-2 px-4 bg-indigo-600 hover:bg-[#FF8C42] indigo-700 text-white rounded-lg font-medium transition-colors"
                        >
                          Mark as Picked Up
                        </Button>
                      )}
                      {(selectedDelivery.status === 'picked_up' || selectedDelivery.status === 'pending') && (
                        <Button onClick={() => updateDeliveryStatus(selectedDelivery.id, 'in_transit')}
                          className="py-2 px-4 bg-purple-600 hover:bg-[#FF8C42] purple-700 text-white rounded-lg font-medium transition-colors"
                        >
                          Mark as In Transit
                        </Button>
                      )}
                      {(selectedDelivery.status === 'in_transit' || selectedDelivery.status === 'picked_up') && (
                        <Button onClick={() => updateDeliveryStatus(selectedDelivery.id, 'out_for_delivery')}
                          className="py-2 px-4 bg-blue-600 hover:bg-[#FF8C42] blue-700 text-white rounded-lg font-medium transition-colors"
                        >
                          Mark as Out for Delivery
                        </Button>
                      )}
                      {selectedDelivery.status === 'out_for_delivery' && (
                        <Button onClick={() => updateDeliveryStatus(selectedDelivery.id, 'delivered')}
                          className="py-2 px-4 bg-green-600 hover:bg-[#FF8C42] green-700 text-white rounded-lg font-medium transition-colors"
                        >
                          Mark as Delivered
                        </Button>
                      )}
