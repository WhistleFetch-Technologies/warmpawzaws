...
export function ShiprocketOrderIntegration({ orderId, order, onUpdate }: ShiprocketOrderIntegrationProps) {
  ...
  const handleCreateShiprocketOrder = async () => {
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to create Shiprocket order';
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  const handleGeneratePickup = async () => {
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to generate pickup';
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  return (
    <Card className="p-6 mt-6 bg-white border-blue-100 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-gray-900">Shiprocket Logistics</h2>
      </div>
      ...
      {!shiprocketOrder ? (
        <div className="text-center py-6 bg-gray-50 rounded-lg border border-dashed border-gray-200">
          ...
          <p className="text-gray-600 mb-4 text-sm">
            Create a Shiprocket order for this e-commerce order to enable shipping.
          </p>
          <Button
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
            <div>
              ...
              <p className="text-sm font-semibold mt-1">{shiprocketOrder.orderId}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Shipment ID</label>
              <p className="text-sm font-semibold mt-1">{shiprocketOrder.shipmentId}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wide">AWB Code</label>
              <p className="text-sm font-mono mt-1 text-blue-600">{shiprocketOrder.awbCode || 'Pending Assignment'}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Courier</label>
              <p className="text-sm font-semibold mt-1">{shiprocketOrder.courierName || '-'}</p>
            </div>
          </div>
              ...
              <div className="relative pl-4 border-l-2 border-gray-200 space-y-6">
                {tracking.tracking_data?.shipment_track?.map((event: any, index: number) => (
                  <div key={index} className="relative">
                    <div className={`absolute -left-[21px] top-0 w-4 h-4 rounded-full border-2 border-white ring-1 ring-gray-200 bg-blue-500`}></div>
                    <div className="flex flex-col">
                      <p className="font-medium text-sm text-gray-900">{event.status}</p>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <p className="text-xs text-gray-500 ml-auto pl-10">
              Enter the Courier ID from your Shiprocket dashboard serviceability check.
            </p>
          </div>
          <DialogFooter>
