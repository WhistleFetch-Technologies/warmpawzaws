...
import { 
  Percent, Save, Plus, Trash2, Edit2, TrendingUp, DollarSign, 
  Layers, Users, Tag, AlertCircle, Check, X, ChevronDown, ChevronUp,
  Info, Copy, Eye, EyeOff, BarChart3
} from 'lucide-react';
...
export function CommissionSettings() {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDefaultRate(data.settings.defaultRate || 15);
        setRules(data.settings.rules || []);
        setVendorTiers(data.settings.vendorTiers || getDefaultVendorTiers());
      } else {
  ...
  const getDefaultVendorTiers = (): VendorTier[] => [
    ...
    },
    {
      ...
      name: 'Gold',
      commissionRate: 12,
      requirements: { minMonthlyOrders: 200, minRating: 4.5, minRevenue: 100000 },
      benefits: ['24/7 support', 'Premium listing', 'Advanced analytics', 'Marketing support'],
    ...
    },
    {
      ...
      name: 'Platinum',
      commissionRate: 8,
      requirements: { minMonthlyOrders: 500, minRating: 4.8, minRevenue: 500000, verificationRequired: true },
      benefits: ['Dedicated manager', 'Top placement', 'Full analytics suite', 'Co-marketing', 'Custom support'],
  ...
  const calculateCommission = (orderValue: number, category?: string, vendorTier?: string): number => {
      ...
      if (matches) {
        if (commission.type === 'percentage') {
          ...
          const tier = commission.tiers.find(t => 
            orderValue >= t.min && (t.max === null || orderValue < t.max)
          );
          if (tier) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-black text-xl font-semibold">Enterprise Commission Management</h2>
          <p className="text-gray-500 text-sm mt-1">Multi-tier commission rules for marketplace optimization</p>
        </div>
        <Button
          onClick={handleSave}
          disabled={saving}
          ...
          {vendorTiers.map((tier) => (
            <div key={tier.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-2xl font-bold text-gray-900">{tier.commissionRate}%</span>
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="font-bold text-gray-900">
                          {rule.commission.type === 'percentage' && `${rule.commission.value}%`}
                ...
                {expandedRules.has(rule.id) && (
                  <div className="p-4 border-t border-gray-200 bg-white">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2 text-sm">
                          {rule.conditions.categories && (
                            <div>
                              ...
                              <span className="font-medium">{rule.conditions.categories.join(', ')}</span>
                            </div>
                          )}
                          {rule.conditions.minOrderValue !== undefined && (
                            <div>
                              <span className="text-gray-500">Min Order:</span>{' '}
                              <span className="font-medium">₹{rule.conditions.minOrderValue}</span>
                            </div>
                          )}
                          {rule.conditions.maxOrderValue !== undefined && (
                            <div>
                              <span className="text-gray-500">Max Order:</span>{' '}
                              <span className="font-medium">₹{rule.conditions.maxOrderValue}</span>
                            </div>
                          )}
                      ...
                      <div>
                        <h5 className="text-sm font-semibold text-gray-700 mb-2">Commission Details</h5>
                        <div className="space-y-2 text-sm">
      ...
      <div className="bg-white rounded-lg p-6 space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center pt-2">
            ...
            <span className="font-bold text-green-600 text-xl">₹{vendorEarnings.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditedRule({
                  ...editedRule,
                  commission: { ...editedRule.commission, type: e.target.value as any }
                })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF8C42]"
              >
            ...
            {(editedRule.commission.type === 'percentage' || editedRule.commission.type === 'fixed') && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {editedRule.commission.type === 'percentage' ? 'Percentage (%)' : 'Fixed Amount (₹)'}
                </label>
                <input
                  type="number"
                  ...
                  onChange={(e) => setEditedRule({
                    ...editedRule,
                    commission: { ...editedRule.commission, value: parseFloat(e.target.value) }
                  })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF8C42]"
                  step="0.1"
                />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditedRule({
                      ...
                      conditions: { 
                        ...editedRule.conditions, 
                        maxOrderValue: e.target.value ? parseFloat(e.target.value) : undefined 
                      }
                    })}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF8C42]"
                    placeholder="Optional"
