        ...
        <TableBody>
          {data.map((vendor) => (
            <TableRow key={vendor.id}>
              ...
              <TableCell className="text-right">{vendor.totalBookings}</TableCell>
              ...
              <TableCell className="text-right">
                <div className={`flex items-center justify-end gap-1 ${vendor.growth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {vendor.growth >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
