    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/ai/chat`,
        {
          ...
          body: JSON.stringify({
            message: `[Current Mode: ${mode.toUpperCase()}] ${userMessage}`, // Inject mode context
            customerId,
            history
          })
        }
      ...
      if (!response.ok) {
         ...
         // Show fallback message instead of throwing
      ...
      if (data.action === 'suggest_ticket' || data.action === 'create_ticket') {
