...
export function CustomerChatInterface({
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/followup/chat/send`,
        {
          ...
          body: JSON.stringify({
            bookingId,
            senderPhone: phone.replace(/[^0-9]/g, ''),
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('senderPhone', phone.replace(/[^0-9]/g, ''));
  ...
  const previewFileFunc = async (fileId: string, fileType: string, fileName: string) => {
    try {
      ...
      console.error('❌ [CHAT] Error previewing file:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="w-full max-w-[430px] mx-auto bg-white flex flex-col h-screen">
        {/* Header */}
        <div className="bg-gradient-to-br from-[#FF8C42] to-[#FF7029] text-white px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold truncate">{vendorName}</h1>
            <p className="text-xs text-white/80 truncate">Chat for {petName}</p>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`max-w-[75%] ${isOwnMessage ? '' : 'flex gap-2'}`}>
                        {!isOwnMessage && (
                          <div className="w-8 h-8 bg-gradient-to-br from-[#FF8C42] to-[#FF7029] rounded-full flex items-center justify-center flex-shrink-0 text-white text-xs font-semibold">
        ...
        <div className="border-t border-gray-200 p-4 bg-white">
          ...
          <p className="text-xs text-gray-500 mt-2 text-center">
            Vet will respond within 2 hours during working hours
          </p>
        </div>
      </div>
