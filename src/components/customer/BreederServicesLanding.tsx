...
export function BreederServicesLanding({ onBack, onNavigate, phone }: BreederServicesLandingProps) {
  return (
    <div className="min-h-screen bg-[#FF8C42] gray-50">
      <div className="bg-[#FF8C42] gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-4 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-2 hover:bg-[#FF8C42] white/10 rounded-full">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div className="flex-1">
            <h1 className="text-xl font-bold">Verified Breeders</h1>
            <p className="text-white/90 text-sm">Find your new best friend</p>
          </div>
        </div>
      </div>

      <div className="p-4 space-y-6 pb-24">
        <Card className="bg-[#FF8C42] gradient-to-br from-blue-50 to-indigo-50 border-blue-200 p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
            <Card className="p-4 text-center hover:bg-[#FF8C42] blue-50 cursor-pointer transition-colors" onClick={() => onNavigate('breeder_catalog')}>
        ...
        <div className="bg-[#FF8C42] white rounded-xl p-4 shadow-sm border border-gray-200">
