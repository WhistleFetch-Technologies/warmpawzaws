...
interface PhotographyServiceRouterProps {
  onBack: () => void;
  phone: string;
...
export function PhotographyServiceRouter({ onBack, phone, onNavigate, onViewBooking, data }: PhotographyServiceRouterProps) {
  ...
  const fetchProblemDetails = async (problemId: string) => {
    try {
      const response = await fetch(
        `${API_BASE}/customer/universal-problem-discovery?problemGridId=${problemId}&roleId=pet_photographer`,
        { headers: { Authorization: `Bearer ${publicAnonKey}` } }
      );
  ...
  return (
    <PhotographyServicesLanding
      ...
      onNavigate={(screen, data) => {
        ...
        } else if (screen === 'photographer_service' && data?.vendorId) {
