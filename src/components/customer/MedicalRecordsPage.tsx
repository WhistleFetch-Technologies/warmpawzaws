...
export function MedicalRecordsPage({ customerId, petId: initialPetId }: MedicalRecordsPageProps) {
  ...
  const loadPets = async () => {
    try {
      ...
      if (data.pets && data.pets.length > 0 && !selectedPet) {
  ...
  const loadDocuments = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/pets/${selectedPet}/medical-documents`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            'Content-Type': 'application/json'
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/pets/${selectedPet}/medical-documents/upload`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const deleteDocument = async (documentId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/medical-documents/${documentId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const shareDocument = async (documentId: string, vetId: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/medical-documents/share`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
      ...
      alert('✅ Document shared with vet successfully');
    } catch (err) {
