...
interface BreederServiceRouterProps {
  onBack: () => void;
  phone: string;
...
export function BreederServiceRouter({ onBack, phone, onNavigate, onViewBooking, data }: BreederServiceRouterProps) {
  ...
  const fetchProblemDetails = async (problemId: string) => {
    try {
      const response = await fetch(
        `${API_BASE}/customer/universal-problem-discovery?problemGridId=${problemId}&roleId=pet_breeder`,
        { headers: { Authorization: `Bearer ${publicAnonKey}` } }
      );
  ...
  return (
    <BreederServicesLanding
      ...
      onNavigate={(screen, data) => {
        ...
        } else if (screen === 'breeder_service' && data?.vendorId) {
