...
import { TaxiServicesLanding } from './TaxiServicesLanding';
...
interface TaxiServiceRouterProps {
  onBack: () => void;
  phone: string;
...
export function TaxiServiceRouter({ onBack, phone, onNavigate, onViewBooking, data }: TaxiServiceRouterProps) {
  ...
  const fetchProblemDetails = async (problemId: string) => {
    try {
      const response = await fetch(
        `${API_BASE}/customer/universal-problem-discovery?problemGridId=${problemId}&roleId=pet_taxi`,
        { headers: { Authorization: `Bearer ${publicAnonKey}` } }
      );
  ...
  return (
    <TaxiServicesLanding
      ...
      onNavigate={(screen, data) => {
        ...
        } else if (screen === 'taxi_service' && data?.vendorId) {
