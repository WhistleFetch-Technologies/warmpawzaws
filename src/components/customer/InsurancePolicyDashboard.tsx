  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [policiesRes, claimsRes] = await Promise.all([
        fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/insurance/customer/${customerId}/policies`,
          { headers: { 'Authorization': `Bearer ${publicAnonKey}` } }
        ),
        fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/insurance/customer/${customerId}/claims`,
          { headers: { 'Authorization': `Bearer ${publicAnonKey}` } }
        )
      ]);
