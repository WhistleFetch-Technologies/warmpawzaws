...
export function RewardsLoyaltyPage({ customerId }: RewardsLoyaltyPageProps) {
  ...
  const loadLoyaltyProfile = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load loyalty profile');
      }
  ...
  const loadRewardsCatalog = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/loyalty/rewards-catalog`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        throw new Error('Failed to load rewards catalog');
      }
  ...
  const redeemPoints = async (rewardId: string, pointsCost: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/loyalty/${customerId}/redeem`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
