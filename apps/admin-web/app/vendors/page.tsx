...
export default function AdminVendorManagement() {
	...
	const [roleFilter, setRoleFilter] = useState<
		"all" | "vet" | "grooming" | "walking" | "boarding" | "training"
	>("all");
	...
	const [rejectingApplication, setRejectingApplication] =
		useState<VendorApplication | null>(null);
	const [requestingInfoApplication, setRequestingInfoApplication] =
		useState<VendorApplication | null>(null);
	...
	const loadData = async () => {
		try {
			...
			try {
				...
				throw new Error(
					"Server is not responding. Please ensure the backend is deployed and running."
				);
			}

			// Load stats
			try {
				const statsData = await apiClient.get("/admin/vendors/stats");
			...
			try {
				const allVendorsData = await apiClient.get("/admin/vendors/all");

				console.log("========================================");
				console.log("📋 FRONTEND: Received vendor data from backend");
				console.log("========================================");
				console.log(
					"📦 Total vendors received:",
					(allVendorsData as any).vendors?.length || 0
				);
				console.log("📊 Status breakdown:");
				const statusCounts: any = {};
				((allVendorsData as any).vendors || []).forEach((v: any) => {
				...
				const allVendorsList = (allVendorsData as any).vendors || [];

				// Deduplicate vendors by ID
				const uniqueVendors = Array.from(
					new Map(allVendorsList.map((item: any) => [item.id, item])).values()
				) as any[];
			...
			// Load quality alerts
			try {
				const alertsData = await apiClient.get("/quality/alerts");
				setQualityAlerts((alertsData as any).alerts || []);
			} catch (error) {
			...
			console.error(
				"Error type:",
				errorObj instanceof TypeError ? "Network/Fetch Error" : "Other Error"
			);
	...
	const filterVendorsByStatus = (
		...
		if (status === "all") {
			...
			setApplications(allVendors.filter((v: any) => (v as any).status === status));
		}
	};
	...
	const resetAndSeedVendors = async () => {
		if (
			!confirm(
				"⚠️ This will DELETE all existing vendors and create fresh seed data. Continue?"
			)
		) {
			return;
		}
		...
		try {
			...
			const data = await apiClient.post("/admin/seed/reset-and-seed");
			console.log("✅ Reset and seed complete:", data);
			alert(
				`✅ Success!\n\nCleared: ${
					(data as any).results.cleared.deleted?.profiles || 0
				} vendors\nCreated: ${
					(data as any).results.seeded.success
				} new vendors\n\nRefreshing page...`
			);
	...
	const clearAllVendorsOnly = async () => {
		...
		try {
			...
			const data = await apiClient.post("/admin/seed/clear-vendors");
			console.log("✅ Clear complete:", data);

			const report = (data as any).results.report;
			alert(
	...
	const debugVendorLookup = async () => {
		...
		try {
			...
			const data = await apiClient.get(`/debug/vendor-lookup/${phone}`);
			console.log("🔍 Debug results:", (data as any).debug);
	...
	const handleApprove = async (applicationId: string) => {
		...
		try {
			...
			if (!vendor) {
				...
				console.error(
					...
					applications.map((a: any) => ({
						id: a.id,
						vendorId: (a as any).vendorId || a.id,
						name: (a as any).fullName || (a as any).businessName || 'Unknown',
					}))
				);
			...
			console.log("========================================");
			setSuccessMessage(
				"Application Approved! Vendor can now access their dashboard."
			);
	...
	const handleReject = async (reason: string, notes?: string) => {
		...
		try {
			const rejectVendorId = (rejectingApplication as any).vendorId || rejectingApplication.id;
			console.log("🔄 Rejecting application:", rejectingApplication.id);
			console.log("📋 Found vendor for rejection:", {
				vendorId: rejectVendorId,
				applicationId: (rejectingApplication as any).applicationId,
				fullName: (rejectingApplication as any).fullName,
			});

			await apiClient.post("/admin/vendor/reject", {
				vendorId: rejectVendorId,
				rejectedBy: "Admin",
				reason: reason,
	...
	const handleRequestMoreInfo = async (
		...
		try {
			console.log(
				"🔄 Requesting more info for application:",
				requestingInfoApplication.id
			);
			const requestInfoVendorId = (requestingInfoApplication as any).vendorId || requestingInfoApplication.id;
			console.log("📋 Found vendor for info request:", {
				vendorId: requestInfoVendorId,
				applicationId: (requestingInfoApplication as any).applicationId,
				fullName: (requestingInfoApplication as any).fullName,
			});

			await apiClient.post("/admin/vendor/request-info", {
				vendorId: (requestingInfoApplication as any).vendorId || requestingInfoApplication.id,
				requestedBy: "Admin",
				message: message,
	...
	const handleFlushAllVendors = async () => {
		...
		try {
			...
			const result = await apiClient.delete("/admin/vendor/flush-all");
			console.log("✅ Flush completed:", result);
			alert(
				`✅ Success!\n\nDeleted ${(result as any).deletedCount} vendor records.\n\nThe system is now clean for fresh testing.`
			);
			setSuccessMessage(`Flushed ${(result as any).deletedCount} Records`);
			setShowSuccessModal(true);
	...
	const handleSeedVendors = async () => {
		...
		try {
			...
			const result = await apiClient.post("/admin/seed-vendors");
			console.log("✅ Seed completed:", result);
			alert(
				`✅ Success!\n\n` +
					`Created ${(result as any).totalCreated} vendor applications.\n\n` +
					`${(result as any).note}\n\n` +
					`You can now test the approval/rejection workflow.`
			);
			setSuccessMessage(`Seeded ${(result as any).totalCreated} Vendors`);
			setShowSuccessModal(true);
	...
	const handleFixCategories = async () => {
		...
		try {
			...
			const result = await apiClient.post("/admin/fix-vendor-categories");
			console.log("✅ Fix completed:", result);
			alert(
				`✅ Success!\n\n` +
					`Fixed ${(result as any).fixed} vendors.\n` +
					`Errors: ${(result as any).errors}\n\n` +
					`Service categories are now properly mapped!`
			);
			setSuccessMessage(`Fixed ${(result as any).fixed} Vendors`);
			setShowSuccessModal(true);
			loadData(); // Reload data
		} catch (error) {
	...
	const handleFixVendorIndexes = async () => {
		...
		try {
			...
			const result = await apiClient.post("/admin/vendors/fix-indexes");
			console.log("✅ Fix completed:", result);
			alert(
				`✅ Vendor Indexes Fixed Successfully!\n\n` +
					`Total Vendors: ${(result as any).stats.total}\n` +
					`Indexes Created: ${(result as any).stats.fixed}\n` +
					`Already Had Indexes: ${(result as any).stats.skipped}\n\n` +
			...
			setSuccessMessage(`Fixed ${(result as any).stats.fixed} Vendor Indexes`);
			setShowSuccessModal(true);
			loadData(); // Reload data
		} catch (error) {
			...
			<div className="min-h-screen bg-gray-50 flex items-center justify-center">
				<div className="text-center">
					<div
						...
						style={{
							borderColor: `${WARM_ORANGE} transparent ${WARM_ORANGE} ${WARM_ORANGE}`,
						}}
					></div>
	...
	return (
		<div className="min-h-screen bg-gray-50 flex">
				...
				<div className="flex-1 overflow-y-auto p-6">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
						...
						>
							<StatCard
								...
								subtitle={`+${
									stats?.pendingApplications.todayCount || 0
								} today`}
								hasBar={true}
						...
						<div
							onClick={() => handleStatCardClick("pending_reverification")}
							className="cursor-pointer"
						>
							<StatCard
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
						...
						<div className="bg-white rounded-xl p-6 border border-gray-200">
								...
								<div className="absolute inset-0 flex items-center justify-center">
									<div className="text-center">
										<div className="text-2xl">
											{(stats?.distribution.active || 0) +
												(stats?.distribution.deactivated || 0) +
												(stats?.distribution.pending || 0)}
										</div>
							...
							<div className="space-y-2">
								<LegendItem
									color="bg-blue-500"
									label={`Active Vendors (${stats?.distribution.active || 0})`}
								/>
								<LegendItem
									color="bg-gray-400"
					...
					<div className="bg-white rounded-xl border border-gray-200">
						...
						<div className="p-3 sm:p-6">
							...
							{activeTab === "deactivation" && <DeactivationRequestsTab />}

							{activeTab === "rate-changes" && <RateChangesTab />}

							{activeTab === "reverification" && <ReverificationTab />}

							{activeTab === "support" && <SupportVendorTab />}

							{activeTab === "compliance" && <ComplianceIssuesTab />}

							{activeTab === "payment-disputes" && <PaymentDisputesTab />}

							{activeTab === "active-vendors" && <ActiveVendorsTab />}
			...
			<RequestInfoModal
				...
				vendorName={
					requestingInfoApplication?.fullName ||
					requestingInfoApplication?.businessName ||
					"Vendor"
				}
...
// 				active ? "bg-[#FF8C42] text-white" : "text-gray-700 hover:bg-gray-100"
// 			}`}
// 			onClick={onClick}
// 		>
// 			{icon}
						...
						{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
							(day, idx) => (
								<span
									...
									className={
										day === "Fri"
											? "bg-[#FF8C42] text-white px-2 py-0.5 rounded"
											: ""
									}
								>
									{day}
...
function QuickAccessCard({ icon, label, bgColor, onClick }: any) {
	return (
		<button
			className={`${bgColor} rounded-xl p-4 text-left hover:opacity-80 transition-opacity`}
			onClick={onClick}
		>
