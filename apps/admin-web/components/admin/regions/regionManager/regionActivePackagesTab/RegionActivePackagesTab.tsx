...
export function RegionActivePackagesTab({
	...
	const loadPackages = async () => {
		try {
			setLoading(true);
			const data = await apiClient.get<any>(`/admin/regions/${regionId}/packages`);
			const responseData = (data as any).data || data;
			
			setPackages(responseData.packages || []);
			setTotalCount(responseData.count || responseData.packages?.length || 0);

			// Group by category
			const grouped: PackagesByCategory = {};
			(responseData.packages || []).forEach((pkg: any) => {
