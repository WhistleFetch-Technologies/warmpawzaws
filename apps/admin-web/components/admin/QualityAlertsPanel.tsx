...
export function QualityAlertsPanel({ onViewVendor, maxAlerts = 5 }: QualityAlertsPanelProps) {
  ...
  const loadAlerts = async () => {
    try {
      setLoading(true);
      const data = await apiClient.get<any>('/quality/alerts');
    ...
    return (
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-900">Quality Alerts</h3>
        </div>
    ...
    return (
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <div className="text-sm text-gray-500 text-center py-4">
          No quality alerts at this time. All vendors are performing well.
        </div>
      </div>
    );
  }
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className="font-semibold text-gray-900">{alert.vendor_name || 'Unknown Vendor'}</h4>
      ...
      {alerts.length >= maxAlerts && (
        <div className="mt-4 text-center">
          <Button variant="outline" size="sm" onClick={() => {
            // Navigate to full alerts view or expand
            window.location.href = '/vendors?tab=active-vendors&filter=quality-alerts';
          }}>
            View All Alerts
          </Button>
        </div>
