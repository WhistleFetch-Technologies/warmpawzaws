...
export function AdminDashboard({ session, onNavigate, initialView }: AdminDashboardProps) {
  ...
  const handleVerifyVendor = async (vendorId: string, status: 'approved' | 'rejected', remarks: string = '') => {
    ...
    try {
      await apiClient.post(`/admin/vendors/${vendorId}/verify`, { status, remarks });
  ...
  const approveAllVendors = async () => {
    setActionLoading(true);
    try {
      const data = await apiClient.post<any>('/admin/fix/approve-all-vendors');
      alert(`✅ ${data.message}. Approved ${data.details.approved} vendors!`);
      await loadVendors();
    } catch (error: any) {
      console.error('Error approving vendors:', error);
      alert(error.message || 'Failed to approve vendors');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const publishVendorServices = async () => {
    setActionLoading(true);
    try {
      const data = await apiClient.post<any>('/admin/fix/publish-vendor-services');
      alert(`✅ ${data.message}. Published ${data.details.published} vendor services!`);
      await loadVendors();
    } catch (error: any) {
      console.error('Error publishing vendor services:', error);
      alert(error.message || 'Failed to publish vendor services');
    } finally {
      setActionLoading(false);
    }
  };
        ...
        <div className="bg-white border-b px-0 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                size="sm"
                className="bg-[#FF8C42] hover:bg-[#FF7A2E] h-9 gap-3"
              >
          ...
          {activeView === 'vendor-admin' ? (
            ...
            <Card className="p-4 border-green-200 bg-green-50/50">
              ...
              <div>
                ...
                <div className="h-8 flex items-end gap-3.5">
