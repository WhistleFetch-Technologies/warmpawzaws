...
export function AWSIntegrationsSettings() {
	...
	const loadSettings = async () => {
		try {
			...
			// Load AWS settings
			const awsRes = await apiClient.get<any>("/admin/settings/aws");
			if (awsRes.success && awsRes.settings) {
				const loadedAws = {
					credentials: {
						accessKeyId: awsRes.settings.credentials?.accessKeyId || "",
						secretAccessKey: awsRes.settings.credentials?.secretAccessKey || "",
						region: awsRes.settings.credentials?.region || "ap-south-1",
					},
					s3: {
						enabled: awsRes.settings.s3?.enabled || false,
						bucket: awsRes.settings.s3?.bucket || "",
						region: awsRes.settings.s3?.region || "ap-south-1",
					},
					sns: {
						enabled: awsRes.settings.sns?.enabled || false,
						region: awsRes.settings.sns?.region || "ap-south-1",
						smsOriginationNumber:
							awsRes.settings.sns?.smsOriginationNumber || "",
						emailSourceAddress: awsRes.settings.sns?.emailSourceAddress || "",
					},
					sqs: {
						enabled: awsRes.settings.sqs?.enabled || false,
						queueUrl: awsRes.settings.sqs?.queueUrl || "",
						region: awsRes.settings.sqs?.region || "ap-south-1",
					},
					chime: {
						enabled: awsRes.settings.chime?.enabled || false,
						region: awsRes.settings.chime?.region || "us-east-1",
					},
					bedrock: {
			...
			const razorpayRes = await apiClient.get<any>(
				"/admin/settings/payment-gateway"
			);
			if (razorpayRes.success && razorpayRes.settings?.razorpay) {
				const loadedRazorpay = {
					bankVerificationEnabled:
						razorpayRes.settings.razorpay.bankVerificationEnabled || false,
					key_id: razorpayRes.settings.razorpay.key_id || "",
					key_secret: razorpayRes.settings.razorpay.key_secret || "",
				};
			...
			const mapsRes = await apiClient.get<any>("/admin/settings/google-maps");
			if (mapsRes.success && mapsRes.settings) {
				const loadedMaps = {
					enabled: mapsRes.settings.enabled || false,
					apiKey: mapsRes.settings.apiKey || "",
					region: mapsRes.settings.region || "IN",
				};
	...
	const handleSaveAll = async () => {
		try {
			...
			// Save AWS settings
			await apiClient.post<any>("/admin/settings/aws", aws);

			// Save Razorpay settings
			await apiClient.post<any>("/admin/settings/payment-gateway", {
				razorpay: {
					...razorpay,
					enabled: true,
				},
			...
			await apiClient.post<any>("/admin/settings/google-maps", googleMaps);
