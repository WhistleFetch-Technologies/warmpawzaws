...
import { Gift, Plus, Save, Trophy, Users } from "lucide-react";
...
export function RewardsLoyaltyManagement() {
	...
	const fetchRules = async () => {
		try {
			...
			await apiClient.post<any>("/loyalty/rules/init", {});

			const res = await apiClient.get<any>("/loyalty/rules");
			if (res.success) {
				setRules(res.rules || []);
			}
		} catch (error) {
	...
	const handleSave = async () => {
		try {
			setLoading(true);
			await apiClient.put<any>("/loyalty/rules", { rules });
										...
										<div className="flex items-center gap-2">
											<Input
												...
												onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
													updatePoints(idx, e.target.value)
												}
												className="w-20"
											/>
