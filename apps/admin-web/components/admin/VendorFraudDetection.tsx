...
interface FraudAlert {
  ...
  alertType: 'suspicious_payment' | 'unusual_activity' | 'duplicate_account' | 'rating_manipulation' | 'cancellation_pattern';
  description: string;
  detectedAt: string;
  evidence: {
...
interface AbnormalBehavior {
  vendorId: string;
  vendorName: string;
  behaviorType: 'high_cancellation' | 'low_rating' | 'unusual_timing' | 'payment_anomaly';
...
export function VendorFraudDetection() {
  ...
  useEffect(() => {
    loadFraudData();
    const interval = setInterval(loadFraudData, 60000); // Refresh every minute
  ...
  const loadFraudData = async () => {
    try {
      ...
      const [alertsData, behaviorsData] = await Promise.all([
        apiClient.get<any>('/admin/vendors/fraud-alerts').catch(() => {
          console.error('Error loading fraud alerts');
          return { alerts: [] };
        }),
        apiClient.get<any>('/admin/vendors/abnormal-behavior').catch(() => {
          console.error('Error loading abnormal behaviors');
          return { behaviors: [] };
        })
      ]);
  ...
  const handleAction = async (alertId: string, action: 'investigate' | 'resolve' | 'dismiss') => {
    try {
      await apiClient.post(`/admin/vendors/fraud-alerts/${alertId}/${action}`);
      loadFraudData();
    } catch (error) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Suspicious activities requiring attention</p>
            </div>
          </div>
          <Badge className="bg-red-600 text-white">
            {fraudAlerts.filter(a => a.status === 'new').length} New
          </Badge>
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-gray-500">
                      {alert.evidence.transactionCount && (
                        <span>Transactions: {alert.evidence.transactionCount}</span>
                      )}
                      {alert.evidence.cancellationRate && (
                        <span>Cancellation: {alert.evidence.cancellationRate}%</span>
                      )}
                      {alert.evidence.ratingAnomaly && (
                        <span>Rating Anomaly: {alert.evidence.ratingAnomaly}</span>
                      )}
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Vendors with unusual patterns</p>
            </div>
          </div>
        </div>
        ...
        {abnormalBehaviors.length === 0 ? (
          <div className="text-center py-8">
            <CheckCircle className="w-12 h-12 text-green-400 mx-auto mb-3" />
            <p className="text-gray-600">No abnormal behaviors detected</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {abnormalBehaviors.map((behavior, idx) => (
              <div
                key={idx}
                className={`p-4 rounded-lg border-2 ${getSeverityColor(behavior.severity)}`}
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500">
                      (Threshold: {behavior.metrics.threshold})
                    </span>
                  </div>
