#!/usr/bin/env node
/**
 * QA / Functional validation – run with: npm run verify:capabilities
 * Prints Rules 1–4 checklist and summary. No DB, pure code-path verification.
 */
...
const CAPABILITY_VERIFICATION = {
  ...
  rule3_calling_notification: {
    ...
    steps: [
      'Vendor: When customer starts video, notify-ready creates notification → vendor poll /notifications → TeleCallNotification shown with ringtone.',
  ...
  rule4_vendor_new_booking: {
    ...
    steps: [
      'Backend: On booking create (bookings-enhanced), insert notification recipient_id=vendor_id, type=new_booking, message with customerName, serviceName, date/time.',
      'Vendor: useVendorNotificationService polls /vendor/notifications, onNewNotification called.',
      'When notification.type === "new_booking", setNewBookingAlert(notification).',
...
function run() {
  console.log('\n=== Capability verification checklist (Rules 1–4) ===\n');
  const results = [];
  for (const [key, v] of Object.entries(CAPABILITY_VERIFICATION)) {
    console.log(`\n--- ${v.name} ---`);
    v.steps.forEach((step, i) => {
      console.log(`  ${i + 1}. ${step}`);
    });
    console.log('  Key files:');
    v.keyFiles.forEach((f) => console.log(`    - ${f}`));
    results.push({ rule: v.name, steps: v.steps.length, passed: true });
  }
  console.log('\n--- Summary ---');
  results.forEach((r) => {
    console.log(`  ${r.rule}: ${r.steps} steps (checklist only – validate manually in app)`);
  });
  console.log('\nDone. Use this list for QA / functional validation.\n');
  process.exit(0);
}
