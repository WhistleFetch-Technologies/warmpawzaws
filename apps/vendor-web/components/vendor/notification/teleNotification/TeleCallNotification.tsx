...
import { TeleCallNotificationProps } from '../constants/interface';

//Play ringtone for incoming video call (loud notification)
...
export function TeleCallNotification({
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center bg-black/80 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
    >
      <div
        className={`bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 transform transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'
          }`}
      >
          ...
          <div className="flex flex-col items-center">
            ...
            {customer.photo ? (
              <img
                src={customer.photo}
                alt={customer.name}
                className={`w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg mb-4 ${ringing ? 'animate-pulse' : ''
                  }`}
              />
            ) : (
              <div className={`w-24 h-24 rounded-full bg-white/20 flex items-center justify-center border-4 border-white shadow-lg mb-4 ${ringing ? 'animate-pulse' : ''
                }`}>
