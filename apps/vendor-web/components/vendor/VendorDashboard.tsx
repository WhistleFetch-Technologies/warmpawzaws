...
export function VendorDashboard({ vendorId, phone, vendorType, serviceStyle, onLogout }: VendorDashboardProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-amber-50">
        <div className="text-center p-0 bg-white rounded-2xl shadow-lg max-w-md">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-0">Something went wrong</h2>
          <p className="text-gray-600 mb-4">{error}</p>
          <button
            onClick={loadDashboard}
            className="px-0 py-0 bg-orange-500 text-white rounded-full hover:bg-orange-600 transition"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <header className="bg-gradient-to-r from-orange-500 to-orange-600 text-white sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button className="p-0 hover:bg-white/10 rounded-full">
      ...
      {/* Main Content */}
      <main className="max-w-7xl mx-auto px-4 py-0 space-y-6">
        {/* Stats Grid */}
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              >
                <span className="text-3xl mb-0">{action.icon}</span>
        ...
        <section>
          ...
          {todayBookings.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-0">No bookings today</h3>
        ...
        {stats.pendingEarnings > 0 && (
          <section className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl p-0 text-white">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold">Pending Settlement</h2>
                <p className="text-3xl font-bold mt-0">₹{stats.pendingEarnings.toLocaleString()}</p>
                <p className="text-sm text-green-100 mt-0">Expected within 7 days</p>
              </div>
              <a
                href="/settlements"
                className="px-4 py-0 bg-white text-green-600 rounded-full font-medium hover:bg-green-50 transition"
              >
                View Details
              </a>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg">
        <div className="max-w-7xl mx-auto flex justify-around py-0">
          {[
