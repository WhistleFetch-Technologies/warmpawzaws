...
export function NutritionistMealManager({
  ...
  const loadData = async () => {
    try {
      ...
      // Load products
      const productsRes = await apiClient.get<any>(`/vendor/${vendorId}/meal-products`);
      ...
      // Load orders
      const ordersRes = await apiClient.get<any>(`/vendor/${vendorId}/meal-orders`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let response: any;
      if (editingProduct) {
        response = await apiClient.put<any>(`/vendor/${vendorId}/meal-products/${editingProduct.id}`, formData);
      } else {
        response = await apiClient.post<any>(`/vendor/${vendorId}/meal-products`, formData);
      }
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    try {
      const response = await apiClient.delete<any>(`/vendor/${vendorId}/meal-products/${productId}`);
  ...
  const handleUpdateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      const response = await apiClient.put<any>(`/vendor/${vendorId}/meal-orders/${orderId}/status`, { status: newStatus });
