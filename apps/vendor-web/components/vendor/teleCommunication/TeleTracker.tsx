...
interface TeleTrackerProps {
    hasActiveCall: boolean;
    activeVideoCalls: ActiveVideoCall[];
    joinCall: (call: ActiveVideoCall) => void;
}

export function TeleTracker({ hasActiveCall, activeVideoCalls, joinCall }: TeleTrackerProps) {
        ...
        <div className="fixed bottom-[72px] left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] vendor-modal-sheet">
            <div className="bg-gradient-to-r from-[#FF8C42] to-[#FF6B35] rounded-2xl px-4 py-3 shadow-lg flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/80 text-xs truncate">{call.customerName}</p>
                </div>
                <button
