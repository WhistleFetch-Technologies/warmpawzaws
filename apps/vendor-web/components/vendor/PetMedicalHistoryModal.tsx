        ...
        <div className="bg-white w-full max-w-[430px] rounded-t-[32px] sm:rounded-[32px] max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-gray-200 px-0 py-4 flex items-center justify-between rounded-t-[32px] z-10">
            <button
              ...
              className="flex items-center gap-0 text-gray-600 hover:text-gray-800"
            >
              <span>←</span>
          ...
          <div className="p-0 space-y-6 pb-24">
            <div className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-2xl p-0">
            ...
            {selectedPrescription.vitals && (
              <div className="bg-white border border-gray-200 rounded-2xl p-0 space-y-3">
                <h3 className="font-bold text-gray-800 flex items-center gap-0">
                ...
                <div className="grid grid-cols-2 gap-0">
                  ...
                  {selectedPrescription.vitals.respiratoryRate && (
                    <div className="bg-gray-50 rounded-xl p-0">
                      <p className="text-xs text-gray-600 mb-0">Respiratory Rate</p>
                ...
                {selectedPrescription.vitals.bloodPressure && (
                  <div className="bg-gray-50 rounded-xl p-0">
                    <p className="text-xs text-gray-600 mb-0">Blood Pressure</p>
            ...
            {selectedPrescription.medications && selectedPrescription.medications.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-0 space-y-4">
                <h3 className="font-bold text-gray-800 flex items-center gap-0">
                ...
                {selectedPrescription.medications.map((med, idx) => (
                  <div key={idx} className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-xl p-4">
                    <h4 className="font-semibold text-gray-800 mb-0">{idx + 1}. {med.name}</h4>
                    <div className="space-y-1 text-sm">
                      ...
                      {med.instructions && (
                        <p className="text-gray-700 mt-0 italic">{med.instructions}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            {selectedPrescription.testsRecommended && selectedPrescription.testsRecommended.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl p-0 space-y-3">
                ...
                {selectedPrescription.testsRecommended.map((test, idx) => (
                  <div key={idx} className={`rounded-xl p-0 border-2 ${
                    ...
                    <div className="flex items-center justify-between mb-0">
                      ...
                      <span className={`text-xs px-0 py-0 rounded-full font-semibold ${
          ...
          <div className="sticky bottom-0 bg-white px-0 py-4 flex justify-center">
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
      <div className="bg-white w-full max-w-[430px] rounded-t-[32px] sm:rounded-[32px] max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="p-0 space-y-6 pb-24">
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-2xl p-0">
            ...
            {prescriptions.length === 0 ? (
              <div className="text-center py-0">
                ...
                <p className="text-gray-600 font-semibold mb-0">No Medical History</p>
                  ...
                  >
                    <div className="flex items-start gap-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-800 mb-0">{prescription.serviceName}</h3>
                        <p className="text-sm text-gray-600 mb-0">{prescription.vendorName}</p>
                        ...
                        <div className="mt-0 flex flex-wrap gap-0">
                          {prescription.diagnosis && (
                            <span className="px-0 py-0 bg-red-100 text-red-700 text-xs rounded-full">
                              Diagnosis
                            </span>
                          )}
                          {prescription.medications && prescription.medications.length > 0 && (
                            <span className="px-0 py-0 bg-blue-100 text-blue-700 text-xs rounded-full">
                          ...
                          {prescription.testsRecommended && prescription.testsRecommended.length > 0 && (
                            <span className="px-0 py-0 bg-yellow-100 text-yellow-700 text-xs rounded-full">
                              {prescription.testsRecommended.length} Test{prescription.testsRecommended.length > 1 ? 's' : ''}
                            </span>
                          )}
                          {prescription.vitals && (
                            <span className="px-0 py-0 bg-green-100 text-green-700 text-xs rounded-full">
                              Vitals
                            </span>
                          )}
                        </div>
