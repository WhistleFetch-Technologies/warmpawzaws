...
export function DynamicVendorOnboardingForm({ 
  ...
  const loadSpecializations = async () => {
    try {
      ...
      console.log('[DYNAMIC FORM] Specializations response status:', 200);
      if (response.ok) {
  ...
  const fetchForm = async () => {
    try {
      ...
      const data = await apiClient.get(`/vendor/onboarding-form/${roleId}`) as any;
      ...
      if (data && data.form) {
        ...
        console.error('[DYNAMIC FORM] ❌ Failed to load form');
