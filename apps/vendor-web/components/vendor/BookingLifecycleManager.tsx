...
export function BookingLifecycleManager() {
  ...
  async function loadVendorBookings() {
    try {
      ...
      const data = await apiClient.get(`/vendor/bookings`) as any;
  ...
  async function handleAcceptBooking(booking: Booking) {
    try {
      ...
      await apiClient.post(`/vendor/bookings/${booking.id}/status`, { status: 'confirmed', note: 'Booking accepted by vendor' });
  ...
  async function handleRejectBooking(booking: Booking) {
    ...
    try {
      await apiClient.post(`/vendor/bookings/${booking.id}/status`, { status: 'cancelled', note: `Rejected by vendor: ${reason}` });
  ...
  async function handleStartService(booking: Booking) {
    try {
      await apiClient.post(`/vendor/bookings/${booking.id}/status`, { status: 'in_progress', note: 'Service started' });
  ...
  async function verifyCompletionOTP() {
    ...
    try {
      ...
      const result = await apiClient.post(`/vendor/bookings/${selectedBooking.id}/verify-otp`, { otp: otpInput, action: 'complete' }) as any;
      
      if (result.verified) {
        // Update status to completed
        await apiClient.post(`/vendor/bookings/${selectedBooking.id}/status`, { status: 'completed', note: 'Service completed and verified' });
