...
import {
  ...
  type VendorAddressFromGeolocationResult,
} from '@/lib/address-from-geolocation';

type UseCurrentLocationButtonProps = {
  onSuccess: (result: VendorAddressFromGeolocationResult) => void;
...
export function UseCurrentLocationButton({
  ...
  return (
    <button
      ...
      className={`w-full py-2 px-4 rounded-lg font-medium flex items-center justify-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all ${variantClasses} ${className}`}
    >
      {detecting ? (
        <>
