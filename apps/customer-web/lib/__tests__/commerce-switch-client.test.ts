...
describe('commerce-switch-client (read-only)', () => {
  ...
  it('shouldAcceptCommerceConfig rejects older version', () => {
    const current = {
      ...
      version: 4,
      schemaVersion: '1.0',
      availableModels: ['marketplace', 'warmpawz_pay'] as import('@warmpawz/commerce-switch-contracts').CommerceModelId[],
      updatedAt: '2026-01-03T00:00:00.000Z',
    };
  ...
  it('restores stale session snapshot on hydrate', () => {
    clearCommerceSwitchCache();
    sessionStorage.setItem(
      'warmpawz_commerce_switch_v1',
      JSON.stringify({
        ...
        fetchedAt: Date.now() - COMMERCE_SWITCH_CACHE_TTL_MS - 5_000,
      })
    );
  ...
  it('accepts a newer admin version after TTL', async () => {
    const now = 1_700_000_000_000;
    const dateNow = jest.spyOn(Date, 'now').mockReturnValue(now);
    mockedGet
      .mockResolvedValueOnce({
