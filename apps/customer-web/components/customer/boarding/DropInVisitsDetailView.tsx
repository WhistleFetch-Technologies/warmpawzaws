...
import { DropInVisitRoutineTimeline } from './DropInVisitRoutineTimeline';
...
import { dropInVisitsMetadata } from '@/lib/pet-sitter-service-detail/drop-in-visits-metadata';
...
export interface DropInVisitsDetailViewProps {
  phone: string;
  onBack?: () => void;
...
function WhyPetParentsLoveIt({
  items,
}: {
  items: { title: string; description: string; toneClass: string; icon: typeof Home }[];
}) {
  if (!items.length) return null;
  ...
  return (
    <section className="mb-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-slate-900">Why Pet Parents Love It</h2>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {items.map(({ title, description, toneClass, icon: Icon }) => (
          <div
            key={title}
            className={`rounded-[20px] border border-slate-100 p-4 shadow-sm ${toneClass}`}
          >
            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-2xl bg-white/80 text-[#FF8C42] shadow-sm">
...
function ImportantNotesCard({ title, items }: { title: string; items: string[] }) {
  if (!items.length) return null;
  ...
  return (
    <section className="mb-6 rounded-[20px] border border-slate-100 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-orange-50">
...
export function DropInVisitsDetailView({
  phone,
  onBack,
  onNavigate,
}: DropInVisitsDetailViewProps) {
  const router = useRouter();
  const sittersRef = useRef<HTMLElement>(null);
  const content = dropInVisitsMetadata;

  const useWapptList =
    isWapptHubCategoryEnabled('sitting') &&
    Boolean(onNavigate) &&
    shouldUseWapptPayVendorCardUi('sitting');

  const loadRows = useCallback(() => fetchPetSitterHubRows(phone), [phone]);
  const marketplaceDiscovery = useHubVendorDiscovery(phone, HUB_DISCOVERY_SITTING, loadRows);
  ...
  const listConfig = resolveWapptVendorListConfig('sitting', 'at_home');
  ...
  const sitterCount = useWapptList ? sortedWapptRows.length : sortedMarketplaceVendors.length;
  ...
  const scrollToSitters = useCallback(() => {
    sittersRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);

  const handleViewAllSitters = useCallback(() => {
    if (onNavigate && useWapptList) {
      onNavigate('wappt-discovery', { category: 'sitting' });
      return;
    }
    scrollToSitters();
  }, [onNavigate, scrollToSitters, useWapptList]);

  const openMarketplaceProfile = useCallback(
    (e: MouseEvent, profileKey: string) => {
      e.stopPropagation();
      const v = findBoardingListVendorByProfileKey(marketplaceDiscovery.vendors, profileKey);
      if (!v) return;
    ...
    [marketplaceDiscovery.vendors, onNavigate, router],
  );
  ...
  const loveItCards = [
    {
      ...
      description: 'Choose visits that fit perfectly in your day.',
      toneClass: 'bg-emerald-50/80',
      icon: Calendar,
    },
    {
      title: 'Less Stress for Pets',
      description: 'Pets stay in their familiar environment.',
      toneClass: 'bg-orange-50/80',
      icon: Heart,
    },
    {
      title: 'Reliable & Safe',
      description: 'Background-checked sitters you can trust.',
      toneClass: 'bg-blue-50/70',
      icon: Shield,
    },
  ];
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <DropInVisitRoutineTimeline
            items={content.timeline}
            title={content.timelineTitle}
          />
        </div>

        <WhyPetParentsLoveIt items={loveItCards} />

        <ImportantNotesCard
          title={content.importantNotesTitle ?? 'Important Notes'}
          items={content.importantNotes ?? []}
        />

        <section id="sitters-section" ref={sittersRef} className="mb-6 scroll-mt-4">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-slate-900">Available Sitters Near You</h2>
              </div>
              {!loading ? (
                <p className="text-sm text-slate-500">
                  {useWapptList
                    ? listConfig.resultsCountLabel(sitterCount)
                    : `${sitterCount} sitter${sitterCount === 1 ? '' : 's'} found`}
                </p>
              ) : null}
            </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mt-4 text-sm text-gray-500">
                {useWapptList ? listConfig.loadingMessage : 'Finding sitters...'}
              </p>
            </div>
          ) : sitterCount === 0 ? (
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-12 text-center">
              ...
              <p className="font-semibold text-gray-800">
                {useWapptList ? listConfig.emptyTitle : 'No sitters found'}
              </p>
            ...
            <div className="space-y-4">
              {useWapptList
                ...
                : sortedMarketplaceVendors.map((v) => (
                    <ServiceHubVendorCard
                      ...
                      category="sitting"
                      serviceKey="pet_sitting"
                      categoryLabelFallback="Pet Sitter"
              ...
              <button
                type="button"
                onClick={handleViewAllSitters}
                className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-[#FF8C42] bg-white py-3 text-sm font-bold text-[#FF8C42] transition-colors hover:bg-orange-50"
              >
                View All Sitters
