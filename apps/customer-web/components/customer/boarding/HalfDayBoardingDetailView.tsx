...
import { HalfDayBoardingRoutineTimeline } from './HalfDayBoardingRoutineTimeline';
...
import { halfDayBoardingMetadata } from '@/lib/boarding-service-detail/half-day-metadata';
...
export interface HalfDayBoardingDetailViewProps {
  phone: string;
  onBack?: () => void;
...
export function HalfDayBoardingDetailView({
  phone,
  onBack,
  onNavigate,
}: HalfDayBoardingDetailViewProps) {
  const router = useRouter();
  const facilitiesRef = useRef<HTMLElement>(null);
  const content = halfDayBoardingMetadata;
  const serviceSlug = 'half-day' as const;
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <HalfDayBoardingRoutineTimeline
      ...
      <div className="fixed bottom-[4.5rem] left-0 right-0 z-30 mx-auto max-w-customer px-4 pb-2">
        ...
        >
          Find Half Day Boarding Facilities
        </button>
      </div>
