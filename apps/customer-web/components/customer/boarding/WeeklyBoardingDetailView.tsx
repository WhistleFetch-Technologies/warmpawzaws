...
import { WeeklyBoardingRoutineTimeline } from './WeeklyBoardingRoutineTimeline';
...
import { weeklyBoardingMetadata } from '@/lib/boarding-service-detail/weekly-metadata';
...
export interface WeeklyBoardingDetailViewProps {
  phone: string;
  onBack?: () => void;
...
export function WeeklyBoardingDetailView({
  phone,
  onBack,
  onNavigate,
}: WeeklyBoardingDetailViewProps) {
  const router = useRouter();
  const facilitiesRef = useRef<HTMLElement>(null);
  const content = weeklyBoardingMetadata;
  const serviceSlug = 'weekly' as const;
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      ...
      <div className="fixed bottom-[4.5rem] left-0 right-0 z-30 mx-auto max-w-customer px-4 pb-2">
        ...
        >
          Find Weekly Boarding Facilities
        </button>
      </div>
