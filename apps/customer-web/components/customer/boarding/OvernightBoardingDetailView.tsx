...
import { OvernightBoardingRoutineTimeline } from './OvernightBoardingRoutineTimeline';
...
import { overnightBoardingMetadata } from '@/lib/boarding-service-detail/overnight-metadata';
...
export interface OvernightBoardingDetailViewProps {
  phone: string;
  onBack?: () => void;
...
export function OvernightBoardingDetailView({
  phone,
  onBack,
  onNavigate,
}: OvernightBoardingDetailViewProps) {
  const router = useRouter();
  const facilitiesRef = useRef<HTMLElement>(null);
  const content = overnightBoardingMetadata;
  const serviceSlug = 'overnight' as const;
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <OvernightBoardingRoutineTimeline
      ...
      <div className="fixed bottom-[4.5rem] left-0 right-0 z-30 mx-auto max-w-customer px-4 pb-2">
        ...
        >
          Find Overnight Boarding Facilities
        </button>
      </div>
