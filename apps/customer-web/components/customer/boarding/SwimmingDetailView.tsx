...
import { SwimmingSessionRoutineTimeline } from './SwimmingSessionRoutineTimeline';
...
import { swimmingMetadata } from '@/lib/boarding-service-detail/swimming-metadata';
...
export interface SwimmingDetailViewProps {
  phone: string;
  onBack?: () => void;
...
export function SwimmingDetailView({
  phone,
  onBack,
  onNavigate,
}: SwimmingDetailViewProps) {
  const router = useRouter();
  const facilitiesRef = useRef<HTMLElement>(null);
  const content = swimmingMetadata;
  const serviceSlug = 'swimming' as const;
  ...
  const facilitiesCountLabel = (count: number) =>
    `${count} ${count === 1 ? 'facility' : 'facilities'} found`;
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <SwimmingSessionRoutineTimeline
        ...
        <section
          id="swimming-facilities-section"
          ref={facilitiesRef}
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-slate-900">Available Swimming Facilities</h2>
              </div>
              {!loading ? (
                <p className="text-sm text-slate-500">
                  {useWapptList
                    ? facilitiesCountLabel(vendorCount)
                    : facilitiesCountLabel(vendorCount)}
                </p>
              ) : null}
            </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mt-4 text-sm text-gray-500">
                {useWapptList ? listConfig.loadingMessage : 'Finding swimming facilities...'}
              </p>
            </div>
          ) : vendorCount === 0 ? (
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-12 text-center">
              <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FFF5EE] text-2xl">
                <Waves className="h-7 w-7 text-[#FF8C42]" />
              </div>
              <p className="font-semibold text-gray-800">
                {useWapptList ? listConfig.emptyTitle : 'No swimming facilities found'}
              </p>
