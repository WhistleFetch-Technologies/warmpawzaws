...
import { ExtendedStayRoutineTimeline } from './ExtendedStayRoutineTimeline';
...
import { extendedStayMetadata } from '@/lib/pet-sitter-service-detail/extended-stay-metadata';
...
export interface ExtendedStayDetailViewProps {
  phone: string;
  onBack?: () => void;
  ...
  return (
    <section className="mb-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-slate-900">What Makes It Special</h2>
      </div>
...
export function ExtendedStayDetailView({
  phone,
  onBack,
  onNavigate,
}: ExtendedStayDetailViewProps) {
  const router = useRouter();
  const sittersRef = useRef<HTMLElement>(null);
  const content = extendedStayMetadata;
  ...
  const specialCards = [
    {
      ...
      description: 'Your pet follows their usual daily schedule.',
      toneClass: 'bg-emerald-50/80',
      icon: Home,
    },
    {
      title: 'Home Environment',
      description: 'No stress of new places or unfamiliar people.',
      toneClass: 'bg-orange-50/80',
      icon: Heart,
    },
    {
      title: 'Complete Care',
      description: "From meals to cuddles, we've got it covered.",
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <ExtendedStayRoutineTimeline
            items={content.timeline}
            title={content.timelineTitle}
          />
        </div>

        <WhatMakesItSpecial items={specialCards} />
