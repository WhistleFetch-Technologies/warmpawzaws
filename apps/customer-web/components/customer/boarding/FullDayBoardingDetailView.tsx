...
import { FullDayBoardingTypicalDayTimeline } from './FullDayBoardingTypicalDayTimeline';
...
import { fullDayBoardingMetadata } from '@/lib/boarding-service-detail/full-day-metadata';
...
export interface FullDayBoardingDetailViewProps {
  phone: string;
  onBack?: () => void;
...
export function FullDayBoardingDetailView({
  phone,
  onBack,
  onNavigate,
}: FullDayBoardingDetailViewProps) {
  const router = useRouter();
  const facilitiesRef = useRef<HTMLElement>(null);
  const content = fullDayBoardingMetadata;
  ...
  const marketplaceDiscovery = useBoardingVendorDiscovery(phone, 'full-day');
  ...
  const openMarketplaceProfile = useCallback(
    (e: MouseEvent, vendorId: string) => {
      e.stopPropagation();
      if (onNavigate) {
        onNavigate('pet-boarding-profile', { vendorId, serviceSlug: 'full-day' });
        return;
      }
      router.push(
        `/pet-boarding/vendor/${encodeURIComponent(vendorId)}?service=${encodeURIComponent('full-day')}`,
      );
    },
    [onNavigate, router],
  );
  ...
  return (
    <div className="mx-auto flex min-h-screen min-h-[100dvh] w-full max-w-customer flex-col overflow-x-hidden bg-white">
      <div className="flex-1 px-4 pb-36 pt-4">
        ...
        <div className="mb-6">
          <FullDayBoardingTypicalDayTimeline
        ...
        >
          ...
          {relaxedFilter && vendorCount > 0 ? (
            <p className="mb-3 rounded-xl border border-amber-100 bg-amber-50 px-3 py-2 text-sm text-amber-800">
              No centers listed <strong>{BOARDING_SERVICE_LABELS['full-day']}</strong> by service
              ...
              {useWapptList
                ...
                : sortedMarketplaceVendors.map((v) => (
                    <ServiceHubVendorCard
                      ...
                      vendor={v}
                      category="boarding"
                      categoryLabelFallback={BOARDING_SERVICE_LABELS['full-day']}
      ...
      <div className="fixed bottom-[4.5rem] left-0 right-0 z-30 mx-auto max-w-customer px-4 pb-2">
        ...
        >
          Find Full Day Boarding Facilities
        </button>
      </div>
