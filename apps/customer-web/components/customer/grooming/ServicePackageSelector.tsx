...
interface ServicePackageSelectorProps {
  ...
  onPackageSelect?: (packageId: string, packageData: any) => void;
}
...
export function ServicePackageSelector({ 
  ...
  const toggleService = (service: any) => {
    ...
    if (onPackageSelect) {
      onPackageSelect(service.id || service.serviceId, service);
    }
  };
  ...
  const handleContinue = () => {
    if (selectedServices.length > 0 && onSelect) {
  ...
  return (
    <div className="min-h-screen bg-[#FF8C42] max-w-md mx-auto pb-32">
      ...
      {/* Fixed Bottom Bar */}
      {selectedServices.length > 0 && onSelect && (
