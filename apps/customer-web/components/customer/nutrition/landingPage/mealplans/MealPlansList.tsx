...
import { ArrowLeft, Star, MapPin, UtensilsCrossed, Calendar } from 'lucide-react';
...
export function MealPlansList({ phone, onBack, onNavigate }: MealPlansListProps) {
  ...
  const fetchMealPlans = async () => {
    try {
      setLoading(true);
      const data = await apiClient.get<{ success?: boolean; mealPlans?: any[] }>('/discover/meal-plans') as any;
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {mealPlans.map((mealPlan: any, index: number) => {
                ...
                const price = mealPlan.price || mealPlan.monthly_price || 0;
                const duration = mealPlan.duration_days || mealPlan.duration || '30';
                const vendorRating = mealPlan.vendor_rating || mealPlan.vendorRating;
