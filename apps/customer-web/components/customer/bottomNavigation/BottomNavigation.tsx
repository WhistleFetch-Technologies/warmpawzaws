...
function ProfileTabButton({
  ...
  return (
    <button type="button" onClick={onClick} className="flex flex-col items-center gap-0.5">
      ...
      >
        <User
          className={`h-6 w-6 ${active ? 'text-[#FF8C42]' : 'text-gray-400'}`}
          strokeWidth={active ? 2.25 : 2}
        />
      </div>
      <span className={`text-xs ${active ? 'font-semibold text-[#FF8C42]' : 'text-gray-400'}`}>Profile</span>
    </button>
  );
}

function ScanToPayFab({
  active,
  onClick,
}: {
  active: boolean;
    ...
    >
      <div
        className={`flex h-14 w-14 items-center justify-center rounded-full shadow-lg ${
          active ? 'bg-[#FF6B00] ring-4 ring-orange-100' : 'bg-[#FF8C42]'
        }`}
      >
        <QrCode className="h-7 w-7 text-white" />
      </div>
      <span className={`text-[10px] font-semibold ${active ? 'text-[#FF6B00]' : 'text-[#FF8C42]'}`}>
        SCAN TO PAY
      </span>
    </button>
  );
...
export function BottomNavigation({
  ...
  const wpayEnabled =
    commerce?.isWarmpawzPay === true &&
    commerce.isLoaded &&
    isWarmpawzPayModuleCapable();
  ...
  const profileTab = (
    <ProfileTabButton active={profileActive} onClick={() => handleNavClick('profile')} />
  );
  ...
  return (
    <div className="cw-customer-tabbar-fixed fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-customer border-t border-gray-200 bg-white">
      {wpayEnabled ? (
        ...
        <div className="grid grid-cols-4 items-end px-1 pb-2 pt-2 sm:px-2">
          {homeTab}
          {shopTab}
          {bookingsTab}
          {profileTab}
