...
async function storeRefreshedTokenIfPresent(res: unknown): Promise<void> {
  const t = (res as any)?.data?.token;
  if (!t || typeof t !== 'object') return;
  const accessToken = t.access_token || t.accessToken;
  const idToken = t.id_token || t.idToken;
  const refreshToken = t.refresh_token || t.refreshToken;
  const expiresIn = t.expires_in || t.expiresIn || 86400;
  if (!accessToken) return;
  const { storeCognitoTokens } = await import('@/lib/cognito-auth');
  storeCognitoTokens(
    { accessToken, idToken: idToken || accessToken, refreshToken: refreshToken || '', expiresIn },
    { isNewLogin: false }
  );
...
export default function SetPasswordPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const postChangePassword = async (): Promise<unknown> => {
        if (phone.length < 10) {
          throw new ApiError(
      ...
      const postShallowSetPassword = async (): Promise<unknown> => {
        return apiClient.post('/customer/set-password', { password: p, confirmPassword: c }, noRetry);
      };

      const postProfile = async (): Promise<unknown> => {
        return apiClient.post('/customer/profile/set-password', { password: p, confirmPassword: c }, noRetry);
      };

      const postAccount = async (): Promise<unknown> => {
        return apiClient.post('/customer/account/password', { password: p, confirmPassword: c }, noRetry);
      };

      const postAuthCustomer = async (): Promise<unknown> => {
        return apiClient.post('/auth/customer/set-password', { password: p, confirmPassword: c }, noRetry);
      };
      ...
      for (const step of chain) {
        try {
          ...
          await storeRefreshedTokenIfPresent(result);
          done = true;
          break;
        } catch (e) {
          ...
          <div className="bg-white rounded-t-[2.5rem] min-h-full px-6 pt-8 pb-8 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
            ...
            {authExpired && (
              <button
                ...
                className="mb-4 w-full py-3 bg-gray-900 text-white text-sm font-semibold rounded-2xl"
              >
                Log in again
              </button>
            )}
            ...
            <button
              type="button"
              onClick={submit}
              disabled={loading || (hasExistingPassword && currentPassword.trim().length < 1)}
