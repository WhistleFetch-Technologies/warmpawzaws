...
import { TrainingBookingRouter } from '@/components/customer/training/TrainingBookingRouter';
import {
  SEARCH_TRAINING_BOOKING_INTENT_KEY,
  type SearchTrainingBookingIntent,
} from '@/lib/search-booking-launch';

function readTrainingBookingIntent(): SearchTrainingBookingIntent | null {
  if (typeof window === 'undefined') return null;
  try {
    const raw = sessionStorage.getItem(SEARCH_TRAINING_BOOKING_INTENT_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as SearchTrainingBookingIntent;
  } catch {
    return null;
  }
}

function BookingTrainingContent() {
  const router = useRouter();
  const [phone, setPhone] = useState<string | null>(null);
  const [intent, setIntent] = useState<SearchTrainingBookingIntent | null>(null);

  useEffect(() => {
    setPhone(localStorage.getItem('customerPhone'));
    setIntent(readTrainingBookingIntent());
  }, []);

  const returnUrl = intent?.returnSearchUrl || '/search?category=training';
  ...
  if (phone === null || intent === null) {
    ...
    return (
      <div className="mx-auto flex min-h-[100dvh] max-w-md flex-col items-center justify-center bg-stone-100 px-6 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">Pick a trainer from search to start booking.</p>
        <Link
  ...
  return (
    <TrainingBookingRouter
      ...
      serviceType={intent.serviceType || 'training'}
...
export function BookingTrainingPageClient() {
  return (
    <Suspense
      fallback={
