...
export function UserAccountSidebar({ phone, onClose, onViewBooking, onViewCustomerProfile, onViewAppointments, onViewWallet, onNavigate }: UserAccountSidebarProps) {
  ...
  const loadCart = async () => {
    try {
      ...
      // Get cart from MockAPI
      const cart = await MockAPI.ecommerce.getCart(`cust_${phone}`);
  ...
  const updateCartQuantity = async (itemId: string, quantity: number) => {
    try {
      // Update cart in MockAPI
      await MockAPI.ecommerce.updateCartItem(`cust_${phone}`, itemId, quantity);
      await loadCart();
    } catch (error) {
  ...
  const removeFromCart = async (itemId: string) => {
    try {
      // Remove from cart in MockAPI
      await MockAPI.ecommerce.removeFromCart(`cust_${phone}`, itemId);
      await loadCart();
    } catch (error) {
  ...
  const loadSaved = async () => {
    try {
      ...
      // Get saved items from MockAPI
      const saved = await MockAPI.ecommerce.getSavedItems(`cust_${phone}`);
      setSavedItems(saved || []);
    } catch (error) {
