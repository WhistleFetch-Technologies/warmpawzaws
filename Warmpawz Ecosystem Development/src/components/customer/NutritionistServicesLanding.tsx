...
export function NutritionistServicesLanding({ onBack, onNavigate, customerId, phone }: NutritionistServicesLandingProps) {
  ...
  const loadNutritionists = async () => {
    try {
      setLoading(true);
      const results = await MockAPI.search.searchVendors({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white sticky top-0 z-50 px-4 py-4">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-lg font-bold">Pet Nutritionists</h1>
            <p className="text-xs text-white/80">Expert Diet Plans</p>
          </div>
        </div>
      </div>
