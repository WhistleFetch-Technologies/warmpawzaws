  ...
  return (
    <div className="min-h-screen bg-gray-50 w-full max-w-[430px] mx-auto">
      ...
      <div className="bg-white rounded-t-[32px] -mt-6 pt-6 pb-24">
        ...
        {/* ✅ NEW: Previous Providers Section */}
        {previousProviders.length > 0 && (
          <div className="px-6 mb-6">
            <PreviousProvidersCarousel
              customerId={phone}
              onBookProvider={(provider) => {
                console.log('Rebook provider:', provider);
                // Navigate to provider profile or booking flow
                onNavigate('vet'); // Simplified for now
              }}
            />
          </div>
        )}

        {/* ✅ NEW: Radar Map Section (For Home Services) */}
        <div className="px-6 mb-6">
          <div className="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-lg">
            <div className="p-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-500">Live view of available staff</p>
            </div>
            <div className="p-4">
              <RadarProviderMap
                userLocation={{ lat: 12.9716, lng: 77.5946 }} // Mock user location (Bangalore)
                radius={5}
                onSelectProvider={(provider) => {
                  console.log('Selected radar provider:', provider);
                  // Open provider quick view
                }}
              />
            </div>
          </div>
