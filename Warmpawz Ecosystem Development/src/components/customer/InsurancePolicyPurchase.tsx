...
export function InsurancePolicyPurchase({ 
  ...
  const fetchInsurancePlans = async () => {
    try {
      // Get insurance plans from MockAPI
      const mockPlans = await MockAPI.integratedServices.getInsurancePlans();
      setPlans(mockPlans);
    } catch (error) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // Purchase insurance policy via MockAPI
      const policy = await MockAPI.integratedServices.purchaseInsurance({
        customerId,
        petId,
        planId: selectedPlan?.id || 'basic',
        documents: {
          vaccinationCard: vaccinationCard.name,
          petPhoto: petPhoto.name,
          medicalRecords: medicalRecords?.name || null
        }
      });
