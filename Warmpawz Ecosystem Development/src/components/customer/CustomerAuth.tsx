...
export function CustomerAuth({ onAuthSuccess }: CustomerAuthProps) {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate if phone starts with valid Indian mobile prefix (6, 7, 8, or 9)
      ...
      // Use MockAPI to generate OTP
      const response = await MockAPI.auth.generateOTP(cleanPhone);
      ...
      // In mock mode, always show the OTP for testing
      toast.success(`OTP: ${response.otp} (Mock Testing Mode)`, { duration: 5000 });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use MockAPI to verify OTP
      const response = await MockAPI.auth.verifyOTP(cleanPhone, otpCode);
      ...
      if (referralCode && response.isNewUser) {
        try {
          console.log('🎁 Applying referral code:', referralCode);
          const referralResponse = await MockAPI.loyalty.applyReferral(
            referralCode,
          ...
          if (referralResponse.success) {
            console.log('✅ Referral code applied:', referralResponse);
            toast.success('🎉 Referral code applied! You\'ll earn bonus points!', { duration: 4000 });
          } else {
            console.log('⚠️ Referral code failed:', referralResponse.error);
            toast.error(referralResponse.error || 'Invalid referral code', { duration: 3000 });
          }
        } catch (refError: any) {
      ...
      const hasCompletedOnboarding = response.customer.onboardingComplete;
      const hasPets = response.customer.petIds && response.customer.petIds.length > 0;
