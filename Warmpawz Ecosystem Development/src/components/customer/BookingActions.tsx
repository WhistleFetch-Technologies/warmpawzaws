...
export function BookingActions({ booking, phone, onSuccess }: BookingActionsProps) {
  ...
  const fetchRefundPreview = async () => {
    try {
      // Mock refund calculation based on booking date
      ...
      if (hoursUntilBooking < 12) {
        refundPercentage = 0;
        refundPolicy = 'No refund - Cancellation within 12 hours of booking';
      } else if (hoursUntilBooking < 24) {
        refundPercentage = 50;
        refundPolicy = '50% refund - Cancellation within 24 hours';
      } else if (hoursUntilBooking < 48) {
        refundPercentage = 75;
        refundPolicy = '75% refund - Cancellation within 48 hours';
      } else {
        refundPercentage = 100;
        refundPolicy = 'Full refund - Cancellation 48+ hours in advance';
      }
      
      const bookingAmount = booking.price || booking.amount || 0;
      const refundAmount = Math.floor((bookingAmount * refundPercentage) / 100);
  ...
  const handleReschedule = async () => {
    ...
    setLoading(true);
    try {
      // Use MockAPI to reschedule booking
      const updatedBooking = await MockAPI.booking.updateBooking(booking.id, {
        startDate: rescheduleData.newDate,
        date: rescheduleData.newDate,
        timeSlot: rescheduleData.newTimeSlot || booking.timeSlot,
        notes: rescheduleData.reason || booking.notes
      });
  ...
  const handleCancel = async () => {
    ...
    setLoading(true);
    try {
      // Use MockAPI to cancel booking
      await MockAPI.booking.cancelBooking(booking.id, cancellationReason);
      
      const message = refundPreview?.refundAmount > 0
        ? `Booking cancelled successfully. ₹${refundPreview.refundAmount} will be refunded within 5-7 business days.`
