...
const MapPlaceholder = () => (
    <div className="w-full h-full bg-gray-100 flex items-center justify-center relative overflow-hidden rounded-xl">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-red-600 rounded-full border-2 border-white shadow-lg"></div>
    </div>
);
...
export function AmbulanceBookingFlow({ phone, customerId, onBack, onSuccess }: AmbulanceBookingProps) {
  ...
  const handleBooking = async () => {
      try {
          ...
          // Create ambulance booking via MockAPI
          const booking = await MockAPI.integratedServices.bookAmbulance({
              customerId,
              phone,
              type: ambulanceType,
          ...
          setAssignedDriver(booking.driver);
          setEta(booking.eta || '12 mins');
          setStep('tracking');
  ...
  const RenderTypeSelection = () => (
      <div className="space-y-4">
          ...
          >
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center border shadow-sm text-2xl">🚑</div>
          ...
          >
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center border shadow-sm text-2xl">🌬️</div>
              <div className="flex-1">
                  <div className="flex justify-between">
          ...
          <Button className="w-full bg-red-600 hover:bg-red-700 text-white h-12 mt-4" onClick={() => setStep('location')}>
  ...
  const RenderTracking = () => (
      <div className="h-full flex flex-col">
          {/* Map Area */}
          <div className="flex-1 bg-gray-100 relative min-h-[300px] rounded-xl overflow-hidden mb-4">
              <MapPlaceholder />
              <div className="absolute bottom-4 left-4 right-4 bg-white p-4 rounded-xl shadow-lg">
                  <div className="flex justify-between items-center mb-2">
                      ...
                      <Badge className="bg-green-100 text-green-700 hover:bg-green-100">On the way</Badge>
                  </div>
          ...
          <Card className="p-4 mb-4">
              <div className="flex items-center gap-4">
                  ...
                  <a href={`tel:${assignedDriver?.phone || "100"}`} className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
                      <Phone className="w-5 h-5 text-green-600" />
                  </a>
              </div>
          </Card>

          <Button variant="outline" className="w-full border-red-200 text-red-600 hover:bg-red-50" onClick={onBack}>
              Return to Home
          </Button>
      </div>
  );
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="p-4 border-b sticky top-0 bg-white z-10 flex items-center gap-3">
