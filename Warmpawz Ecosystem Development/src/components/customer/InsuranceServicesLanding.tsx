...
export function InsuranceServicesLanding({ onBack, onNavigate, customerId, phone }: InsuranceServicesLandingProps) {
  ...
  const loadInsuranceData = async () => {
    try {
      ...
      // Fetch insurance services from MockAPI
      const insuranceServices = await MockAPI.search.searchServices({
        roleId: 'pet_insurance',
        limit: 50
      });

      console.log('✅ [INSURANCE] Loaded services:', insuranceServices);
      ...
      insuranceServices.forEach((service: any) => {
        ...
        if (!vendorMap.has(vendorId)) {
          vendorMap.set(vendorId, {
            ...
            completedPolicies: service.reviewCount || 0,
