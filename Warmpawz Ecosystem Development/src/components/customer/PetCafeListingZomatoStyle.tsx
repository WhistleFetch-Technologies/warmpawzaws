...
export function PetCafeListingZomatoStyle({ cafeId, onBack }: PetCafeListingZomatoStyleProps) {
  ...
  const loadCafeDetails = async () => {
    try {
      ...
      const vendor = await MockAPI.vendor.getProfile(cafeId);
      const { menu } = await MockAPI.integratedServices.getCafeDetails(cafeId);

      if (vendor) {
        // Transform vendor data to CafeDetails format
        const profile: CafeDetails = {
          id: vendor.id,
          name: vendor.business_name || vendor.name,
          description: "A cozy place for you and your pet.",
          address: vendor.location?.address || "Location",
          rating: vendor.rating || 4.5,
          reviewsCount: vendor.reviews_count || 0,
          costForTwo: 500, // Mock default
