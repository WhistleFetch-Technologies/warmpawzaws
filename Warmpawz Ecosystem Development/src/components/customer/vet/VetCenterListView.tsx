...
export function VetCenterListView({ phone, onBack, onNavigate }: VetCenterListViewProps) {
  ...
  const loadVetCenters = async () => {
    try {
      console.log('🏥 [VET-LIST] Loading vet centers...');
      
      const response = await fetch(
        `${API_BASE}/vendors/by-role/vet`,
        {
      ...
      if (response.ok) {
        ...
        console.log('✅ [VET-LIST] Loaded centers:', data.vendors?.length || 0);
        setCenters(data.vendors || []);
      }
    } catch (error) {
      console.error('❌ [VET-LIST] Error loading centers:', error);
    } finally {
      setLoading(false);
    }
  };
          ...
          filteredCenters.map((center) => (
            ...
            >
              <div className="flex gap-3">
                <ImageWithFallback
                  src={center.logo || '/placeholder-vet.png'}
