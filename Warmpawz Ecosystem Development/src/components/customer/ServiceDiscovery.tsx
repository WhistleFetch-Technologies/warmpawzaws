...
export function ServiceDiscovery({ onSelectVendor }: ServiceDiscoveryProps) {
  ...
  const searchVendors = async () => {
    try {
      ...
      // Use MockAPI to search vendors
      const results = await MockAPI.search.searchVendors({
        role: selectedCategory || undefined,
        rating_min: filters.minRating ? parseFloat(filters.minRating) : undefined
      });
      ...
      if (filters.sortBy === 'rating') {
        ...
        sortedResults.sort((a, b) => (b.reviews_count || 0) - (a.reviews_count || 0));
      }

      setVendors(sortedResults);
    } catch (error) {
