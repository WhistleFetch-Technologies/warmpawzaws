...
import { ArrowLeft, MapPin, Star, Clock, Award, Home, Shield } from 'lucide-react';
import { getApiBaseUrl, getAuthHeaders } from '../../../utils/api-config';
import { ImageWithFallback } from '../../figma/ImageWithFallback';

interface BoardingCenterProfileViewProps {
  phone: string;
  centerId: string;
  onBack: () => void;
...
export function BoardingCenterProfileView({ phone, centerId, onBack, onNavigate }: BoardingCenterProfileViewProps) {
  ...
  if (!center) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center max-w-md mx-auto">
        <p className="text-gray-500">Boarding facility not found</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 max-w-md mx-auto pb-24">
      <div className="bg-white sticky top-0 z-10 border-b border-gray-200">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="font-semibold">Facility Details</h1>
        </div>
      </div>
      ...
      <div className="bg-white p-4 border-b border-gray-200">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-orange-50 rounded-lg p-3 text-center">
            <Home className="w-5 h-5 text-[#FF8C42] mx-auto mb-1" />
            <p className="text-xs text-gray-600">Capacity</p>
            <p className="font-semibold text-sm">{center.capacity || '20'} pets</p>
          </div>
          <div className="bg-orange-50 rounded-lg p-3 text-center">
            <Shield className="w-5 h-5 text-[#FF8C42] mx-auto mb-1" />
          ...
          <div className="bg-orange-50 rounded-lg p-3 text-center">
            ...
            <p className="font-semibold text-sm">{center.rating || '4.8'}/5</p>
          </div>
        </div>
      </div>
      ...
      {center.services && center.services.length > 0 && (
        <div className="bg-white mt-2 p-4">
          <h3 className="font-semibold mb-3">Boarding Packages</h3>
