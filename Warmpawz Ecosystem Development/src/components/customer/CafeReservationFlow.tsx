...
export function CafeReservationFlow({ phone, onBack, preSelectedVendorId }: CafeReservationFlowProps) {
  ...
  const loadCafes = async () => {
    try {
      ...
      // Get pet cafes from MockAPI
      const results = await MockAPI.search.searchServices({
        roleId: 'pet_cafe',
        limit: 20
      });
  ...
  const loadCafeDetails = async (vendorId: string) => {
    try {
      ...
      // Get vendor services (tables/packages)
      const services = await MockAPI.search.searchServices({
        vendorId,
        limit: 20
      });
      ...
      if (!selectedCafe && services && services.length > 0) {
  ...
  const handleCreateReservation = async () => {
    ...
    try {
      ...
      // Create cafe reservation via MockAPI
      const booking = await MockAPI.booking.createBooking({
        ...
        scheduledDate: date,
        scheduledTime: time,
        notes: `Cafe Reservation: ${guestCount} Pax, ${petCount} Pets. ${specialRequest ? 'Note: ' + specialRequest : ''}`,
