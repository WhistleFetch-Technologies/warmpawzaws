...
export function RateChangesTab() {
  ...
  const handleExport = () => {
    const csv = [
      ['Request ID', 'Business Name', 'Service', 'Current Rate', 'Proposed Rate', 'Change %', 'Status', 'Type'],
      ...requests.map(r => [
        r.id,
        r.businessName,
        r.service,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Review and approve custom services, packages, and rate changes from vendors
          </p>
        </div>
        <Button 
            ...
            {requests.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-6 py-12 text-center">
                  <div className="text-gray-400">
                    ...
                    <p className="text-gray-600 font-medium">No pending rate changes</p>
                    <p className="text-sm text-gray-500 mt-1">
              ...
              requests.map((request) => (
                <tr key={request.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-500">#{request.id.substring(0, 20)}</div>
                      ...
                      <div className="text-gray-900">{request.businessName}</div>
                      {request.description && (
                  ...
                  <td className="px-6 py-4">
                    <div className="text-gray-900">
                      {request.type === 'custom_service' ? '—' : `₹${request.currentRate}`}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className={request.type === 'custom_service' ? 'text-orange-600 font-medium' : 'text-green-600'}>
          ...
          {selectedRequest && (
            <>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 mt-1">{selectedRequest.businessName}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Vendor ID:</span>
                      <p className="font-medium text-gray-900 mt-1">{selectedRequest.vendorId}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Service Style:</span>
                      <p className="font-medium text-gray-900 mt-1 capitalize">
                        {selectedRequest.serviceStyle?.replace('_', ' ') || 'N/A'}
                      </p>
                    </div>
                    <div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 mt-1">{selectedRequest.service}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium text-gray-900 mt-1">{selectedRequest.categoryName || 'N/A'}</p>
                      </div>
                      {selectedRequest.subCategoryName && (
                        <div>
                          <span className="text-gray-600">Sub-Category:</span>
                          <p className="font-medium text-gray-900 mt-1">{selectedRequest.subCategoryName}</p>
                        </div>
                      )}
                    </div>
                    {selectedRequest.duration && (
                      <div>
                        ...
                        <p className="font-medium text-gray-900 mt-1">{selectedRequest.duration} minutes</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-xl font-medium text-gray-900 mt-1">
                        {selectedRequest.type === 'custom_service' ? '—' : `₹${selectedRequest.currentRate}`}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className={`text-xl font-medium mt-1 ${
                        selectedRequest.type === 'custom_service' ? 'text-orange-600' : 'text-green-600'
                      }`}>
                ...
                {/* Reason */}
                <div>
                  <span className="text-sm text-gray-600">Reason for Change:</span>
                  <p className="text-gray-900 mt-1 bg-gray-50 p-3 rounded-lg">{selectedRequest.reason}</p>
                </div>

                {/* Package Details */}
                {selectedRequest.isPackage && selectedRequest.packageDetails && (
          ...
          {selectedRequest && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  You're approving "{selectedRequest.service}" from {selectedRequest.businessName}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-sm text-gray-700">
                    This service will be set to <span className="font-semibold text-green-700">"live"</span> status
                    and will be <span className="font-semibold">immediately visible to customers</span>.
                  </p>
                  <p className="text-sm text-green-700 font-medium mt-2">
                    Rate: ₹{selectedRequest.proposedRate}
                    {selectedRequest.duration && ` • ${selectedRequest.duration} min`}
                  </p>
                </div>
          ...
          {selectedRequest && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  You're rejecting "{selectedRequest.service}" from {selectedRequest.businessName}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="text-sm text-gray-700">
                    This service will be set to <span className="font-semibold text-red-700">"rejected"</span> status.
                  </p>
                  <p className="text-sm text-red-700 font-medium mt-2">
                    • Service will be DISABLED in vendor app<br />
                    • Will NOT be visible to customers<br />
                    • Vendor will receive notification with your reason
                  </p>
                </div>
          ...
          {selectedRequest && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Ask "{selectedRequest.businessName}" for more details about "{selectedRequest.service}"
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <p className="text-sm text-gray-700">
                    This service will be set to <span className="font-semibold text-orange-700">"needs_clarification"</span> status.
                  </p>
                  <p className="text-sm text-orange-700 font-medium mt-2">
                    • Vendor can edit and provide more details<br />
                    • Service will NOT be visible to customers<br />
                    • Returns to pending after vendor resubmits
                  </p>
                </div>
