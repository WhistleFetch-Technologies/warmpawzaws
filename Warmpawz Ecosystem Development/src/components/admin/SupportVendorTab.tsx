...
export function SupportVendorTab() {
  ...
  const loadSupportTickets = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3dd53475/admin/vendors/support/tickets`,
        {
          headers: {
  ...
  return (
    <div>
      <div className="mb-4">
        <div className="text-sm text-gray-600 mb-4">Current Vendor's who need support</div>
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          filteredTickets.map((ticket) => (
            <div key={ticket.id} className="grid grid-cols-12 gap-4 px-4 py-4 bg-white border border-gray-200 rounded-lg items-center hover:bg-gray-50">
              ...
              <div className="col-span-2">
                <span className={`inline-block px-3 py-1 text-xs rounded-full border ${getPriorityColor(ticket.priority)}`}>
