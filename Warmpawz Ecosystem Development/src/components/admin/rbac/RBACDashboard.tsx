...
export function RBACDashboard({ onBack }: RBACDashboardProps) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-semibold">Role & User Management</h1>
                <p className="text-sm text-gray-500">Manage roles, permissions, and access policies</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Button onClick={() => setShowRoleDialog(true)} className="bg-[#FF8C42] hover:bg-[#ff7a28]">
            ...
            <div className="space-y-6">
              {Object.entries(permissionsByCategory).map(([category, perms]: [string, any]) => (
                <Card key={category} className="p-6">
                  <h3 className="text-lg font-semibold mb-4 capitalize">{category} Permissions</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {perms.map((perm: any) => (
                      <div key={perm.key} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        <CheckCircle className="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="font-medium text-sm">{perm.name}</p>
                          <p className="text-xs text-gray-500 mt-1">{perm.description}</p>
                          <Badge variant="outline" className="mt-2 text-xs">{perm.key}</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <TableBody>
                    {policies.map((policy) => (
                      <TableRow key={policy.id}>
                        ...
                        <TableCell className="text-sm text-gray-500">{policy.description}</TableCell>
                        <TableCell>
                          <Badge className={policy.effect === 'allow' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                            {policy.effect}
                          </Badge>
                        </TableCell>
      ...
      <Dialog open={showRoleDialog} onOpenChange={setShowRoleDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={createRole} className="bg-[#FF8C42] hover:bg-[#ff7a28]">
              Create Role
            </Button>
          </DialogFooter>
        </DialogContent>
