...
export function ComplianceIssuesTab() {
  ...
  return (
    <div>
      <div className="mb-4">
        <div className="text-sm text-gray-600 mb-4">Current Compliance Issues</div>
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          filteredIssues.map((issue) => (
            <div key={issue.id} className="grid grid-cols-12 gap-4 px-4 py-4 bg-white border border-gray-200 rounded-lg items-center hover:bg-gray-50">
              <div className="col-span-3">
                ...
                <div className="text-xs text-red-600 ml-4">{issue.alertDetails}</div>
              </div>
              
              <div className="col-span-2">
                <span className={`inline-block px-3 py-1 text-xs rounded-full border ${getPriorityColor(issue.priority)}`}>
                  {issue.priority.charAt(0).toUpperCase() + issue.priority.slice(1)}
                </span>
              </div>
              
              <div className="col-span-2">
                <div className="text-sm">{issue.type.charAt(0).toUpperCase() + issue.type.slice(1)}</div>
              </div>
