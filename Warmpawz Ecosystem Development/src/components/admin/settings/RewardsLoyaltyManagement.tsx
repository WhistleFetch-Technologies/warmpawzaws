  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Loyalty & Rewards Program</h2>
          <p className="text-muted-foreground">Manage Pawints earning rules for Customers and Vendors</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total Pawints Issued</CardTitle>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Define how users earn points. Changes apply immediately after saving.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {rules.map((rule, idx) => (
                <TableRow key={rule.id}>
                  ...
                  <TableCell>
                    <Badge variant={rule.category === 'Vendor' ? 'secondary' : 'outline'}>
                      {rule.category}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <div className="font-medium">{rule.description}</div>
                    <div className="text-xs text-muted-foreground font-mono">{rule.action}</div>
                  </TableCell>
                  <TableCell>
                    {rule.type === 'fixed' && <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">Fixed</Badge>}
                    ...
                    {rule.type === 'multiplier' && (
                      <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100">Multiplier</Badge>
                    )}
                  </TableCell>
