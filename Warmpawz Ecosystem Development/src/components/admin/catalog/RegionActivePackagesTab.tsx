    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {categories.map(category => {
          ...
          const minPrice = Math.min(...categoryPackages.map(p => p.finalPrice));
          const maxPrice = Math.max(...categoryPackages.map(p => p.finalPrice));

          return (
            <Card key={category || index} className="border-2 border-gray-200 overflow-hidden">
              {/* Category Header */}
              <button
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-600">
                            Base: {formatPrice(pkg.basePrice)} + Tax ({pkg.taxRate}%)
                          </p>
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-xl font-bold text-blue-900">
              {formatPrice(
                Math.min(...packages.map(p => p.currentRegionPricing?.finalPrice || 0))
              )}
            </p>
          </div>
          ...
          <div className="p-4 bg-purple-50 rounded-lg">
            ...
            <p className="text-xl font-bold text-purple-900">
              {formatPrice(
                Math.max(...packages.map(p => p.currentRegionPricing?.finalPrice || 0))
              )}
            </p>
          </div>
        </div>
