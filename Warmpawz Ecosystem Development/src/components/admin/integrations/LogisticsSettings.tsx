...
export function LogisticsSettings() {
  const { fetchLogistics, saveLogisticsPartner, loading } = useAdminIntegrations();
  const [partners, setPartners] = useState<LogisticsPartner[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentPartner, setCurrentPartner] = useState<LogisticsPartner>(DEFAULT_PARTNER);
  ...
  const toggleCategory = (category: string) => {
    ...
    if (current.includes(category)) {
      setCurrentPartner({ ...currentPartner, categories: current.filter(c => c !== category) });
    } else {
      setCurrentPartner({ ...currentPartner, categories: [...current, category] });
    }
  };
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex items-center justify-between mb-4">
          ...
          {activeTab === 'partners' && (
            <Button onClick={() => {
              setCurrentPartner({ ...DEFAULT_PARTNER, id: `partner_${Date.now()}` });
              setModalTab('details');
              setIsModalOpen(true);
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {partners.length === 0 && (
              <div className="text-center py-16 bg-slate-50/50 rounded-xl border-2 border-dashed border-slate-200">
                ...
                <h3 className="text-lg font-semibold text-slate-900">No Logistics Partners</h3>
                <p className="text-slate-500 max-w-sm mx-auto mt-2">Add your first delivery partner to start managing shipments and delivery rules.</p>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0 gap-0">
          <DialogHeader className="p-6 pb-4 border-b bg-slate-50/50">
            <DialogTitle className="flex items-center gap-2 text-xl">
               ...
               {partners.some(p => p.id === currentPartner.id) ? 'Edit Partner Configuration' : 'New Logistics Partner'}
            </DialogTitle>
            ...
            <Tabs value={modalTab} onValueChange={setModalTab} className="w-full">
              ...
              <TabsContent value="details" className="space-y-6 mt-0">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={e => setCurrentPartner({...currentPartner, id: e.target.value})} 
                      disabled={partners.some(p => p.id === currentPartner.id && p !== DEFAULT_PARTNER)} 
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(v: DeliveryType) => setCurrentPartner({...currentPartner, type: v})}
                  >
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({...currentPartner, apiEndpoint: e.target.value})} 
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({...currentPartner, apiKey: e.target.value})}
                        className="font-mono text-sm" 
                        placeholder="••••••••••••••••"
                      />
                    </div>
                ...
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">Enable or disable this integration</p>
                  </div>
                  <Switch 
                    checked={currentPartner.enabled} 
                    onCheckedChange={(c) => setCurrentPartner({...currentPartner, enabled: c})} 
                  />
                </div>
              </TabsContent>
                ...
                <div className="space-y-4">
                   <div className="space-y-2">
                     ...
                     <div className="p-4 bg-slate-50 rounded-lg border space-y-4">
                           ...
                           <Input 
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  ...
                                  if (val && !currentPartner.regions?.includes(val)) {
                                    setCurrentPartner({
                                      ...currentPartner,
                                      regions: [...(currentPartner.regions || []), val]
                                    });
                       ...
                       {/* Quick Select */}
                       <div>
                         <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 block">Major Cities</span>
                         <div className="flex flex-wrap gap-2">
                           {['Bangalore', 'Mumbai', 'Delhi', 'Hyderabad', 'Chennai', 'Kolkata', 'Pune', 'Gurgaon'].map((city) => (
                             <Button
                               key={city}
                               variant="outline"
                               size="sm"
                               ...
                               onClick={() => {
                                 if (!currentPartner.regions?.includes(city)) {
                                   setCurrentPartner({
                                     ...currentPartner,
                                     regions: [...(currentPartner.regions || []), city]
                                   });
                                 }
                               }}
                               disabled={currentPartner.regions?.includes(city)}
                             >
                               {currentPartner.regions?.includes(city) ? <Check className="w-3 h-3 mr-1" /> : <Plus className="w-3 h-3 mr-1" />}
                               {city}
                             </Button>
                           ))}
                         </div>
                       ...
                       <div className="bg-white rounded-lg border border-slate-200 p-3 min-h-[80px]">
                          <div className="flex items-center justify-between mb-2">
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Selected Coverage</span>
                             <span className="text-[10px] text-slate-400">{currentPartner.regions?.length || 0} regions</span>
                          </div>
                          ...
                          {(!currentPartner.regions || currentPartner.regions.length === 0) ? (
                            <div className="flex flex-col items-center justify-center h-12 text-slate-400 text-xs bg-slate-50 rounded border border-dashed border-slate-200">
                              <span className="flex items-center gap-1"><Globe className="w-3 h-3" /> Global / Pan-India Availability</span>
                            </div>
                          ) : (
                            <div className="flex flex-wrap gap-2">
                              {currentPartner.regions.map((region, idx) => (
                                 <Badge key={idx} variant="secondary" className="pl-2 pr-1 py-1 flex items-center gap-1 bg-orange-50 text-orange-700 border-orange-100 border hover:bg-orange-100 transition-colors group">
                                   <MapPin className="w-3 h-3 opacity-50" />
                                   {region}
                                   <button 
                                     onClick={() => {
                                       const newRegions = [...currentPartner.regions];
                                       newRegions.splice(idx, 1);
                                       setCurrentPartner({ ...currentPartner, regions: newRegions });
                                     }}
                   ...
                   <div className="space-y-3">
                    ...
                    <ScrollArea className="h-[240px] rounded-lg border p-4">
                      <div className="grid grid-cols-2 gap-3">
                        {PRODUCT_CATEGORIES.map((cat) => (
                          <div key={cat} className="flex items-center space-x-2 p-2 rounded hover:bg-slate-50 transition-colors">
                            <Checkbox 
                              id={cat} 
                    ...
                    <p className="text-xs text-muted-foreground text-right">
                      Selected: {currentPartner.categories?.length || 0} categories
                    </p>
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="pricing" className="space-y-6 mt-0">
                <div className="bg-green-50/50 border border-green-100 rounded-lg p-4">
                  <h3 className="text-sm font-semibold text-green-900 mb-1">Dynamic Pricing Logic</h3>
                  <p className="text-xs text-green-700">
                    Total Cost = (Base Fee + (Distance × Per Km)) × Surge Multiplier
                  </p>
                </div>
                    ...
                    <div className="relative">
                      ...
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({
                          ...currentPartner, 
                          pricing: { ...currentPartner.pricing, baseFee: Number(e.target.value) }
                        })} 
                        className="pl-9"
                      />
                    </div>
                    ...
                    <Label>Charge Per Km (₹)</Label>
                     <div className="relative">
                      ...
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({
                          ...currentPartner, 
                          pricing: { ...currentPartner.pricing, perKm: Number(e.target.value) }
                        })} 
                        className="pl-9"
                      />
                    </div>
                   ...
                   <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({
                          ...currentPartner, 
                          pricing: { ...currentPartner.pricing, minCartValue: Number(e.target.value) }
                        })} 
                        className="pr-24"
                      />
                    ...
                    <p className="text-xs text-muted-foreground">
                      Orders above this amount will have ₹0 delivery fee. Set to 0 to disable.
                    </p>
                  </div>
                   ...
                   <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      <Input 
                        ...
                        onChange={e => setCurrentPartner({
                          ...currentPartner, 
                          pricing: { ...currentPartner.pricing, surgeMultiplier: Number(e.target.value) }
                        })} 
                        className="w-32"
                      />
                      <div className="text-xs text-slate-500">
                        <span className="font-bold text-slate-900">1.0</span> = Standard Price<br/>
                        <span className="font-bold text-slate-900">1.5</span> = 50% Extra (Peak Hours)
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex w-full justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleSave} disabled={loading} className="bg-slate-900 text-white hover:bg-slate-800">
                  {partners.some(p => p.id === currentPartner.id) ? 'Update Partner' : 'Add Partner'}
                </Button>
              </div>
            </div>
          </DialogFooter>
...
function PartnerCard({ partner, onEdit }: { partner: LogisticsPartner, onEdit: () => void }) {
  return (
    <Card className="overflow-hidden hover:shadow-md transition-shadow group">
      <div className="flex flex-col md:flex-row">
        {/* Status Strip */}
        <div className={`w-full md:w-1.5 h-2 md:h-auto transition-colors ${partner.enabled ? 'bg-green-500' : 'bg-slate-300'}`} />
        ...
        <div className="p-5 flex-1 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          ...
          <div className="md:col-span-4 space-y-1">
            <div className="flex items-center gap-2">
              ...
              {!partner.enabled && <Badge variant="outline" className="text-xs h-5 px-1.5 text-slate-500">Disabled</Badge>}
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Badge variant="secondary" className="text-[10px] px-1.5 font-normal tracking-wide uppercase">{partner.type.replace('_', ' ')}</Badge>
          ...
          <div className="md:col-span-3">
            <div className="text-xs text-slate-400 uppercase tracking-wider font-medium mb-1">Pricing Model</div>
            <div className="text-sm font-medium text-slate-700">
              ₹{partner.pricing.baseFee} <span className="text-slate-400 font-normal">base</span> + ₹{partner.pricing.perKm}<span className="text-slate-400 font-normal">/km</span>
            </div>
          ...
          {/* Regions / Categories */}
          <div className="md:col-span-3">
             <div className="text-xs text-slate-400 uppercase tracking-wider font-medium mb-1">Coverage</div>
             <div className="flex flex-wrap gap-1">
                {partner.regions?.length > 0 ? (
                  partner.regions.slice(0, 2).map(r => (
                    <span key={r} className="text-[10px] bg-orange-50 text-orange-700 px-1.5 py-0.5 rounded border border-orange-100">{r}</span>
                  ))
                ) : (
                  <span className="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100">Global</span>
                )}
                 {partner.regions?.length > 2 && (
                  <span className="text-[10px] text-slate-400">+{partner.regions.length - 2}</span>
                )}
              </div>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
            <Button variant="ghost" size="sm" onClick={onEdit} className="hover:bg-slate-100">
...
function RuleSimulator({ partners }: { partners: LogisticsPartner[] }) {
  ...
  const results = useMemo(() => {
    return partners
      .filter(p => p.enabled)
      .filter(p => p.categories.length === 0 || p.categories.includes(simState.category))
      .filter(p => p.regions.length === 0 || p.regions.some(r => r.toLowerCase() === simState.city.toLowerCase()))
      .map(p => {
        let cost = p.pricing.baseFee + (simState.distance * p.pricing.perKm);
        let isFree = false;
        
        if (p.pricing.minCartValue > 0 && simState.cartValue >= p.pricing.minCartValue) {
          cost = 0;
          isFree = true;
        }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <CardContent className="space-y-5 pt-6">
            <div className="space-y-2">
              ...
              <Input 
                ...
                onChange={e => setSimState({...simState, cartValue: Number(e.target.value)})} 
              />
            </div>
            <div className="space-y-2">
              ...
              <Input 
                ...
                onChange={e => setSimState({...simState, distance: Number(e.target.value)})} 
              />
            </div>
            <div className="space-y-2">
              <Label>Product Category</Label>
              <Select value={simState.category} onValueChange={v => setSimState({...simState, category: v})}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Input 
                value={simState.city} 
                onChange={e => setSimState({...simState, city: e.target.value})} 
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-lg text-slate-900">Quote Comparison</h3>
            <p className="text-sm text-slate-500">Real-time calculation based on active rules</p>
          </div>
          <Badge variant="outline" className="bg-white">{results.length} Eligible Partner{results.length !== 1 && 's'}</Badge>
        </div>
        ...
        {results.length === 0 ? (
           <div className="text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
              ...
              <p className="text-sm text-slate-500 mt-1">Adjust your simulation parameters or check partner coverage.</p>
           </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {results.map(({ partner, cost, isFree }, index) => (
              <Card key={partner.id} className={`overflow-hidden transition-all ${index === 0 ? 'ring-2 ring-green-500 shadow-md' : 'hover:shadow-sm'}`}>
                {index === 0 && (
                ...
                <div className="p-5 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                        <Badge variant="secondary" className="text-[10px] px-1.5 h-5">{partner.type}</Badge>
                        <span>₹{partner.pricing.baseFee} + ₹{partner.pricing.perKm}/km</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {isFree ? (
                      ...
                      <div className="text-xs text-slate-400 font-medium mt-1">Estimated Cost</div>
                    )}
                  </div>
                </div>
              </Card>
