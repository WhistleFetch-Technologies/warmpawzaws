  ...
  const loadSellers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const petProductSellers = data.vendors?.filter((v: any) => v.roleId === 'pet_product') || [];
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage pet product sellers on the platform</p>
        </div>
      </div>
                ...
                sellers.map((seller) => (
                  <tr key={seller.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-medium text-black">{seller.businessName || seller.fullName}</p>
                        <p className="text-xs text-gray-500">ID: {seller.id.slice(0, 8)}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-600">{seller.phone}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs ${
                        seller.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                      }`}>
                        {seller.isActive ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
