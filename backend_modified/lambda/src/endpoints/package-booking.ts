...
import { getActivePackages, createBookingFromPackage } from '../controllers/booking.controller';
...
export function registerPackageBookingEndpoints(app: Hono) {
   ...
   */
  app.get("/customer/:customerId/packages/active", getActivePackages);

  /**
   * POST /bookings/create-from-package
   * Create a booking using package credits instead of new payment
   */
  app.post("/bookings/create-from-package", createBookingFromPackage);
