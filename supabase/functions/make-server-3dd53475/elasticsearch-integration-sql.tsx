/**
 * ============================================================================
 * ELASTICSEARCH INTEGRATION - SQL-ONLY VERSION
 * ============================================================================
 * 
 ...
 * - Search analytics stored in `search_analytics`, `popular_searches`, `zero_result_searches` tables
 * - Vendor/staff data fetched from SQL repositories
 * 
 * Date: 2025-01-27
...
// Search Analytics (SQL-based)
class SearchAnalytics {
  static async trackSearch(query: string, results: number, responseTime: number, filters: any) {
  ...
  static async getAnalytics() {
    ...
    try {
      ...
      const { data: zeroResultSearches } = await db
        .from('zero_result_searches')
        .select('query, search_count')
        .order('search_count', { ascending: false })
        .limit(10);

      // Get searches by category (aggregate from search_analytics)
      const { data: categorySearches } = await db
        .from('search_analytics')
      ...
      const totalResponseTime = 0; // Would need to track this separately if needed

      // Aggregate by category (simplified - would need category field in search_analytics)
      const searchesByCategory: Record<string, number> = {};
      // This would need category tracking in search_analytics table
      ...
      return {
        ...
        zeroResultSearches: (zeroResultSearches || []).map((s: any) => ({
          query: s.query,
          count: s.search_count
        })),
        searchesByCategory: Object.entries(searchesByCategory).map(([category, count]) => ({
          category,
          count
        })),
        totalSearches,
        averageSearchTime: totalSearches > 0 ? Math.round(totalResponseTime / totalSearches) : 0
      };
    } catch (error) {
...
export function elasticsearchIntegration(app: Hono) {
  ...
  app.post(`${BASE_PATH}/elasticsearch/index/center/:centerId`, async (c) => {
    try {
      ...
      const document = {
        ...
        services: [], // Would need to fetch from services table
        ...
        phone: vendor.phone,
        email: vendor.email,
        openingHours: (vendor as any).operating_hours || {},
        amenities: [],
  ...
  app.post(`${BASE_PATH}/elasticsearch/index/staff/:staffId`, async (c) => {
    try {
      ...
      const document = {
        ...
        location: null, // Would need location from vendor
  ...
  app.get(`${BASE_PATH}/elasticsearch/search`, async (c) => {
    try {
      ...
      await SearchAnalytics.trackSearch(query, totalResults, responseTime, filters);
  ...
  app.post(`${BASE_PATH}/elasticsearch/reindex/all`, async (c) => {
    try {
      ...
      if (vendors.length > 0) {
        const centerDocs = vendors.map((vendor) => ({
          id: vendor.id,
          vendorId: vendor.id,
          businessName: vendor.business_name,
          services: [],
      ...
      if (allStaff.length > 0) {
        const staffDocs = allStaff.map((s) => ({
          id: s.id,
          staffId: s.staffId,
          name: s.fullName,
          role: s.role,
  ...
  console.log('✅ Elasticsearch Integration (SQL-only) registered');
}
