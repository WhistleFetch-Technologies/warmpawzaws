/**
 * ============================================================================
 * AUTO-ASSIGNMENT LOGIC - SQL-ONLY VERSION
 * ============================================================================
 * 
 ...
 * - Uses `StaffRepository`, `BookingsRepository`, `ServicesRepository`
 * - Uses `staff`, `bookings`, `staff_services`, `staff_availability` tables
 * 
 * Date: 2025-01-28
...
export async function assignInstantTele(
  ...
  try {
    // Step 1: ✅ SQL: Load candidate staff details
    ...
    // Step 5: ✅ SQL: Update booking with assignment
    await assignStaffToBooking(bookingId, selected.staffId);

    // Step 6: ✅ SQL: Notify staff
...
export async function assignHomeService(
  ...
  try {
    // Step 1: ✅ SQL: Get all staff who can provide this service
...
/**
 * ✅ SQL: Load staff details from database
 */
async function loadStaffDetails(staffIds: string[]): Promise<StaffAvailability[]> {
  ...
  for (const staffId of staffIds) {
    try {
      ...
      staff.push({
        staffId,
        staffName: staffData.name || staffData.full_name,
        staffPhoto: staffData.photo,
        rating: staffData.rating || 4.5,
        isOnline: availabilityData?.is_online || false,
        currentLocation: availabilityData?.current_location ? {
          latitude: availabilityData.current_location.lat,
          longitude: availabilityData.current_location.lng
        } : undefined,
        activeBookings: activeBookings?.length || 0,
        maxConcurrentBookings: availabilityData?.max_concurrent_bookings || 1,
...
/**
 * ✅ SQL: Get staff who can provide a specific service
 */
...
/**
 * ✅ SQL: Filter staff by scheduled availability
 */
...
/**
 * ✅ SQL: Notify staff of new assignment
 */
