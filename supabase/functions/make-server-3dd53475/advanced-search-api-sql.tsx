/**
 * ============================================================================
 * ADVANCED SEARCH API - SQL VERSION
 * ============================================================================
 * 
 * Multi-field fuzzy search, autocomplete, and search analytics
 * Replaces: search_analytics_* KV keys
 * 
 * RULES:
...
export function advancedSearchAPISQL(app: Hono) {
      ...
      try {
        ...
        await db
          ...
          .upsert({
            ...
            results_count: formattedResults.total,
            zero_results: formattedResults.total === 0,
        ...
        const { data: existingPopular } = await db
          .from('popular_searches')
          .select('search_count')
          .eq('query', normalizedQuery)
          .single();
        
        const newCount = (existingPopular?.search_count || 0) + 1;
  ...
  app.get(`${BASE_PATH}/search/popular`, async (c) => {
    try {
      ...
      // ✅ SQL: Get popular searches from database
      const { data: popularSearches, error } = await db
        .from('popular_searches')
  ...
  console.log('✅ Advanced Search API endpoints registered (SQL-only)');
}
