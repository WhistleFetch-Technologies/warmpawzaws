 ...
 * VET SPECIALIZED SERVICES ENDPOINTS - SQL VERSION
 */
export function vetSpecializedServicesSQL(app: Hono) {
  ...
  app.post(`${BASE_PATH}/vendor/:vendorId/pharmacy/verify-prescription`, async (c) => {
    try {
      ...
      const orderPharmacyId = (order as any).pharmacy_vendor_id || (order as any).selected_pharmacy_id;
      ...
      await notificationsRepo.create({
        ...
        message: `Your prescription order #${order.order_number} has been verified. Awaiting invoice.`,
  ...
  app.post(`${BASE_PATH}/vendor/:vendorId/pharmacy/send-invoice`, async (c) => {
    try {
      ...
      // ✅ SQL: Update order with invoice details using generateProformaInvoice
      await medicineOrdersRepo.generateProformaInvoice(
        orderId,
        `https://warmpawz.com/invoices/${invoiceId}`,
        amount,
        []
      );
  ...
  app.post(`${BASE_PATH}/vendor/:vendorId/pharmacy/dispatch-order`, async (c) => {
    try {
      ...
      await medicineOrdersRepo.updateDeliveryStatus(
        orderId,
        'in_transit',
        deliveryPartnerId
      );
      ...
      await notificationsRepo.create({
        ...
        message: `Your order #${order.order_number} has been dispatched! Track: ${trackingUrl}`,
  ...
  app.get(`${BASE_PATH}/vendor/:vendorId/emergency-protocols`, async (c) => {
    try {
      ...
      const protocols = (vendor.metadata as any)?.emergency_protocols || [];
  ...
  app.post(`${BASE_PATH}/vendor/:vendorId/emergency-protocols`, async (c) => {
    try {
      ...
      const protocols = (metadata.emergency_protocols || []) as any[];
