/**
 * ============================================================================
 * CANCELLATION POLICY ENDPOINTS - SQL-ONLY VERSION
 * ============================================================================
 * 
 ...
 * - Uses `platform_settings` table (JSONB) for cancellation policies
 * 
 * Date: 2025-01-28
...
export function cancellationPolicyEndpoints(app: Hono) {
  ...
  // Helper to get all cancellation policies
  async function getAllPolicies(): Promise<any[]> {
    ...
    return data?.setting_value?.policies || [];
  }

  // Helper to save all cancellation policies
  async function savePolicies(policies: any[]): Promise<void> {
    await db
      .from('platform_settings')
      .upsert({
  ...
  app.get(`${BASE_PATH}/admin/finance/cancellation-policies`, async (c) => {
    try {
      ...
      // Sort by priority
      const sorted = policies.sort((a: any, b: any) => (a.priority || 0) - (b.priority || 0));
  ...
  console.log('✅ Cancellation policy endpoints (SQL-only) registered');
}
