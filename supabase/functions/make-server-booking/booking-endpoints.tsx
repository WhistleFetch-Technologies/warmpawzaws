...
export function bookingEndpoints(app: Hono) {
  ...
  app.post("/make-server-3dd53475/bookings/create", async (c) => {
    try {
      ...
      // ✅ BUSINESS RULES VALIDATION: Validate booking context using Business Rules Engine
      // Validate after staff assignment so we can use the correct staff_id
      const { validateBookingContext, formatValidationError } = await import("../../lib/utils/booking-validation-helper.ts");
      
      // Extract latitude/longitude from customerAddress if it's an object
      let latitude: number | undefined;
      let longitude: number | undefined;
      if (customerAddress) {
        if (typeof customerAddress === 'object' && 'latitude' in customerAddress && 'longitude' in customerAddress) {
          latitude = customerAddress.latitude;
          longitude = customerAddress.longitude;
        }
      }

      const validationResult = await validateBookingContext({
        vendor_id: vendorId,
        staff_id: assignedStaffId,
        customer_id: customerId,
        service_type: serviceType || 'at_center',
      ...
      if (!validationResult.valid) {
        if (validationResult.violations) {
          const formattedError = formatValidationError(validationResult.violations);
          return c.json({
            error: formattedError.message,
            error_code: formattedError.error_code,
            violations: formattedError.violations
          }, 400);
        }
        return c.json({ error: validationResult.error || 'Booking validation failed' }, 400);
      }
