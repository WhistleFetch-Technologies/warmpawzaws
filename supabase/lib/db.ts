...
export async function insertQuery<T>(
  ...
  try {
    ...
    const cleanData = Array.isArray(data) 
      ? data.map(item => cleanInsertData(item, table))
      : cleanInsertData(data, table);
    
    console.log(`[insertQuery] Inserting into ${table}:`, JSON.stringify(cleanData, null, 2));
    console.log(`[insertQuery] Data keys:`, Object.keys(cleanData));
    ...
    if (error) {
      ...
      console.error(`[insertQuery] Error code:`, error.code);
      console.error(`[insertQuery] Error message:`, error.message);
      console.error(`[insertQuery] Error details:`, error.details);
      console.error(`[insertQuery] Error hint:`, error.hint);
      console.error(`[insertQuery] Data attempted:`, JSON.stringify(cleanData, null, 2));
      console.error(`[insertQuery] Data keys attempted:`, Object.keys(cleanData));
      throw error;
    }
 ...
 */
function cleanInsertData(data: any, tableName?: string): any {
  if (!data || typeof data !== 'object') return data;
  
  // Create a clean copy
  const cleaned: Record<string, any> = {};
  
  // For customers table, allow fields that match the ACTUAL database schema
  if (tableName === 'customers') {
    // ALLOWED fields for customers table based on actual schema:
    // - customer_id (VARCHAR NOT NULL) - REQUIRED
    // - phone (VARCHAR NOT NULL) - REQUIRED
    // - user_id (UUID nullable)
    // - email (VARCHAR nullable)
    // - full_name (VARCHAR nullable)
    // - address (JSONB nullable)
    // - journey_stage (VARCHAR nullable)
    // - preferences (JSONB, default '{}')
    // - loyalty_points (INTEGER, default 0)
    // - total_bookings (INTEGER, default 0)
    // - total_spent (NUMERIC, default 0)
    // - is_active (BOOLEAN, default true)
    // DO NOT include: id, created_at, updated_at (auto-generated)
    
    const allowedFields = [
      'customer_id', // REQUIRED in actual schema
      'phone', // REQUIRED in actual schema
      'user_id',
      'email',
      'full_name',
      'address',
    ...
    console.log(`[cleanInsertData] Cleaned data for customers table:`, JSON.stringify(cleaned, null, 2));
