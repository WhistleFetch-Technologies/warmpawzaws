...
echo ""
PROTECTED_URL="https://vendor.warmpawz.com"
[ -f "$PROJECT_ROOT/config/urls.json" ] && command -v jq &>/dev/null && PROTECTED_URL=$(jq -r '.protectedUrls.prod.vendor // empty' "$PROJECT_ROOT/config/urls.json") || true
PROTECTED_URL="${PROTECTED_URL:-$CLOUDFRONT_URL}"
echo -e "🌐 Access URLs (use protected URL for HTTPS):"
echo -e "   - Vendor Web (PROD): ${PROTECTED_URL}"
