...
if (-not $SKIP_CONFIRM) {
    Write-Host "⚠️  ⚠️  ⚠️  WARNING: PRODUCTION DEPLOYMENT ⚠️  ⚠️  ⚠️" -ForegroundColor Red
...
if ([string]::IsNullOrEmpty($API_BASE_URL)) {
    Write-Host "⚠️  Getting API endpoint from AWS..." -ForegroundColor Yellow
    try {
...
if ($DEPLOY_ONLY -and (Test-Path "dist")) {
    ...
    try {
        ...
        Write-Host "⚠️  First build failed, retrying with full clean..." -ForegroundColor Yellow
        if (Test-Path ".next") { Remove-Item -Recurse -Force ".next" }
        if (Test-Path "dist") { Remove-Item -Recurse -Force "dist" }
        if (Test-Path "node_modules\.cache") { Remove-Item -Recurse -Force "node_modules\.cache" }
    ...
    if (-not (Test-Path "dist\_next\static")) {
        Write-Host "⚠️  Warning: dist\_next\static directory not found" -ForegroundColor Yellow
...
$runtimeConfigS3Path = Join-Path $APP_DIR "dist" "runtime-config.js"
...
if (Test-Path (Join-Path $APP_DIR "dist\_next\static")) {
...
if ($staticExists) {
    ...
    Write-Host "⚠️  Warning: _next/static directory not found in S3" -ForegroundColor Yellow
}
