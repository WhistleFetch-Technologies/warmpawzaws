...
if ($DEPLOY_ONLY -and (Test-Path "dist")) {
    Write-Host "[OK] Skipping build (--deploy-only, dist exists)" -ForegroundColor Green
} else {
    ...
    if (-not $buildSuccess) {
        Write-Host "Retrying build after full clean..." -ForegroundColor Yellow
        if (Test-Path ".next") { Remove-Item -Recurse -Force ".next" }
        if (Test-Path "dist") { Remove-Item -Recurse -Force "dist" }
        if (Test-Path "node_modules/.cache") { Remove-Item -Recurse -Force "node_modules/.cache" }
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host "[ERROR] Build failed after retry" -ForegroundColor Red
            exit 1
        }
    }
    ...
    if (-not (Test-Path "dist/_next/static")) {
        ...
        $jsFiles = (Get-ChildItem -Path "dist/_next/static" -Recurse -Filter "*.js" -ErrorAction SilentlyContinue).Count
        Write-Host "[OK] Found ${jsFiles} JavaScript files in build" -ForegroundColor Green
    }
...
Write-Host "[OK] runtime-config.js injected (apiBaseUrl -> API Gateway)" -ForegroundColor Green
...
aws s3 cp "${runtimeConfigPath}" "s3://${S3_BUCKET}/runtime-config.js" `
...
$nextStaticPath = Join-Path (Join-Path (Join-Path (Join-Path $APP_DIR "dist") "_next") "static") ""
if (Test-Path $nextStaticPath) {
    Write-Host "Setting cache headers for _next/static files..." -ForegroundColor Blue
    $staticFiles = Get-ChildItem -Path $nextStaticPath -Recurse -File -Include "*.js", "*.css"
...
try {
    aws s3 ls "s3://${S3_BUCKET}/index.html" --region $REGION | Out-Null
...
try {
    ...
    Write-Host "[OK] Found ${jsCount} JavaScript files in S3" -ForegroundColor Green
} catch {
    Write-Host "[WARNING] _next/static directory not found in S3" -ForegroundColor Yellow
}

Write-Host "[OK] S3 upload completed successfully" -ForegroundColor Green
...
try {
    ...
    if ($invalidation.Invalidation.Id) {
        ...
        Write-Host "[OK] CloudFront invalidation created: ${INVALIDATION_ID}" -ForegroundColor Green
...
Write-Host "   [OK] ${APP_NAME}: Built successfully" -ForegroundColor Green
Write-Host "   [OK] S3 Upload: Synced to ${S3_BUCKET}" -ForegroundColor Green
Write-Host "   [OK] CloudFront: Cache invalidation created (${INVALIDATION_ID})" -ForegroundColor Green
Write-Host ""
