...
echo ""
PROTECTED_URL="https://customer.warmpawz.com"
[ -f "$PROJECT_ROOT/config/urls.json" ] && command -v jq &>/dev/null && PROTECTED_URL=$(jq -r '.protectedUrls.prod.customer // empty' "$PROJECT_ROOT/config/urls.json") || true
PROTECTED_URL="${PROTECTED_URL:-$CLOUDFRONT_URL}"
echo -e "🌐 Access URLs (use protected URL for HTTPS):"
echo -e "   - Customer Web (PROD): ${PROTECTED_URL}"
...
echo -e "   2. Test the deployed application at ${PROTECTED_URL}"
