#!/bin/bash
# Direct AWS CLI deployment script for admin-web (PRODUCTION)
# Usage: ./prodscripts/deploy-admin-web-prod.sh [--deploy-only]
...
if [ "$SKIP_CONFIRM" = false ]; then
  ...
  echo "This will deploy admin-web to PRODUCTION environment!"
  echo ""
...
if [ "$DEPLOY_ONLY" = true ] && [ -d "dist" ]; then
  ...
  rm -rf .next dist dist-export node_modules/.cache
  sleep 2
