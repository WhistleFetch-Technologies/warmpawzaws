#!/usr/bin/env node
/**
 * Migration 747 — customer:delivery:fee_policy in platform_settings (DEV).
 * Runs via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-747-rds-data-api-dev-cli.js
 *
 * Requires: AWS CLI v2, credentials for dev, RDS Data API on warmpawz-dev-cluster.
 */
...
const CLUSTER_IDENTIFIER = 'warmpawz-dev-cluster';
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '747_customer_delivery_fee_policy.sql'
);

/** Strip block/line comments so split does not break inside commented quotes (copied from 704). */
...
function getClusterInfoCli() {
  console.log('Resolving dev cluster + secret (AWS CLI)...\n');
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_747_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 747 — customer delivery fee policy (DEV, RDS Data API CLI)');
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      if (/already exists|duplicate key|unique violation|ON CONFLICT/i.test(msg)) {
  ...
  console.log('============================================================================');
  console.log(`Done. ${ok}/${statements.length} statement(s) executed on DEV.`);
  console.log('============================================================================');
}
