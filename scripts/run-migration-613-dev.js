/**
 * Migration 613: Allow Duplicate Phone Numbers When Deleted
 * Environment: Dev
...
async function executeSQL(sql, expectResult = false) {
  try {
    ...
    // Check if it's a "already exists" or "does not exist" error (which is OK)
    ...
    if (errorOutput.includes('already exists') || 
      ...
      return { status: 'already_exists_or_not_found' };
    }
    throw error;
  }
}
...
async function main() {
  ...
  try {
    // Step 1: Add is_deleted column to vendor_identity if it doesn't exist
