/**
 * Extract all table schemas from production RDS and generate SQL files
...
async function getTableDefinition(tableName) {
  // Get the CREATE TABLE statement using pg_dump approach via query
  // We'll build it from system catalogs
  const sql = `
    SELECT 
  ...
  try {
    ...
    if (result.records && result.records.length > 0) {
      return parseRecord(result.records[0])[0];
    }
  } catch (error) {
    // pg_get_tabledef might not exist, fall back to manual extraction
    console.log(`   ⚠️  Could not use pg_get_tabledef for ${tableName}, using manual extraction`);
  }
...
async function getTableIndexes(tableName) {
  const sql = `
    SELECT
      i.indexname,
      i.indexdef,
      pg_get_indexdef(i.indexrelid) as full_indexdef
    FROM pg_indexes i
...
function generateCreateTableSQL(tableName, columns, constraints) {
  ...
  if (primaryKeyCols.length > 0) {
    ...
    // Check if primary key is already defined inline
    const hasInlinePK = columns.some(col => 
      constraints.some(c => 
        c.type === 'PRIMARY KEY' && c.columns.includes(col.name)
      )
    );
    if (!hasInlinePK) {
...
function determineFolder(tableName) {
  // Check existing folders and match table names
  const folderMap = {
    'vendor': ['vendors', 'vendor_identity', 'vendor_onboarding_applications'],
    'auth': ['rbac_permissions_catalog'],
    'common': ['prescription_submissions', 'prescriptions', 'pricing_rules', 'problem_grid_mappings', 'promotions', 'rbac_policies', 'referrals', 'refund_rules', 'refund_tiers', 'refunds'],
    'event': ['rbac_audit_logs'],
    'product': ['product_policies', 'products']
  };
  
  for (const [folder, tables] of Object.entries(folderMap)) {
    if (tables.includes(tableName)) {
      return folder;
    }
  }
  
  // Check if table name starts with a known prefix
  if (tableName.startsWith('vendor')) return 'vendor';
  if (tableName.startsWith('rbac_')) {
    if (tableName.includes('audit')) return 'event';
    if (tableName.includes('permission')) return 'auth';
    return 'common';
  }
  if (tableName.startsWith('product')) return 'product';
  if (tableName.includes('prescription')) return 'common';
  if (tableName.includes('refund')) return 'common';
  if (tableName.includes('promotion')) return 'common';
  if (tableName.includes('pricing')) return 'common';
  if (tableName.includes('referral')) return 'common';
  
  // Default: place in schemas root
  return null;
}
...
async function main() {
  ...
  console.log('EXTRACTING ALL TABLE SCHEMAS FROM PRODUCTION RDS');
  console.log('='.repeat(80));
  console.log('');
  ...
  try {
    ...
    for (const table of tables) {
      ...
      try {
        ...
        // Determine folder
        const folder = determineFolder(table.name);
        const targetDir = folder 
          ? path.join(SCHEMAS_DIR, folder)
          : SCHEMAS_DIR;
