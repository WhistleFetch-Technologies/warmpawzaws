...
find apps/admin-web/app apps/admin-web/components -name "*.tsx" -o -name "*.ts" | while read file; do
  grep -h "apiClient\.\(get\|post\|put\|patch\|delete\)" "$file" 2>/dev/null | \
    grep -oE "['\"](/[^'\"]+)['\"]" | \
    sed "s/['\"]//g" >> "$UI_ENDPOINTS_FILE" || true
done
...
while IFS= read -r endpoint; do
  # Clean endpoint (remove query params, etc.)
  clean_endpoint=$(echo "$endpoint" | sed 's/?.*$//' | sed 's/#.*$//')
  
  # Check if endpoint exists in Lambda handlers
  if grep -r "$clean_endpoint" backend/lambda/src/endpoints --include="*.ts" >/dev/null 2>&1; then
...
echo ""
echo -e "${BLUE}📋 Step 3: Checking handler registration...${NC}"
# This would require parsing handler/index.ts

# Step 4: Check database tables
echo ""
echo -e "${BLUE}📋 Step 4: Checking database tables...${NC}"
# This would require checking migration files
