#!/usr/bin/env node
/**
 * Migration 735 — booking_state_transitions: add pending_payment rows. PRODUCTION.
 *
 * Why: Trigger `trigger_enforce_booking_state` (installed by migration 046)
 * blocks `UPDATE bookings SET status='confirmed' WHERE status='pending_payment'`
 * because the seed table is missing that transition. Razorpay-paid bookings
 * therefore stay stuck in `pending_payment` even after capture in prod.
 *
 * Runs each statement via `aws rds-data execute-statement --cli-input-json file://...`
 * with dollar-quote-aware splitting (mirrors run-migration-733-rds-data-api-prod-cli.js).
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_735=YES or the script exits without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_735='YES'; node scripts/run-migration-735-rds-data-api-prod-cli.js
 *
 * Usage (bash):
 *   I_CONFIRM_PROD_MIGRATION_735=YES node scripts/run-migration-735-rds-data-api-prod-cli.js
 *
 * Requires: AWS CLI v2, prod credentials, RDS Data API enabled on warmpawz-prod-cluster.
 */
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '735_add_pending_payment_state_transitions.sql'
);

/** Dollar-quote-aware comment stripping (parity with 733 runner). */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_735_prod_${Date.now()}.json`);
  const payload = {
  ...
  try {
    execSync(`${awsCli()} rds-data execute-statement --cli-input-json "${fileUrl}" --region ${REGION} --output json`, {
      encoding: 'utf8',
      stdio: ['inherit', 'pipe', 'inherit'],
      timeout: 300000,
      maxBuffer: 16 * 1024 * 1024,
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_735 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_735=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 735 — booking_state_transitions: pending_payment rows (PROD, RDS Data API CLI)');
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      if (/already exists|duplicate key|constraint .* already exists/i.test(msg)) {
  ...
  console.log('Verify: pending_payment transitions present...');
  /* Schema-agnostic verify (works with both 010 and 046 column sets). */
  const verifySql =
    "SELECT from_status, to_status FROM booking_state_transitions " +
    "WHERE from_status = 'pending_payment' OR to_status = 'pending_payment' " +
    "ORDER BY from_status, to_status";
  try {
    executeStatementCli(clusterArn, secretArn, verifySql);
    console.log('   (See execute-statement output for result records.)');
  } catch (e) {
    console.warn('   Verify query failed (non-fatal):', e.message);
  }
