/**
 * Verify vendor_onboarding_applications schema matches production RDS
...
async function getProductionColumns() {
  const sql = `
    ...
    WHERE table_schema = 'public' 
      AND table_name = 'vendor_onboarding_applications'
...
async function main() {
  ...
  console.log('VERIFYING vendor_onboarding_applications SCHEMA AGAINST PRODUCTION RDS');
  console.log('='.repeat(80));
  console.log('');

  try {
    // Get columns from production
    console.log('📋 Querying production RDS for vendor_onboarding_applications columns...');
    ...
    // Read schema file
    const schemaFile = path.join(__dirname, '..', 'db', 'schemas', 'vendor', 'vendor_onboarding_appliction.sql');
    ...
    for (const line of lines) {
      ...
      if (trimmed.includes('CREATE TABLE') && trimmed.includes('vendor_onboarding_applications')) {
        inTableDef = true;
        continue;
      }
