/**
 * Verify vendor_identity schema matches production RDS (Improved version)
...
function extractColumnsFromSchema(schemaContent) {
  const columns = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Track braces for CHECK constraints
    const openBraces = (line.match(/\(/g) || []).length;
    const closeBraces = (line.match(/\)/g) || []).length;
    braceDepth += openBraces - closeBraces;
    
    // Detect end of table definition (closing parenthesis at brace depth 0)
    ...
    // Extract column definitions - match patterns like:
    // column_name TYPE,
    ...
    if (colMatch) {
      ...
      // Skip if it's a constraint name or other non-column
...
async function main() {
  ...
  console.log('VERIFYING vendor_identity SCHEMA AGAINST PRODUCTION RDS (v2)');
  console.log('='.repeat(80));
  console.log('');
  ...
  try {
    ...
    const schemaColumns = extractColumnsFromSchema(schemaContent);
    
    console.log(`   Found ${schemaColumns.length} columns in schema file`);
    console.log(`   Columns: ${schemaColumns.join(', ')}`);
    console.log('');
    ...
    if (missingInSchema.length === 0 && missingInProd.length === 0) {
      console.log('='.repeat(80));
      console.log('✅ SCHEMA FILE MATCHES PRODUCTION DATABASE!');
      console.log('='.repeat(80));
      console.log('');
    } else if (missingInSchema.length > 0) {
      console.log('='.repeat(80));
