#!/usr/bin/env node
/**
 * Run Migration 518: Add review columns to diagnostic_reports (AWS RDS)
 * Usage: node scripts/run-migration-518-diagnostic-reports-review-columns.js
 */
...
async function run() {
  console.log('🚀 Migration 518: Add Review Columns to diagnostic_reports');
  console.log('============================================================');
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, '..', 'db', 'migrations', '518_diagnostic_reports_review_columns.sql'),
    'utf8'
  );
  ...
  try {
    ...
    console.log('─'.repeat(50));
    console.log('✅ Migration 518 applied successfully!');
    ...
    if (rows.length > 0) {
      ...
      console.log('⚠️  No review columns found (may have already existed)');
    }
