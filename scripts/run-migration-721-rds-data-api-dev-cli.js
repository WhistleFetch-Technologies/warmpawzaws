#!/usr/bin/env node
/**
 * Migration 721 (vendor_admin_portal_codes) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-721-rds-data-api-dev-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '721_vendor_admin_portal_codes.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_721_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 721 — vendor_admin_portal_codes (DEV ONLY, RDS Data API CLI)');
