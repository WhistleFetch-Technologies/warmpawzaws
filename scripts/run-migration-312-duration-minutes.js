#!/usr/bin/env node
/**
 * Run Migration 312: Add duration_minutes column to bookings table
 * Connects to AWS RDS and runs the migration script
 *
 * Usage:
 *   node scripts/run-migration-312-duration-minutes.js [environment] [region]
 *
 * Example:
 *   node scripts/run-migration-312-duration-minutes.js dev ap-south-1
 */
...
async function runMigration() {
  console.log('🚀 Migration 312: Add duration_minutes to bookings table');
  console.log('============================================================');
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, '..', 'db', 'migrations', '312_add_duration_minutes_to_bookings.sql');
    ...
    console.log('🔍 Checking current bookings table structure...');
    ...
    if (beforeCheck.rows.length > 0) {
      ...
      console.log('⚠️  Column duration_minutes does not exist - will be added by migration');
    }
    console.log('');
    ...
    if (afterCheck.rows.length > 0) {
      ...
      throw new Error('Migration verification failed: duration_minutes column not found');
    }
    console.log('');
    ...
    const indexCheck = await pool.query(`
      ...
      WHERE schemaname = 'public'
        ...
        AND indexname = 'idx_bookings_duration_minutes'
    `);
    if (indexCheck.rows.length > 0) {
      console.log('✅ Index idx_bookings_duration_minutes exists');
    } else {
      console.log('⚠️  Index idx_bookings_duration_minutes not found');
    }
    console.log('');

    // Check existing bookings count
    const bookingCount = await pool.query('SELECT COUNT(*) as count FROM bookings');
    console.log(`📊 Total bookings in database: ${bookingCount.rows[0].count}`);
    
    // Check how many have duration_minutes set
    const durationCount = await pool.query('SELECT COUNT(*) as count FROM bookings WHERE duration_minutes IS NOT NULL');
    console.log(`📊 Bookings with duration_minutes: ${durationCount.rows[0].count}`);
    console.log('');

    await pool.end();
    console.log('🎉 Migration 312 completed successfully!');
    console.log('');
    console.log('📝 Next: Test multi-service booking creation.');
    console.log('');
