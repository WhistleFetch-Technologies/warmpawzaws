#!/usr/bin/env node
/**
 * Migration 1004 (customer username + password auth columns) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-1004-rds-data-api-dev-cli.js
 *
 * Requires: AWS CLI v2, dev account credentials, RDS Data API enabled on warmpawz-dev-cluster.
 */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_1004_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 1004 — customer username/password columns (DEV ONLY, RDS Data API CLI)');
