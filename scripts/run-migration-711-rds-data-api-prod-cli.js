#!/usr/bin/env node
/**
 * Migration 711 (service_categories customer dashboard visibility) — PRODUCTION.
 ...
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_711=YES or the script exits without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_711='YES'; node scripts/run-migration-711-rds-data-api-prod-cli.js
 *
 * Usage (bash):
 *   I_CONFIRM_PROD_MIGRATION_711=YES node scripts/run-migration-711-rds-data-api-prod-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '711_service_categories_customer_dashboard_visibility.sql'
);

/** Same algorithm as rds-data-api-utils-dev.js (no SDK dependency). */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_711_prod_input.json');
  const payload = {
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_711 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_711=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 711 — customer dashboard visibility (PRODUCTION, RDS Data API CLI)');
