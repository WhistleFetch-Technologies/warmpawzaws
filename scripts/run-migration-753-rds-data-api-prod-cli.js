#!/usr/bin/env node
/**
 * Migration 753 (booking-service JPA columns) — PRODUCTION.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_753=YES or the script exits without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_753='YES'; node scripts/run-migration-753-rds-data-api-prod-cli.js
 *
 * Usage (bash):
 *   I_CONFIRM_PROD_MIGRATION_753=YES node scripts/run-migration-753-rds-data-api-prod-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '753_booking_service_java_entity_columns.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_753_prod_${Date.now()}.json`);
  const payload = {
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_753 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_753=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 753 — booking-service JPA columns (PROD, RDS Data API CLI)');
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      if (
        /already exists|duplicate key|column .* already exists|relation .* already exists/i.test(
          msg
        )
      ) {
  ...
  console.log('Verify: address_line1 column present on bookings...');
  const verifySql =
    "SELECT column_name FROM information_schema.columns " +
    "WHERE table_name = 'bookings' AND column_name = 'address_line1'";
  try {
