 ...
 * Based on list-solo-vendors-services.js pattern
 */
...
const DB_CLUSTER_ARN = process.env.DB_CLUSTER_ARN || 'arn:aws:rds:ap-south-1:057442119249:cluster:warmpawz-dev-db';
...
async function listGroomerTrainerVendors() {
  ...
  try {
    // Query to get all groomer/trainer vendors with their services
    const query = `
      SELECT 
        v.id as vendor_id,
        v.business_name,
    ...
    rows.forEach(row => {
      ...
      if (!vendorsMap.has(vendorId)) {
        vendorsMap.set(vendorId, {
          ...
          vendor_configuration: row.vendor_configuration || 'N/A',
      ...
      if (row.vendor_service_id) {
        vendorsMap.get(vendorId).services.push({
          ...
          service_name: row.service_name || row.catalog_service_name || 'Unnamed Service',
          service_style: row.service_style || row.catalog_service_style || 'N/A',
          ...
          is_package: row.catalog_is_package || (row.metadata && typeof row.metadata === 'object' && row.metadata.isPackage) || false,
    ...
    vendorsMap.forEach((vendor, vendorId) => {
      ...
      console.log(`   Configuration: ${vendor.vendor_configuration}`);
      console.log('');
    ...
    if (error.message?.includes('SecretId')) {
      console.error('\n💡 Make sure DB_SECRET_ARN is set correctly in environment variables');
    }
    if (error.message?.includes('resourceArn')) {
      console.error('\n💡 Make sure DB_CLUSTER_ARN is set correctly in environment variables');
    }
    throw error;
  }
}
