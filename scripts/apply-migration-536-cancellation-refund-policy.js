 ...
 *
 * Purpose:
 * - cancellation_policies: cancellation_windows, vendor_cancellation_penalty,
 *   no_show_policy, service_category, service_format
 * - vendor_refund_tiers: max_partial_refund_percentage, service_category, service_format
 * - ecommerce_policies: return_window_hours, cancel_before_dispatch_full_refund,
 *   non_returnable_categories
 *
 * Usage:
 *   node scripts/apply-migration-536-cancellation-refund-policy.js
 *
 * With RDS (AWS CLI + Secrets Manager):
 *   ENVIRONMENT=dev node scripts/apply-migration-536-cancellation-refund-policy.js
 *
 * With direct DB URL (e.g. local or CI):
 *   DATABASE_URL=postgresql://user:pass@host:5432/db node scripts/apply-migration-536-cancellation-refund-policy.js
 *
...
async function applyMigration() {
  ...
  console.log('🚀 MIGRATION 536: Cancellation & Refund Policy – Business Rules');
  console.log('='.repeat(70));
  console.log('');
  ...
  console.log('📄 Migration file: 536_cancellation_refund_policy_business_rules.sql');
  console.log('');
  ...
  try {
    ...
    console.log('🔍 Pre-check: cancellation_policies columns (new in 536)...');
    const preCp = await pool.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'cancellation_policies'
        AND column_name IN ('cancellation_windows', 'vendor_cancellation_penalty', 'no_show_policy', 'service_category', 'service_format')
      ORDER BY column_name
    `).catch(() => ({ rows: [] }));
    const existingCp = (preCp.rows || []).map((r) => r.column_name);
    console.log(`   cancellation_policies new columns: ${existingCp.length ? existingCp.join(', ') : 'none'}`);
    console.log('');
    ...
    console.log('🔍 Post-check: cancellation_policies...');
    const postCp = await pool.query(`
      ...
      ORDER BY column_name
    `);
    const afterCp = (postCp.rows || []).map((r) => `${r.column_name} (${r.data_type})`);
    console.log(`   Columns: ${afterCp.length ? afterCp.join(', ') : 'none'}`);
    if (afterCp.length < 3) {
      throw new Error('Verification failed: expected cancellation_windows, vendor_cancellation_penalty, no_show_policy at least');
    }

    console.log('🔍 Post-check: vendor_refund_tiers...');
    const postVrt = await pool.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'vendor_refund_tiers'
        AND column_name IN ('max_partial_refund_percentage', 'service_category', 'service_format')
      ORDER BY column_name
    `).catch(() => ({ rows: [] }));
    const afterVrt = (postVrt.rows || []).map((r) => r.column_name);
    console.log(`   vendor_refund_tiers new columns: ${afterVrt.length ? afterVrt.join(', ') : 'none'}`);

    console.log('🔍 Post-check: ecommerce_policies...');
    const postEp = await pool.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'ecommerce_policies'
        AND column_name IN ('return_window_hours', 'cancel_before_dispatch_full_refund', 'non_returnable_categories')
      ORDER BY column_name
    `).catch(() => ({ rows: [] }));
    const afterEp = (postEp.rows || []).map((r) => r.column_name);
    console.log(`   ecommerce_policies new columns: ${afterEp.length ? afterEp.join(', ') : 'none'}`);
    console.log('');

    console.log('✅ Migration 536 (cancellation & refund policy business rules) complete.');
    await pool.end();
    console.log('');
    console.log('🎉 Done. Admin Finance → Cancellation Policy, Refund Policies, Ecommerce Policies can use new fields.');
  } catch (error) {
    console.error('');
