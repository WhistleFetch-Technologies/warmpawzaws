...
/**
 * ============================================================================
 * MIGRATION 530: Cancellation Policies - policy_type, vendor_types, service_types, priority
 * ============================================================================
 *
 * Purpose:
 * - Add policy_type, vendor_types (TEXT[]), service_types (TEXT[]), priority to cancellation_policies
 * - Enables Admin → Finance → Cancellation Policy to create Standard / Vendor Specific / Service Specific policies
 *
 * Usage:
 *   node scripts/apply-migration-530-cancellation-policies.js
 *
 * With RDS (AWS CLI + Secrets Manager):
 *   ENVIRONMENT=dev node scripts/apply-migration-530-cancellation-policies.js
 *
 * With direct DB URL (e.g. local or CI):
 *   DATABASE_URL=postgresql://user:pass@host:5432/db node scripts/apply-migration-530-cancellation-policies.js
 *
...
async function applyMigration() {
  ...
  console.log('🚀 MIGRATION 530: Cancellation Policies - policy_type, vendor_types, service_types, priority');
  console.log('='.repeat(70));
  console.log('');
  ...
  const migrationPath = path.join(
    ...
    'db',
    'migrations',
    '530_cancellation_policies_policy_type_vendor_service.sql'
  );
  ...
  console.log('📄 Migration file: 530_cancellation_policies_policy_type_vendor_service.sql');
  console.log('');
  ...
  try {
    ...
    console.log('🔍 Pre-check: cancellation_policies columns...');
    const pre = await pool.query(`
      SELECT column_name, data_type
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'cancellation_policies'
        AND column_name IN ('policy_type', 'vendor_types', 'service_types', 'priority')
    ...
    console.log('🔍 Post-check: cancellation_policies columns...');
    ...
    if (after.length < 4) {
      throw new Error('Verification failed: expected policy_type, vendor_types, service_types, priority');
    }
    console.log('');

    console.log('✅ Migration 530 (cancellation policies policy_type, vendor_types, service_types, priority) complete.');
    await pool.end();
    console.log('');
    console.log('🎉 Done. Admin Cancellation Policy can now save Standard / Vendor Specific / Service Specific policies.');
  } catch (error) {
    console.error('');
