#!/usr/bin/env node
/**
 * Run Migration 517: Update report_type constraint in diagnostic_reports (AWS RDS)
 * Usage: node scripts/run-migration-517-diagnostic-report-type-constraint.js
 */
...
async function run() {
  console.log('🚀 Migration 517: Update report_type constraint in diagnostic_reports');
  console.log('=====================================================================');
  ...
  const sql = fs.readFileSync(
    path.join(__dirname, '..', 'db', 'migrations', '517_diagnostic_reports_report_type_constraint.sql'),
    'utf8'
  );
  ...
  try {
    ...
    console.log('─'.repeat(50));
    console.log('✅ Migration 517 applied successfully!');
    
    // Verify the constraint
    console.log('');
    console.log('🔍 Verifying report_type constraint...');
    const { rows } = await pool.query(`
      SELECT constraint_name, check_clause
      FROM information_schema.check_constraints
      WHERE constraint_name = 'diagnostic_reports_report_type_check'
    `);
    
    if (rows.length > 0) {
      console.log('✅ Constraint found:');
      console.log(`   Constraint: ${rows[0].constraint_name}`);
      console.log(`   Check clause: ${rows[0].check_clause}`);
    } else {
      console.log('⚠️  Constraint not found (may have been dropped or never existed)');
    }
