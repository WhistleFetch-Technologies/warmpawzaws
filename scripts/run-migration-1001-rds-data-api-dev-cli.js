#!/usr/bin/env node
/**
 * Migration 1001 (Pidge logistics partner) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-1001-rds-data-api-dev-cli.js
 *
 * Requires: AWS CLI v2, credentials for dev account, RDS Data API enabled on dev cluster.
 * Stdin/stderr are inherited for the initial describe-* calls so SSO/MFA prompts work.
 */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_1001_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 1001 — Pidge logistics partner (DEV ONLY, RDS Data API CLI)');
