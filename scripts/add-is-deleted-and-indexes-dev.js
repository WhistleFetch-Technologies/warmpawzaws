/**
 * Migration: Add is_deleted column and unique indexes to vendors, vendor_identity, and vendor_onboarding_applications tables
 * Environment: Dev
...
async function main() {
  ...
  console.log('MIGRATION: Add is_deleted column and unique indexes');
  console.log('Environment: DEV');
  console.log('='.repeat(80));
  console.log('');

  try {
    // Step 1: Check and add is_deleted to vendors table
    console.log('📋 Step 1: Checking is_deleted column in vendors table...');
    ...
    // Step 2: Check and add is_deleted to vendor_identity table
    console.log('📋 Step 2: Checking is_deleted column in vendor_identity table...');
    ...
    // Step 3: Check and add is_deleted to vendor_onboarding_applications table
    console.log('📋 Step 3: Checking is_deleted column in vendor_onboarding_applications table...');
    const voaHasIsDeleted = await checkColumnExists('vendor_onboarding_applications', 'is_deleted');
    
    if (!voaHasIsDeleted) {
      console.log('   ⚠️  Column does not exist. Adding is_deleted column...');
      await executeSQL('ALTER TABLE vendor_onboarding_applications ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT false;');
      console.log('   ✅ Column added successfully');
    } else {
      console.log('   ✅ Column already exists');
    }
    ...
    // Step 4: Create unique index on vendors.phone
    console.log('📋 Step 4: Checking unique index on vendors.phone...');
    ...
    // Step 5: Create unique index on vendor_identity.phone
    console.log('📋 Step 5: Checking unique index on vendor_identity.phone...');
    const vendorIdentityIndexExists = await checkIndexExists('idx_vendor_identity_phone_unique_active');
    
    if (!vendorIdentityIndexExists) {
      console.log('   ⚠️  Index does not exist. Creating unique index...');
      await executeSQL('CREATE UNIQUE INDEX IF NOT EXISTS idx_vendor_identity_phone_unique_active ON vendor_identity (phone) WHERE is_deleted = false;');
      console.log('   ✅ Index created successfully');
    } else {
      console.log('   ✅ Index already exists');
    }
    ...
    const voaHasColumn = await checkColumnExists('vendor_onboarding_applications', 'is_deleted');
    const vendorsHasIndex = await checkIndexExists('idx_vendors_phone_unique_active');
    const vendorIdentityHasIndex = await checkIndexExists('idx_vendor_identity_phone_unique_active');
    ...
    console.log(`vendor_onboarding_applications.is_deleted: ${voaHasColumn ? '✅ EXISTS' : '❌ MISSING'}`);
    console.log(`idx_vendors_phone_unique_active:            ${vendorsHasIndex ? '✅ EXISTS' : '❌ MISSING'}`);
    console.log(`idx_vendor_identity_phone_unique_active:    ${vendorIdentityHasIndex ? '✅ EXISTS' : '❌ MISSING'}`);
    console.log('='.repeat(80));
    
    if (vendorsHasColumn && vendorIdentityHasColumn && voaHasColumn && vendorsHasIndex && vendorIdentityHasIndex) {
      console.log('');
