#!/usr/bin/env node
/**
 * Migration 704 — referral_redemptions (unique referred_id, one code many referees) — PROD, RDS Data API.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   APPLY_PROD_704=yes node scripts/run-migration-704-rds-data-api-prod-cli.js
 *
 * Requires: AWS CLI v2, credentials, RDS Data API enabled on prod cluster.
 */
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '704_referral_redemptions_one_code_many_referees.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_704_prod_input.json');
  const payload = {
...
function main() {
  if (process.env.APPLY_PROD_704 !== 'yes') {
    console.error('Refusing to run on prod without APPLY_PROD_704=yes');
    process.exit(1);
  }

  console.log('============================================================================');
  console.log('MIGRATION 704 — referral_redemptions (referred_id UNIQUE) (PROD, CLI)');
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      const out = executeStatementCli(clusterArn, secretArn, stmt);
  ...
  console.log('============================================================================');
  console.log(`Done. ${ok}/${statements.length} statement(s) executed on PROD.`);
  console.log('============================================================================');
}
