#!/usr/bin/env node
/**
 * Run Migration 507: Add booking_id to diagnostic_reports (AWS RDS)
 * Usage: node scripts/run-migration-507-diagnostic-reports-booking-id.js
 */
...
async function run() {
  ...
  const sql = fs.readFileSync(path.join(__dirname, '..', 'db', 'migrations', '507_diagnostic_reports_booking_id.sql'), 'utf8');
  await pool.query(sql);
  console.log('Migration 507 applied.');
  await pool.end();
}
