...
async function testCompleteFlow() {
  ...
  try {
    ...
    // Step 3: Create vendor_identity with referral in metadata
    ...
    // Create vendor_identity first (required for foreign key)
    await pool.query(
    ...
    // Step 4: Create application (approval endpoint needs this)
    ...
    console.log(`   ✅ Created vendor_identity: ${vendorIdentityId}`);
    console.log(`   Linked to application: ${applicationId}`);
    console.log(`   Metadata: ${JSON.stringify(metadata, null, 2)}\n`);

    // Verify metadata was stored correctly
    const verifyIdentity = await pool.query(
      `SELECT metadata, application_id FROM vendor_identity WHERE id = $1`,
      [vendorIdentityId]
    );
    ...
    // Step 6: Approve application via admin API
    ...
    // Step 7: Wait for referral processing
    console.log('6️⃣  Waiting for referral processing (5 seconds)...\n');
    await new Promise(resolve => setTimeout(resolve, 5000));

    // Step 8: Check Shreesha's wallet after
    ...
    // Step 9: Check referral record status
    ...
    // Step 10: Check recent transactions
    console.log('9️⃣  Checking recent wallet transactions...\n');
    const txResult = await makeRequest(`${API_BASE_URL}/vendor/${SHREESHA_VENDOR_IDENTITY_ID}/wallet/transactions?limit=10`);
    
    if (txResult.statusCode === 200 && txResult.response.success) {
      const recentTx = txResult.response.transactions?.filter(tx => {
        const txDate = new Date(tx.created_at);
        const now = new Date();
        return (now - txDate) < 120000; // Last 2 minutes
      });
      ...
      if (recentTx && recentTx.length > 0) {
        ...
        console.log(`   ⚠️  No recent transactions found in the last 2 minutes\n`);
      }
    }
