# Fast dev deploy - skips npm ci (node_modules must already exist)
...
function Deploy-App($Name, $Bucket, $CfId, $BuildEnv) {
  ...
  if (-not (Test-Path "node_modules\next")) {
    throw "node_modules missing for $Name - run npm ci once in apps/$Name"
  }
...
console.log('Lambda V2 AUTHORITATIVE');
"@ $envFile
Pop-Location
...
Write-Host "`nAll dev deploys complete (fast path)." -ForegroundColor Green
