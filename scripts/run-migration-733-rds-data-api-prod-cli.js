#!/usr/bin/env node
/**
 * Migration 733 (bookings_status_check + pending_payment) — PRODUCTION.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 * Dollar-quote–aware splitting for DO $$ blocks.
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_733=YES or the script exits without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_733='YES'; node scripts/run-migration-733-rds-data-api-prod-cli.js
 *
 * Usage (bash):
 *   I_CONFIRM_PROD_MIGRATION_733=YES node scripts/run-migration-733-rds-data-api-prod-cli.js
 *
...
/** Same as run-migration-722-rds-data-api-prod-cli.js */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_733_prod_${Date.now()}.json`);
  const payload = {
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_733 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_733=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 733 — bookings pending_payment status check (PRODUCTION, RDS Data API CLI)');
  ...
  console.log('🔍 Verify: bookings_status_check includes pending_payment...');
  const verifySql = `SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint WHERE conrelid = 'public.bookings'::regclass AND conname = 'bookings_status_check' LIMIT 1`;
  try {
