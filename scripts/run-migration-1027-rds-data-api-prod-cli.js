#!/usr/bin/env node
/**
 * Migration 1027 — meal_refund_cases Phase 3 (PROD only, RDS Data API).
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_1027='YES'
 *   node scripts/run-migration-1027-rds-data-api-prod-cli.js
 */
...
const MIGRATION_FILE = path.join(__dirname, '..', 'db', 'migrations', '1027_meal_refund_cases_phase3.sql');
...
function executeStatementCli(clusterArn, secretArn, database, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_1027_${Date.now()}_${Math.random().toString(36).slice(2)}.json`);
  fs.writeFileSync(
    inputFile,
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_1027 !== 'YES') {
    console.error('Refusing to run. Set: I_CONFIRM_PROD_MIGRATION_1027=YES');
    process.exit(1);
  }
  ...
  console.log('PROD migration 1027_meal_refund_cases_phase3 (RDS Data API)\n');
  ...
  const verifySql =
    "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'meal_refund_cases' AND column_name = 'refund_requested_at'";
  const inputFile = path.join(__dirname, `_tmp_rds_verify_1027_${Date.now()}.json`);
  fs.writeFileSync(
    inputFile,
    JSON.stringify({
  ...
  try {
    ...
    console.log(
      'Verify:',
      out.includes('refund_requested_at') ? 'refund_requested_at column present' : out.slice(0, 400)
    );
  } finally {
    try {
      fs.unlinkSync(inputFile);
