/**
 * Apply db/migrations/715_support_categories_escalation_rules.sql on DEV + PROD via RDS Data API.
 */
const fs = require('fs');
...
async function main() {
  const sqlPath = path.join(__dirname, '..', 'db', 'migrations', '715_support_categories_escalation_rules.sql');
  ...
  console.log('715 support_categories + support_escalation_rules applied on dev + prod.');
}
