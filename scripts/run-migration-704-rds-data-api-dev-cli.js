#!/usr/bin/env node
/**
 * Migration 704 — referral_redemptions (unique referred_id, one code many referees) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-704-rds-data-api-dev-cli.js
 *
...
/** Same algorithm as run-migration-701-rds-data-api-dev-cli.js */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_704_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 704 — referral_redemptions (referred_id UNIQUE) (DEV, RDS Data API CLI)');
