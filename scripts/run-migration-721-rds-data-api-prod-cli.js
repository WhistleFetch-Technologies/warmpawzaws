#!/usr/bin/env node
/**
 * Migration 721 (vendor_admin_portal_codes) — PRODUCTION.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_721=YES or the script exits without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_721='YES'; node scripts/run-migration-721-rds-data-api-prod-cli.js
 */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_721_prod_input.json');
  const payload = {
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_721 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_721=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 721 — vendor_admin_portal_codes (PRODUCTION, RDS Data API CLI)');
