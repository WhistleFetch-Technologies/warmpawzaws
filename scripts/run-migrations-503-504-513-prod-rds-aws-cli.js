#!/usr/bin/env node
/**
 * Run diagnostic_tests alignment migrations against PROD via AWS CLI:
 *   aws rds-data execute-statement (one statement per call).
 *
 * Files (order): 503 → 504 → 513
 *   - db/migrations/503_add_diagnostic_tests_columns.sql
 *   - db/migrations/504_add_diagnostic_tests_category_column.sql
 *   - db/migrations/513_diagnostic_tests_service_style.sql
 *
 * Prerequisites: AWS CLI v2, credentials, RDS Data API enabled on prod cluster.
 *
 * Usage:
 *   node scripts/run-migrations-503-504-513-prod-rds-aws-cli.js
 *
...
const FILES = [
  path.join(ROOT, 'db', 'migrations', '503_add_diagnostic_tests_columns.sql'),
  path.join(ROOT, 'db', 'migrations', '504_add_diagnostic_tests_category_column.sql'),
  path.join(ROOT, 'db', 'migrations', '513_diagnostic_tests_service_style.sql'),
];
...
function main() {
  console.log('PROD RDS Data API — diagnostic_tests migrations 503, 504, 513 (AWS CLI)');
  ...
  console.log('\nDone. Migrations 503, 504, 513 executed against prod.');
}
