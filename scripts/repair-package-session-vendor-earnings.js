...
const USE_DATA_API =
  process.argv.includes('--data-api') || process.env.USE_RDS_DATA_API === '1';

const SELECT_INFLATED = `
      SELECT
        ve.id,
        ve.vendor_id,
        ve.booking_id,
...
async function runViaDataApi() {
  ...
  const rows = await query(SELECT_INFLATED);
  const updates = computeUpdates(rows);
  if (updates.length === 0) {
  ...
  logUpdates(updates);
  if (!APPLY) {
    console.log('Dry-run only. Re-run with --apply --data-api to write.');
    return;
  }
  ...
  const cols = await query(`
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name IN ('pending_payout', 'total_earnings')
  `);
  const colNames = new Set(cols.map((c) => c.column_name));
  const canUpdateVendorTotals =
    colNames.has('pending_payout') && colNames.has('total_earnings');
  ...
  try {
    for (const u of updates) {
      await client.send(
        new ExecuteStatementCommand({
          ...
          database: 'warmpawz',
          transactionId,
          sql: `UPDATE vendor_earnings
         SET amount = ${sqlLiteral(u.to)}::numeric,
             commission_amount = ${sqlLiteral(u.commission)}::numeric,
             total_amount = ${sqlLiteral(u.gross)}::numeric
         WHERE id = ${sqlLiteral(u.id)}::uuid AND status = 'pending'`,
        })
      );
    }
    if (canUpdateVendorTotals) {
      for (const [vendorId, delta] of vendorDeltas(updates)) {
        await client.send(
          new ExecuteStatementCommand({
            resourceArn: clusterInfo.clusterArn,
            secretArn: clusterInfo.secretArn,
            ...
            transactionId,
            sql: `UPDATE vendors
         SET pending_payout = GREATEST(COALESCE(pending_payout, 0) + ${sqlLiteral(delta)}, 0),
             total_earnings = GREATEST(COALESCE(total_earnings, 0) + ${sqlLiteral(delta)}, 0),
             updated_at = NOW()
         WHERE id = ${sqlLiteral(vendorId)}::uuid`,
          })
        );
      }
    } else {
      console.log('Skipping vendors.pending_payout/total_earnings (columns missing).');
    }
    await client.send(
    ...
    console.log(`Applied ${updates.length} row update(s) via Data API.`);
  } catch (err) {
    try {
      await client.send(
...
async function runViaPg() {
  ...
  try {
    const { rows } = await client.query(SELECT_INFLATED);
    const updates = computeUpdates(rows);
    if (updates.length === 0) {
      console.log('No inflated package session earnings found.');
      return;
...
async function main() {
  console.log(
    `Repair package session earnings (${ENVIRONMENT}) ${APPLY ? 'APPLY' : 'DRY-RUN'}${USE_DATA_API ? ' [data-api]' : ''}`
  );
  try {
    if (USE_DATA_API) {
      await runViaDataApi();
