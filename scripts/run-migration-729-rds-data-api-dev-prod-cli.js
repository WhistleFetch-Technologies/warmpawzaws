#!/usr/bin/env node
/**
 * Migration 729 — package catalog session sync + purchase repair (DEV and/or PROD).
 * Runs db/migrations/729_package_catalog_session_sync_and_purchase_repair.sql via:
 *   aws rds-data execute-statement --cli-input-json file://...
 *
 * Uses dollar-quote–aware splitting (same as run-migration-722-rds-data-api-prod-cli.js).
 *
 * Usage (PowerShell, both environments):
 *   $env:I_CONFIRM_MIGRATION_729='YES'
 *   $env:MIGRATION_729_TARGET='both'   # dev | prod | both (default: both)
 *   node scripts/run-migration-729-rds-data-api-dev-prod-cli.js
 *
 * Usage (bash):
 *   I_CONFIRM_MIGRATION_729=YES MIGRATION_729_TARGET=both node scripts/run-migration-729-rds-data-api-dev-prod-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '729_package_catalog_session_sync_and_purchase_repair.sql'
);
...
function executeStatementCli(clusterArn, secretArn, database, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_729_${Date.now()}.json`);
  const payload = {
...
function resolveTargets() {
  const raw = (process.env.MIGRATION_729_TARGET || 'both').toLowerCase().trim();
  if (raw === 'dev') return ['dev'];
  if (raw === 'prod') return ['prod'];
  if (raw === 'both') return ['dev', 'prod'];
  throw new Error(`Invalid MIGRATION_729_TARGET="${raw}". Use dev | prod | both.`);
}

function main() {
  if (process.env.I_CONFIRM_MIGRATION_729 !== 'YES') {
    console.error(
      'Refusing to run. Set exactly:\n' +
        '  I_CONFIRM_MIGRATION_729=YES\n' +
        'Optional: MIGRATION_729_TARGET=dev|prod|both (default both)\n' +
        'Then re-run.'
    );
    process.exit(1);
  }
  ...
  console.log('============================================================================');
  console.log('MIGRATION 729 — catalog session sync + purchase repair (RDS Data API)');
