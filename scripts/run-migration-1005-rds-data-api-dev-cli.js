#!/usr/bin/env node
/**
 * Migration 1005 (customers latitude/longitude) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-1005-rds-data-api-dev-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '1005_customers_latitude_longitude.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_1005_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 1005 — customers latitude/longitude (DEV ONLY, RDS Data API CLI)');
