...
async function testOtpVerificationWithReferral() {
  try {
    console.log('═══════════════════════════════════════════════════════════════');
    ...
    if (metadata.referral_code_id || metadata.referral_code) {
      ...
      await pool.end();
      return { success: true, vendorIdentityId: vi.id };
    } else {
...
async function runMultipleTests() {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  TESTING OTP VERIFICATION WITH REFERRAL CODE (10 TIMES)');
  console.log('═══════════════════════════════════════════════════════════════\n');
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    const result = await testOtpVerificationWithReferral();
    results.push(result);
  ...
  if (successful >= 8) {
    ...
    console.log('❌ FAILURE: Flow needs fixes\n');
    console.log('Failed reasons:');
    results.forEach((r, i) => {
      if (!r.success) {
        console.log(`  Test ${i + 1}: ${r.reason}`);
      }
    });
  }
}
