#!/usr/bin/env node
/**
 * Migration 1028 — pets.vaccination_records + microchip_id (PROD only).
 * Runs via RDS Data API (no VPC tunnel required when HttpEndpointEnabled).
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_1028='YES'
 *   node scripts/run-migration-1028-rds-data-api-prod-cli.js
 */
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '1028_pets_vaccination_records_microchip.sql'
);
...
function executeStatementCli(clusterArn, secretArn, database, sql) {
  const inputFile = path.join(__dirname, `_tmp_rds_data_1028_${Date.now()}_${Math.random().toString(36).slice(2)}.json`);
  fs.writeFileSync(
    inputFile,
...
function verifyColumns(clusterArn, secretArn, dbName) {
  const sql = `
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'pets'
  AND column_name IN ('vaccination_records', 'microchip_id')
ORDER BY column_name`;
  const inputFile = path.join(__dirname, `_tmp_rds_verify_1028_${Date.now()}.json`);
  fs.writeFileSync(
    inputFile,
    JSON.stringify({
  ...
  try {
    ...
    console.log('\nVerify pets columns (expect 2 rows):');
    ...
    if (records.length < 2) {
      throw new Error('Verification failed: expected vaccination_records and microchip_id on pets');
    }
  } finally {
    try {
      fs.unlinkSync(inputFile);
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_1028 !== 'YES') {
    console.error('Refusing to run. Set: I_CONFIRM_PROD_MIGRATION_1028=YES');
    process.exit(1);
  }
  ...
  console.log('============================================================================');
  console.log('PROD migration 1028 — pets vaccination_records + microchip_id (RDS Data API)');
  ...
  console.log(`--- ${base} (${statements.length} statement(s)) ---\n`);
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      ...
      if (/already exists|duplicate key|duplicate_object|column .* already exists/i.test(msg)) {
  ...
  verifyColumns(clusterArn, secretArn, dbName);

  console.log('\n============================================================================');
  console.log(`Done: ${totalOk}/${statements.length} statement(s) on PROD.`);
  console.log('============================================================================');
}
