#!/usr/bin/env node
/**
 * Migration 753 (booking-service JPA columns) — DEV only.
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   node scripts/run-migration-753-rds-data-api-dev-cli.js
 *
 * Requires: AWS CLI v2, credentials, RDS Data API enabled on dev cluster.
 */
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_753_dev_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 753 — booking-service JPA columns (DEV ONLY, RDS Data API CLI)');
