#!/usr/bin/env node
/**
 * Migration 701 (GST catalogue category + tax_category_roles + HSN nullable gst_rate) — DEV only.
 ...
 *
 * Usage:
 *   node scripts/run-migration-701-rds-data-api-dev-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '701_gst_catalog_category_tax_roles.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_701_input.json');
  const payload = {
...
function main() {
  console.log('============================================================================');
  console.log('MIGRATION 701 — GST catalog category + roles (DEV ONLY, RDS Data API CLI)');
