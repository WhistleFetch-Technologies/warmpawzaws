/**
 * Apply db/migrations/716_support_sla_config.sql on DEV + PROD via RDS Data API.
 */
const fs = require('fs');
...
async function main() {
  const sqlPath = path.join(__dirname, '..', 'db', 'migrations', '716_support_sla_config.sql');
  ...
  console.log('716 support_sla_config applied on dev + prod.');
}
