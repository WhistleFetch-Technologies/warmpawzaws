#!/usr/bin/env node
/**
 * Migration 1004 (customers username + password auth columns) — PRODUCTION.
 ...
 *
 * SAFETY: Set I_CONFIRM_PROD_MIGRATION_1004=YES or the script exits with code 1 without running.
 *
 * Usage (PowerShell):
 *   $env:I_CONFIRM_PROD_MIGRATION_1004='YES'; node scripts/run-migration-1004-rds-data-api-prod-cli.js
 *
 * Optional overrides: AWS_REGION, CLUSTER_IDENTIFIER, SECRET_NAME, DATABASE_NAME env vars.
 */
...
const CLUSTER_IDENTIFIER = process.env.CLUSTER_IDENTIFIER || 'warmpawz-prod-cluster';
const SECRET_NAME =
  process.env.SECRET_NAME || 'warmpawz-prod-rds-master-20260207201049162400000001';
const DATABASE_NAME = process.env.DATABASE_NAME || 'warmpawz';
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '1004_customer_username_password_auth.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_1004_prod_input.json');
  const payload = {
...
function main() {
  if (process.env.I_CONFIRM_PROD_MIGRATION_1004 !== 'YES') {
    console.error(
      'Refusing to run on PROD. Set environment variable exactly:\n' +
        '  I_CONFIRM_PROD_MIGRATION_1004=YES\n' +
  ...
  console.log('============================================================================');
  console.log('MIGRATION 1004 — customer username/password columns (PRODUCTION, RDS Data API CLI)');
