#!/usr/bin/env node
/**
 * Migration 631 — medical_records.created_by / created_by_role (PROD, RDS Data API).
 * Runs each statement via: aws rds-data execute-statement --cli-input-json file://...
 *
 * Usage:
 *   APPLY_PROD_631=yes node scripts/run-migration-631-rds-data-api-prod-cli.js
 *
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  '631_add_medical_records_created_by_columns.sql'
);
...
function executeStatementCli(clusterArn, secretArn, sql) {
  const inputFile = path.join(__dirname, '_tmp_rds_data_631_prod_input.json');
  const payload = {
...
function main() {
  if (process.env.APPLY_PROD_631 !== 'yes') {
    console.error('Refusing to run on prod without APPLY_PROD_631=yes');
    process.exit(1);
  }

  console.log('============================================================================');
  console.log('MIGRATION 631 — medical_records created_by / created_by_role (PROD, CLI)');
