...
const MIGRATION_FILE = path.join(__dirname, '..', 'db', 'migrations', '605_fix_events_category_constraint.sql');
...
async function runMigration() {
  console.log('============================================================================');
  console.log('MIGRATION 605: Fix events category check constraint (PROD)');
  console.log('============================================================================\n');
  ...
  if (verifyResult.success && verifyResult.response.records && verifyResult.response.records.length > 0) {
    console.log('✅ Constraint events_category_check exists');
    const constraintDef = verifyResult.response.records[0][1]?.stringValue || 'unknown';
    console.log(`   Definition: ${constraintDef}`);
  } else {
    console.log('⚠️  Constraint events_category_check NOT found');
  }

  console.log('\n============================================================================');
  if (errorCount === 0) {
    console.log('✅ MIGRATION 605 COMPLETED SUCCESSFULLY (PROD)');
  } else {
    console.log('⚠️  MIGRATION 605 COMPLETED WITH ERRORS (PROD)');
  }
  console.log('============================================================================\n');
}
