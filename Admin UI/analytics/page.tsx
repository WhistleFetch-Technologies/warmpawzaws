	...
	const generateReport = async (reportId: string) => {
		try {
			const response = await fetch(
				`${API_BASE}/admin/reports/${reportId}/generate`,
				{
					method: "POST",
					headers: {
		...
		try {
			...
			if (revenueData && revenueData.length > 0) {
				...
				revenueData.forEach((item) => {
					csvRows.push(
						`${item.date},₹${item.revenue},₹${item.commission},${item.count}`
					);
				});
				csvRows.push("");
			}
			...
			if (vendorData && vendorData.length > 0) {
				...
				vendorData.slice(0, 20).forEach((vendor) => {
					csvRows.push(
						`${vendor.name},₹${vendor.revenue},${vendor.bookings},${vendor.rating}`
					);
				});
			}
									...
									<TableBody>
										{(categoryData || []).map((cat: any, idx: number) => (
											<TableRow key={idx}>
												...
												<TableCell>
													₹{((cat.revenue || 0) / (cat.count || 1)).toFixed(0)}
												</TableCell>
												<TableCell>
