...
import { RegionActivePackagesTab } from "@/components/regionManager";
...
export default function RegionManager({ onBack }: RegionManagerProps = {}) {
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 404) {
					console.warn(
						"🌍 [REGION] Regions endpoint not found, initializing with empty array"
					);
					setRegions([]);
					toast.info(
			...
			console.log(
				"🌍 [REGION] Regions data type:",
				Array.isArray(regionsData) ? "array" : typeof regionsData
			);
			...
			if (regionsData.length > 0 || data.success) {
				...
				if (uniqueRegions.length === 0 && !autoInitAttempted) {
					console.log(
						"🇮🇳 No regions found - attempting one-time auto-initialization..."
					);
					...
					console.warn(
						"⚠️ Auto-initialization already attempted. KV store may be having issues."
					);
	...
	const handleCreateFromTemplate = async (templateId: string) => {
		try {
			...
			if (data.success) {
				toast.success(
					data.message ||
						`${templateId.toUpperCase()} region created successfully!`
				);
				await loadRegions();
				setView("list");
			} else {
						...
						<Card className="p-6">
							...
							<Tabs defaultValue="basic" className="w-full">
								<TabsList className="grid w-full grid-cols-7">
									<TabsTrigger key="basic" value="basic">
										Basic
									</TabsTrigger>
									<TabsTrigger key="currency" value="currency">
										Currency
									</TabsTrigger>
									<TabsTrigger key="phone" value="phone">
										Phone
									</TabsTrigger>
									<TabsTrigger key="localization" value="localization">
										Localization
									</TabsTrigger>
									<TabsTrigger key="services" value="services">
										Services
									</TabsTrigger>
									<TabsTrigger key="breeds" value="breeds">
										Breeds
									</TabsTrigger>
