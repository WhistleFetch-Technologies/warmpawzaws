			...
			if (matches) {
				if (commission.type === "percentage") {
					...
					if (commission.minAmount)
						amount = Math.max(amount, commission.minAmount);
					if (commission.maxAmount)
						amount = Math.min(amount, commission.maxAmount);
					return amount;
			...
			<div className="bg-white rounded-xl border border-gray-200 p-6">
				...
				<div className="max-w-md">
					...
					<p className="text-xs text-gray-500 mt-2">
						Applied when no specific rule matches. This is the fallback
						commission rate.
					</p>
				</div>
			</div>
									...
									<div className="space-y-1 text-xs text-gray-600">
										{tier.requirements.minMonthlyOrders && (
											<div className="flex items-center gap-1">
												...
												<span>
													{tier.requirements.minMonthlyOrders}+ orders/month
												</span>
											</div>
										)}
										...
										{tier.requirements.minRevenue && (
											<div className="flex items-center gap-1">
												<Check className="w-3 h-3" />
												<span>
													₹{tier.requirements.minRevenue.toLocaleString()}+
													revenue
												</span>
											</div>
										)}
										...
										<div className="p-4 border-t border-gray-200 bg-white">
											<div className="grid grid-cols-2 gap-4">
												...
												<div>
													...
													<div className="space-y-2 text-sm">
														{rule.commission.type === "tiered" &&
															rule.commission.tiers && (
																<div>
					...
					<div className="grid grid-cols-2 gap-4">
						<div>
							...
							<select
								...
								onChange={(e) =>
									setEditedRule({ ...editedRule, type: e.target.value as any })
								}
								className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF8C42]"
							>
