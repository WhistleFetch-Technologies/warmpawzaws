...
import { Button, Checkbox, Input, Label, Switch } from "@repo/ui";
...
const VENDOR_TYPES = [
	...
	{ id: "sunset-services", name: "Sunset Services", icon: "🌅" },
	{
		...
		name: "Healthcare Service Provider",
		icon: "🏥",
	},
	{
		id: "pet-products-seller",
		name: "Food, Medicine & Pet Products Sellers",
		icon: "🛒",
	},
];
...
export function PaymentRulesSection() {
	...
	const handleSaveRule = async () => {
		...
		try {
			...
			if (response.ok) {
				toast.success(
					isCreatingNew ? "Payment rule created!" : "Payment rule updated!"
				);
				await loadData();
				setEditingRule(null);
	...
	const toggleVendorType = (vendorTypeId: string) => {
		...
		if (updated.vendorTypes.includes(vendorTypeId)) {
			updated.vendorTypes = updated.vendorTypes.filter(
				(id) => id !== vendorTypeId
			);
		} else {
			...
			<div className="flex items-center justify-between pb-4 border-b border-gray-200">
				<div>
					...
					<p className="text-sm text-gray-500">
						Configure payment rules per vendor type & service location
					</p>
				</div>
					...
					<div className="mb-6">
						...
						<div className="grid grid-cols-3 gap-4">
							<div
								...
								onClick={() =>
									setEditingRule({ ...editingRule, serviceLocation: "at_home" })
								}
							>
								<div
							...
							<div
								...
								onClick={() =>
									setEditingRule({ ...editingRule, serviceLocation: "both" })
								}
							>
								<div
									className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center text-lg ${
					...
					<div className="mb-6">
						<div className="flex items-center justify-between mb-3">
							<Label className="text-sm font-medium">
								Applicable Vendor Types ({editingRule.vendorTypes.length}{" "}
								selected)
							</Label>
							<Button
								size="sm"
						...
						<div className="grid grid-cols-3 gap-3 border border-gray-200 rounded-lg p-4 bg-white max-h-80 overflow-y-auto">
							{VENDOR_TYPES.map((vendorType) => {
								const isSelected = editingRule.vendorTypes.includes(
									vendorType.id
								);
					...
					<div className="mb-6">
						...
						<div className="grid grid-cols-3 gap-4">
							<div
								...
								onClick={() =>
									setEditingRule({ ...editingRule, reservationType: "flat" })
								}
							>
								<div
									className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center text-lg ${
							...
							<div
								...
								onClick={() =>
									setEditingRule({ ...editingRule, reservationType: "full" })
								}
							>
								<div
									className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center text-lg ${
								...
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setEditingRule({
												...editingRule,
												reservationPercentage: parseFloat(e.target.value) || 0,
											})
										}
										className="h-10"
										min="0"
								...
								<div>
									...
									<Input
										...
										onChange={(e) =>
											setEditingRule({
												...editingRule,
												minimumAdvancePayment: parseFloat(e.target.value) || 0,
											})
										}
										className="h-10"
										min="0"
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setEditingRule({
										...editingRule,
										escrowHoldPeriodHours: parseFloat(e.target.value) || 0,
									})
								}
								className="h-10"
								min="0"
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setEditingRule({
										...editingRule,
										travelSurchargePerKm: parseFloat(e.target.value) || 0,
									})
								}
								className="h-10"
								min="0"
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setEditingRule({
										...editingRule,
										equipmentFee: parseFloat(e.target.value) || 0,
									})
								}
								className="h-10"
								min="0"
			...
			{!editingRule && (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<h3 className="font-semibold text-gray-900">
							Active Payment Rules ({rules.length})
						</h3>
					</div>
										...
										<div>
											<div className="flex items-center gap-2 mb-2">
												...
												<span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
													...
													{rule.serviceLocation === "at_center" &&
														"🏢 At Center"}
												...
												<span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full font-medium">
													...
													{rule.reservationType === "percentage" &&
														"% Percentage"}
											...
											<div className="flex flex-wrap gap-2">
												{(rule.vendorTypes || []).map((vtId) => {
													const vendorType = VENDOR_TYPES.find(
														(vt) => vt.id === vtId
													);
													return vendorType ? (
														<span
															key={vtId}
															className="px-2 py-1 bg-orange-100 text-[#FF8C42] text-xs rounded-full font-medium flex items-center gap-1"
														>
									...
									<div className="grid grid-cols-4 gap-4 text-sm">
										<div>
											...
											<span className="ml-2 font-medium">
												{rule.reservationType === "flat" &&
													`₹${rule.flatAmount}`}
												{rule.reservationType === "percentage" &&
													`${rule.reservationPercentage}%`}
										...
										<div>
											...
											<span className="ml-2 font-medium">
												{rule.escrowHoldPeriodHours}h
											</span>
										</div>
										<div>
											<span className="text-gray-500">Auto-Capture:</span>
											<span
												className={`ml-2 font-medium ${rule.autoCapturePayment ? "text-green-600" : "text-gray-400"}`}
											>
										...
										<div>
											...
											<span
												className={`ml-2 font-medium ${rule.isActive ? "text-green-600" : "text-gray-400"}`}
											>
