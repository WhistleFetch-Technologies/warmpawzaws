...
export function PaymentGatewaySettings({
	...
	const toggleGatewayStatus = async (gatewayId: string, enabled: boolean) => {
		try {
			...
			setGateways(
				gateways.map((g) => (g.id === gatewayId ? { ...g, enabled } : g))
			);
									...
									<TabsContent value="config" className="space-y-6 mt-6">
										...
										<div className="space-y-4">
											...
											<div>
												...
												<div className="flex gap-2 mt-2">
													<Input
														type={
															showSecrets["apiSecret"] ? "text" : "password"
														}
										...
										<div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
											...
											<div className="flex-1">
												...
												<p className="text-xs text-gray-500">
													Make this gateway available for transactions
												</p>
											</div>
											<Switch
									...
									<TabsContent value="fees" className="space-y-6 mt-6">
											...
											<div>
												...
												<Input
													...
													onChange={(e) =>
														setSelectedGateway({
															...selectedGateway,
															feeFixed: parseFloat(e.target.value),
														})
													}
													disabled={!editMode}
													className="mt-2"
										...
										<div className="p-4 bg-blue-50 rounded-lg">
											...
											<p className="text-sm text-gray-600">
												...
												<br />
												Fee = (₹1,000 × {selectedGateway.feePercentage}%) + ₹
												{selectedGateway.feeFixed}
												<br />
												<span className="font-semibold">
													Total Fee = ₹
													{(
														(1000 * selectedGateway.feePercentage) / 100 +
														selectedGateway.feeFixed
													).toFixed(2)}
												</span>
											</p>
									...
									<TabsContent value="regions" className="space-y-6 mt-6">
												...
												{[
													...
													<div
														...
														className={`p-3 border-2 rounded-lg cursor-pointer capitalize ${
															selectedGateway.regions.includes(region) ||
															selectedGateway.regions.includes("all")
																? "border-[#FF8C42] bg-orange-50"
																: "border-gray-200"
														}`}
														onClick={() => {
															if (!editMode) return;
															const regions = selectedGateway.regions.includes(
																region
															)
																? selectedGateway.regions.filter(
																		(r) => r !== region
																	)
										...
										<div className="p-4 bg-gray-50 rounded-lg">
											...
											<span className="text-sm text-gray-600">
												Customers in selected regions will see this gateway as a
												payment option
											</span>
										</div>
									</TabsContent>
									...
									<TabsContent value="webhooks" className="space-y-6 mt-6">
										<div>
											...
											<div className="flex gap-2 mt-2">
												<Input
													type={
														showSecrets["webhookSecret"] ? "text" : "password"
													}
												...
												<Button
													...
													variant="outline"
													onClick={() =>
														toggleSecretVisibility("webhookSecret")
													}
												>
										...
										<div>
											...
											<p className="text-xs text-gray-500 mt-2">
												Configure this URL in your {selectedGateway.provider}{" "}
												dashboard
											</p>
										</div>
										...
										<div className="p-4 bg-yellow-50 rounded-lg">
											...
											<span className="text-sm text-gray-600">
												Webhooks are required for payment confirmation and
												refund processing
											</span>
										</div>
									</TabsContent>
