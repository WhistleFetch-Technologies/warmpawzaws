...
export function RefundPoliciesSection() {
	...
	const handleSaveTier = async () => {
		...
		if (
			editingTier.refundPercentage < 0 ||
			editingTier.refundPercentage > 100
		) {
		...
		try {
			...
			if (response.ok) {
				toast.success(
					isCreatingNew ? "Refund tier created!" : "Refund tier updated!"
				);
				await loadData();
				setEditingTier(null);
			...
			<div className="flex items-center justify-between pb-4 border-b border-gray-200">
				<div>
					...
					<p className="text-sm text-gray-500">
						Configure cancellation and refund tiers per vendor type & location
					</p>
				</div>
					...
					<div className="mb-6">
						...
						<div className="grid grid-cols-3 gap-4">
							<div
								...
								onClick={() =>
									setEditingTier({ ...editingTier, serviceLocation: "at_home" })
								}
							>
								<div
							...
							<div
								...
								onClick={() =>
									setEditingTier({ ...editingTier, serviceLocation: "both" })
								}
							>
								<div
									className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center text-lg ${
					...
					<div className="mb-6">
						<div className="flex items-center justify-between mb-3">
							<Label className="text-sm font-medium">
								Applicable Vendor Types ({editingTier.vendorTypes.length}{" "}
								selected)
							</Label>
							<Button
								size="sm"
						...
						<div className="grid grid-cols-3 gap-3 border border-gray-200 rounded-lg p-4 bg-white max-h-80 overflow-y-auto">
							{VENDOR_TYPES.map((vendorType) => {
								const isSelected = editingTier.vendorTypes.includes(
									vendorType.id
								);
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setEditingTier({
										...editingTier,
										hoursBeforeService: parseFloat(e.target.value) || 0,
									})
								}
								className="h-10"
								min="0"
							...
							<p className="text-xs text-gray-500 mt-1">
								Cancellation window before service starts
							</p>
						</div>
						<div>
							<Label className="text-sm mb-2 font-medium">
							...
							/>
							<p className="text-xs text-gray-500 mt-1">
								Percentage of amount to refund
							</p>
						</div>
						<div>
							<Label className="text-sm mb-2 font-medium">
							...
							/>
							<p className="text-xs text-gray-500 mt-1">
								Fixed cancellation fee (0 for none)
							</p>
						</div>
					</div>
					...
					<div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
						...
						<p className="text-sm text-blue-800">
							...
							<span className="font-bold">
								{editingTier.hoursBeforeService} hours
							</span>{" "}
							before service, they will receive{" "}
							<span className="font-bold">
								{editingTier.refundPercentage}% refund
							</span>
							{editingTier.cancellationFee > 0 && (
								<>
									...
									minus a{" "}
									<span className="font-bold">
										₹{editingTier.cancellationFee} cancellation fee
									</span>
								</>
							)}
							.
							...
							<span className="text-xs">
								...
								{(1000 * editingTier.refundPercentage) / 100 -
									editingTier.cancellationFee}{" "}
								refunded
							</span>
						</p>
					</div>
			...
			{!editingTier && (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<h3 className="font-semibold text-gray-900">
							Active Refund Tiers ({tiers.length})
						</h3>
					</div>
					...
					{tiers.length === 0 ? (
						<div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
							...
							<p className="text-gray-500 mb-4">
								No refund tiers configured yet
							</p>
							<Button
								onClick={handleCreateTier}
										...
										<div>
											<div className="flex items-center gap-2 mb-2">
												...
												<span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
													...
													{tier.serviceLocation === "at_center" &&
														"🏢 At Center"}
													{tier.serviceLocation === "both" &&
														"🌐 Both Locations"}
												</span>
											</div>
									...
									<div className="grid grid-cols-4 gap-4 text-sm">
										<div>
											...
											<span className="ml-2 font-medium">
												{tier.hoursBeforeService}h before
											</span>
										</div>
										<div>
										...
										<div>
											...
											<span className="ml-2 font-medium">
												₹{tier.cancellationFee}
											</span>
										</div>
										<div>
											<span className="text-gray-500">Status:</span>
											<span
												className={`ml-2 font-medium ${tier.isActive ? "text-green-600" : "text-gray-400"}`}
											>
