...
export function SettlementScheduleSettings() {
	...
	const handleProcessNow = async () => {
		...
		try {
			const response = await fetch(
				`${API_BASE}/admin/finance/process-settlements`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
					...
					<CardHeader>
						<div className="flex items-center gap-2">
							...
							<div>
								...
								<CardDescription>
									Set when automatic payouts should run
								</CardDescription>
							</div>
						</div>
					</CardHeader>
					<CardContent className="space-y-6">
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(c) =>
									setSettings({ ...settings, enabled: c })
								}
							/>
						</div>
						...
						<div className="space-y-4">
							<div className="space-y-2">
								...
								<Select
									...
									onValueChange={(value: any) =>
										setSettings({ ...settings, scheduleType: value })
									}
								>
									<SelectTrigger>
										<SelectValue />
							...
							settings.scheduleType === "biweekly" ? (
								<div className="space-y-2">
									<Label>Day of Week</Label>
									<Select
										value={String(settings.scheduleDay)}
										onValueChange={(value) =>
											setSettings({ ...settings, scheduleDay: parseInt(value) })
										}
									>
										<SelectTrigger>
											<SelectValue />
					...
					<CardHeader>
						<div className="flex items-center gap-2">
							...
							<div>
								...
								<CardDescription>
									Configure payout thresholds and periods
								</CardDescription>
							</div>
						</div>
					</CardHeader>
					<CardContent className="space-y-6">
						<div className="space-y-4">
							<div className="space-y-2">
								...
								<div className="relative">
									<Input
										...
										onChange={(e) =>
											setSettings({
												...settings,
												settlementPeriodDays: parseInt(e.target.value) || 0,
											})
										}
										className="pr-12"
									/>
								...
								<p className="text-xs text-slate-500">
									Bookings completed T+{settings.settlementPeriodDays} days ago
									will be eligible for payout
								</p>
							</div>
							...
							<div className="flex items-center justify-between">
								...
								<Switch
									...
									onCheckedChange={(c) =>
										setSettings({ ...settings, autoProcess: c })
									}
								/>
							</div>
						</div>
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg">
							<div
								className={`p-2 rounded-lg ${settings.enabled ? "bg-green-100" : "bg-gray-100"}`}
							>
								{settings.enabled ? (
