...
export function SettlementDashboard() {
	...
	const loadData = async () => {
		...
		try {
			...
			const settlementsRes = await fetch(
				`${API_BASE}/admin/payments/settlements`,
				{
	...
	const handleProcessSettlements = async () => {
		const dueSettlements = settlements
		...
		try {
			const response = await fetch(
				`${API_BASE}/admin/payments/settlements/process`,
				{
					method: "POST",
					headers: {
			...
			if (response.ok) {
				toast.success(
					`Processed ${dueSettlements.length} settlements successfully`
				);
				// Refresh data
				loadData();
			} else {
	...
	const pieData = analytics
		? Object.entries(analytics.revenueByTier).map(([name, value]) => ({
				name,
				value,
			}))
		: [];
	...
	return (
		<div className="space-y-6 w-full max-w-full">
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
				<div>
					...
					<p className="text-sm text-slate-500">
						Manage payouts and view revenue insights
					</p>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
				...
				<Card className="bg-linear-to-br from-orange-50 to-amber-50 border-orange-100">
					<CardContent className="p-6">
						<div className="flex justify-between items-start">
							<div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
				...
				{/* Revenue Charts */}
				<Card className="lg:col-span-1 w-full max-w-full">
					<CardHeader>
						<CardTitle className="text-lg">Revenue by Tier</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="min-h-50 md:min-h-62.5 w-full">
