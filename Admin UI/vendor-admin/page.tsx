...
import {
	...
	RequestInfoModal,
	AddVendorModal,
} from "@/components/vendor-administration";
		...
		try {
			...
			try {
				...
				if (healthResponse.ok) {
					...
					console.warn(
						"⚠️ Health check failed with status:",
			...
			if (allVendorsResponse.ok) {
				...
				console.log(
					...
					allVendorsData.vendors?.length || 0
				);
			...
			console.error(
				"Error type:",
				error instanceof TypeError ? "Network/Fetch Error" : "Other Error"
			);
	...
	const resetAndSeedVendors = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				alert(
					`✅ Success!\n\nCleared: ${
						data.results.cleared.deleted?.profiles || 0
					} vendors\nCreated: ${
						data.results.seeded.success
					} new vendors\n\nRefreshing page...`
				);
	...
	const handleSeedVendors = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				alert(
					"❌ Failed to seed vendor data. Check console for details.\n\nMake sure role configurations exist first."
				);
			}
		} catch (error) {
