	...
	const filteredVendors = vendors.filter((vendor) => {
		...
		if (performanceFilter !== "all") {
			...
			if (
				...
				(vendor.rating < 3.5 || vendor.rating >= 4.5)
			)
				return false;
		...
		<div>
			<div className="mb-4">
				<div className="text-sm text-gray-600 mb-4">
					Manage Vendors Active Right Now
				</div>
					...
					filteredVendors.map((vendor) => (
						...
						>
							<div className="col-span-3">
								<div className="flex items-center gap-2 mb-1">
									...
									<div className="text-sm">
										{vendor.name} |{" "}
										<span className={getTierColor(vendor.tierColor)}>
											{vendor.tier}
										</span>
									</div>
								</div>
							...
							<div className="col-span-2">
								<span
									className={`inline-block px-3 py-1 text-xs rounded-full border ${getCategoryColor(vendor.category)}`}
								>
									{vendor.category}
								</span>
							</div>
