...
export function EnhancedPendingApplicationsTab({
	onViewDetails,
}: EnhancedPendingApplicationsTabProps) {
	const [activeStatusTab, setActiveStatusTab] =
		useState<StatusTab>("new_applications");
	const [vendors, setVendors] = useState<Vendor[]>([]);
	const [loading, setLoading] = useState(true);
	const [processingVendorIds, setProcessingVendorIds] = useState<Set<string>>(
		new Set()
	);
	...
	const updateStatusCounts = () => {
		const counts = {
			new_applications: vendors.filter((v) => v.status === "pending_approval")
				.length,
			approved: vendors.filter((v) => v.status === "approved").length,
			rejected: vendors.filter((v) => v.status === "rejected").length,
			reverification: vendors.filter(
				(v) => v.status === "pending_reverification"
			).length,
		};
		...
		try {
			...
			if (statusCheckResponse.ok) {
				...
				if (
					...
					console.log(
						`⚠️ Vendor already processed with status: ${currentStatus}`
					);
					await loadVendors();
	...
	const filteredVendors = getVendorsByStatus().filter((vendor) => {
		...
		if (roleFilter !== "all") {
			...
			if (
				...
				!serviceCategory.includes("groom") &&
				!roleName.includes("groom") &&
				!serviceType.includes("groom")
			)
				return false;
			if (
				roleFilter === "vet" &&
				!category.includes("vet") &&
				!serviceCategory.includes("vet") &&
				!roleName.includes("vet") &&
				!serviceType.includes("vet")
			)
				return false;
			if (
				roleFilter === "walking" &&
				!category.includes("walk") &&
				!serviceCategory.includes("walk") &&
				!roleName.includes("walk") &&
				!serviceType.includes("walk")
			)
				return false;
			if (
				roleFilter === "boarding" &&
				!category.includes("board") &&
				!serviceCategory.includes("board") &&
				!roleName.includes("board") &&
				!serviceType.includes("board")
			)
				return false;
			if (
				roleFilter === "training" &&
				!category.includes("train") &&
				!serviceCategory.includes("train") &&
				!roleName.includes("train") &&
				!serviceType.includes("train")
			)
				return false;
		}
		...
		// Priority filter
		if (priorityFilter !== "all" && vendor.priority !== priorityFilter)
			return false;
	...
	const statusTabs = [
		{
			id: "new_applications" as StatusTab,
			label: "New Applications",
			count: statusCounts.new_applications,
		},
		{
			id: "approved" as StatusTab,
			label: "Approved",
			count: statusCounts.approved,
		},
		{
			id: "rejected" as StatusTab,
			label: "Rejected",
			count: statusCounts.rejected,
		},
		{
			id: "reverification" as StatusTab,
			label: "Re-Verification",
			count: statusCounts.reverification,
		},
	];
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
				<div>
					...
					<p className="text-sm text-gray-600">
						Showing {filteredVendors.length} of {getVendorsByStatus().length}{" "}
						vendors
					</p>
				</div>
				<Button
				...
				{loading ? (
					...
					<div className="divide-y divide-gray-100">
						{filteredVendors.map((vendor, index) => (
							<div
								key={`${vendor.vendorId || vendor.id}-${vendor.status}-${index}`}
								className="lg:grid lg:grid-cols-12 gap-4 px-3 sm:px-6 py-4 hover:bg-gray-50 transition-colors"
							>
								{/* Mobile Layout */}
								<div className="lg:hidden space-y-3">
									<div className="flex items-start gap-3">
										<div
											className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${getPriorityColor(vendor.priority || "medium")}`}
										/>
										<div className="flex-1">
											<p className="text-sm text-gray-900">
												{vendor.fullName || vendor.vendorName || "N/A"}
											</p>
											{vendor.businessName && (
									...
									<div className="flex flex-wrap items-center gap-2">
										<span
											className={`inline-block px-2 py-1 rounded text-xs border ${getCategoryBadgeStyle(vendor.category || vendor.serviceCategory || "N/A")}`}
										>
											{vendor.roleName ||
												vendor.serviceCategory ||
												vendor.category ||
									...
									<div className="flex items-center gap-2">
										<div className="flex-1 bg-gray-200 rounded-full h-2">
											<div
												...
												style={{
													width: `${vendor.progressPercentage || vendor.progress || 100}%`,
												}}
											/>
										</div>
										<span className="text-xs text-gray-600 min-w-[35px]">
											{vendor.progressPercentage || vendor.progress || 100}%
										</span>
									</div>
								...
								<div className="hidden lg:contents">
									...
									<div className="col-span-3">
										<div className="flex items-start gap-3">
											<div
												className={`w-2 h-2 rounded-full mt-2 ${getPriorityColor(vendor.priority || "medium")}`}
											/>
											<div>
												<p className="text-sm text-gray-900">
													{vendor.fullName || vendor.vendorName || "N/A"}
